...
import com.decentralchain.generator.utils.Implicits._
...
import com.decentralchain.utils.LoggerFacade
...
object Gen {
  ...
  def transfers(senderGen: Iterator[KeyPair], recipientGen: Iterator[Address], feeGen: Iterator[Long]): Iterator[Transaction] = {
    ...
    senderGen
      ...
      .map {
        case (((src, dst), fee), i) =>
          TransferTransaction.selfSigned(2.toByte, src, dst, unitoken, fee, unitoken, fee, ByteStr.empty, now + i)
      }
    ...
    senderGen
      ...
      .map {
        case ((sender, count), i) =>
          ...
          MassTransferTransaction.selfSigned(1.toByte, sender, unitoken, transfers, fee, now + i, ByteStr.empty)
      }
