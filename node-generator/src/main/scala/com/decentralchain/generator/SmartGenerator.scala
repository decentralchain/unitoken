...
class SmartGenerator(settings: SmartGenerator.Settings, val accounts: Seq[KeyPair], estimator: ScriptEstimator) extends TransactionGenerator {
  ...
  private def generate(settings: SmartGenerator.Settings): Seq[Transaction] = {
    ...
    val setScripts = Range(0, settings.scripts) flatMap (
        _ =>
          accounts.map { i =>
            SetScriptTransaction.selfSigned(1.toByte, i, Some(script), 1.unitoken, System.currentTimeMillis()).explicitGet()
          }
      )
    ...
    val txs = Range(0, settings.transfers).map { i =>
      TransferTransaction
        .selfSigned(2.toByte, bank, bank.toAddress, unitoken, 1.unitoken - 2 * fee, unitoken, fee, ByteStr.empty, now + i)
        .explicitGet()
    }
    ...
    val extxs = Range(0, settings.exchange).map { i =>
      ...
      val pair            = AssetPair(unitoken, Asset.fromCompatId(tradeAssetIssue))
      val sellOrder       = Order.sell(TxVersion.V2, seller, matcher.publicKey, pair, 100000000L, 1, ts, ts + 30.days.toMillis, 0.003.unitoken)
      val buyOrder        = Order.buy(TxVersion.V2, buyer, matcher.publicKey, pair, 100000000L, 1, ts, ts + 1.day.toMillis, 0.003.unitoken)

      ExchangeTransaction.signed(TxVersion.V2, matcher.privateKey, buyOrder, sellOrder, 100000000, 1, 0.003.unitoken, 0.003.unitoken, 0.011.unitoken, ts).explicitGet()
    }
