...
import com.decentralchain.generator.config.FicusImplicits
...
import com.decentralchain.utils.{LoggerFacade, NTP}
...
object TransactionsGeneratorApp extends App with ScoptImplicits with FicusImplicits with EnumerationReader {
  ...
  // IDEA bugs
  implicit val inetSocketAddressReader: ValueReader[InetSocketAddress] = com.decentralchain.settings.inetSocketAddressReader
  ...
  val parser = new OptionParser[GeneratorSettings]("generator") {
    head("TransactionsGenerator - unitoken load testing transactions generator")
  ...
  val unitokenSettings = unitokenSettings.default()
  ...
  parser.parse(args, defaultConfig) match {
    ...
    case Some(finalConfig) =>
      ...
      val estimator = unitokenSettings.estimator
      ...
      val sender = new NetworkSender(unitokenSettings.networkSettings.trafficLogger, finalConfig.addressScheme, "generator", nonce = Random.nextLong())
