...
class ReplTest extends PropSpec with ScriptGen with Matchers with NoShrink {
  ...
  property("unitoken context funcs absent") {
    val repl = Repl()
  ...
  property("url slash strip") {
    val url = "testnodes.unitokennodes.com"
  ...
  property("reconfigure") {
    ...
    val settings = NodeConnectionSettings("testnodes.unitokennodes.com", 'T'.toByte, address1)
