package com.decentralchain.lang.v1.evaluator.ctx.impl.unitoken
...
import com.decentralchain.lang.v1.compiler.Types.{BOOLEAN, BYTESTR, LIST, LONG, STRING, UNION, UNIT, optionLong}
import com.decentralchain.lang.v1.evaluator.FunctionIds._
import com.decentralchain.lang.v1.evaluator.ctx.impl.converters._
import com.decentralchain.lang.v1.evaluator.ctx.impl.unitoken.Bindings.{scriptTransfer => _, _}
import com.decentralchain.lang.v1.evaluator.ctx.impl.unitoken.Types.{addressOrAliasType, addressType, commonDataEntryType, optionAddress, _}
import com.decentralchain.lang.v1.evaluator.ctx.impl.{EnvironmentFunctions, PureContext, notImplemented, unit}
...
import com.decentralchain.lang.v1.traits.domain.{Issue, Recipient}
...
object Functions {
  ...
  val unitokenBalanceV4F: BaseFunction[Environment] =
    NativeFunction.withEnvironment[Environment](
      "unitokenBalance",
      Map[StdLibVersion, Long](V1 -> 100L, V2 -> 100L, V3 -> 100L, V4 -> 10L),
      ACCOUNTunitokenBALANCE,
      balanceDetailsType,
      ("addressOrAlias", addressOrAliasType)
    ) {
      new ContextfulNativeFunction[Environment]("unitokenBalance", LONG, Seq(("addressOrAlias", addressOrAliasType))) {
        override def ev[F[_]: Monad](input: (Environment[F], List[EVALUATED])): F[Either[ExecutionError, EVALUATED]] =
          input match {
            case (env, (c: CaseObj) :: Nil) =>
              env.accountunitokenBalanceOf(caseObjToRecipient(c)).map(_.map(b => CaseObj(balanceDetailsType, Map(
            ...
            case (_, xs) => notImplemented[F, EVALUATED](s"unitokenBalance(a: Address|Alias)", xs)
          }
      }
    }
  ...
  val unitokenBalanceF: BaseFunction[Environment] =
    UserFunction("unitokenBalance", 109, LONG, ("@addressOrAlias", addressOrAliasType)) {
