...
object unitokenDockerPlugin extends AutoPlugin {
    ...
    inTask(docker)(
      Seq(
        ...
        dockerfile := {
          ...
          val bin            = "/opt/unitoken/start-unitoken.sh"

          new Dockerfile {
            from(baseImage.value)

            runRaw(s"""mkdir -p /opt/unitoken && \\
                    |apk update && \\
                    |apk add --no-cache openssl ca-certificates && \\
                    |wget --quiet "https://search.maven.org/remotecontent?filepath=org/aspectj/aspectjweaver/1.9.1/aspectjweaver-1.9.1.jar" -O /opt/unitoken/aspectjweaver.jar && \\
            ...
            add(additionalFiles.value, "/opt/unitoken/")
...
object unitokenDockerKeys {
  val additionalFiles = taskKey[Seq[File]]("Additional files to copy to /opt/unitoken")
