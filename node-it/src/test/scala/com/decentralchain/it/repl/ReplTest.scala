...
class ReplTest extends BaseTransactionSuite with FailedTransactionSuiteLike[String] with OverflowBlock {
  ...
  test("unitoken context") {
    val issuer = miner.createKeyPair()
    val sample = miner.createKeyPair()
    val trans  = miner.transfer(miner.keyPair, issuer.toAddress.toString, 100.unitoken, 1.unitoken, version = TxVersion.V3, waitForTx = true)
    miner.transfer(miner.keyPair, sample.toAddress.toString, 100.unitoken, 1.unitoken, waitForTx = true)
    ...
    miner.setScript(issuer, Some(failDApp), 1.unitoken, waitForTx = true)
    ...
    await(repl.execute(s""" unitokenBalance(Address(base58'${sample.toAddress}')).regular """)) shouldBe Right(s"res12: Int = ${100.unitoken}")
    await(repl.execute(""" this.unitokenBalance() """))
