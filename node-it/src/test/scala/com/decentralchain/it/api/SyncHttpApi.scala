...
import com.decentralchain.state.{AssetDistribution, AssetDistributionPage, DataEntry, Portfolio}
...
object SyncHttpApi extends Assertions {
  ...
  implicit class NodeExtSync(n: Node) extends Assertions with Matchers {
    import com.decentralchain.it.api.AsyncHttpApi.{NodeAsyncHttpApi => async}
  ...
  implicit class NodesExtSync(nodes: Seq[Node]) {
    ...
    import com.decentralchain.it.api.AsyncHttpApi.{NodesAsyncHttpApi => async}
    ...
    def rollbackToBlockId(id: String): Unit = {
      sync(
        Future.traverse(nodes) { node =>
          com.decentralchain.it.api.AsyncHttpApi.NodeAsyncHttpApi(node).rollbackToBlockId(id)
        },
        ConditionAwaitTime
      )
    }
