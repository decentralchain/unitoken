...
import com.decentralchain.account.{AddressOrAlias, AddressScheme, KeyPair}
import com.decentralchain.api.http.ConnectReq
...
import com.decentralchain.common.utils.{Base58, Base64, EitherExt2}
import com.decentralchain.features.api.ActivationStatus
import com.decentralchain.http.DebugMessage._
import com.decentralchain.http.{DebugMessage, RollbackParams, `X-Api-Key`}
...
import com.decentralchain.state.{AssetDistribution, AssetDistributionPage, DataEntry, EmptyDataEntry, Portfolio}
import com.decentralchain.transaction.Asset.{IssuedAsset, unitoken}
import com.decentralchain.transaction.assets._
import com.decentralchain.transaction.assets.exchange.{Order, ExchangeTransaction => ExchangeTx}
...
import com.decentralchain.transaction.{Asset, CreateAliasTransaction, DataTransaction, Proofs, TxVersion}
...
object AsyncHttpApi extends Assertions {
  ...
  implicit class NodeAsyncHttpApi(val n: Node) extends Assertions with Matchers {
    ...
    def payment(sourceAddress: String, recipient: String, amount: Long, fee: Long): Future[Transaction] =
      postJson("/unitoken/payment", PaymentRequest(amount, fee, sourceAddress, recipient)).as[Transaction]
    ...
    def invokeScript(
      ...
      signedTraceBroadcast(
        InvokeScriptTransaction(
          ...
          payment,
          fee,
          feeAssetId.map(aid => IssuedAsset(ByteStr.decodeBase58(aid).get)).getOrElse(Asset.unitoken),
          System.currentTimeMillis(),
          Proofs.empty,
          AddressScheme.current.chainId
    ...
    def updateAssetInfo(
      ...
      val tx = UpdateAssetInfoTransaction(
        ...
        fee,
        if (feeAssetId.isDefined) IssuedAsset(ByteStr(Base58.decode(feeAssetId.get))) else unitoken,
        Proofs.empty,
        AddressScheme.current.chainId
      ).signWith(sender.privateKey)
    ...
    def debugStateAt(height: Long): Future[Map[String, Long]] = getWithApiKey(s"/debug/stateunitoken/$height").as[Map[String, Long]]
