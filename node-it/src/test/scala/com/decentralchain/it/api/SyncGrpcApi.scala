...
import com.decentralchain.protobuf.block.Block.Header
import com.decentralchain.protobuf.block.{PBBlocks, VanillaBlock}
...
object SyncGrpcApi extends Assertions {
  ...
  implicit class NodeExtGrpc(n: Node) {
    ...
    import com.decentralchain.account.{Address => Addr}
    import com.decentralchain.it.api.AsyncGrpcApi.{NodeAsyncGrpcApi => async}
    ...
    def unitokenBalance(address: ByteString): unitokenBalances = {
      accounts.getBalances(BalancesRequest.of(address, Seq(ByteString.EMPTY))).next().getunitoken
    }
