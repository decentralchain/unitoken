...
class TransferTransactionSuite extends BaseTransactionSuite with CancelAfterFailure {
  test("transfer with empty string assetId") {
    val tx = TransferTransaction.selfSigned(2.toByte, sender.keyPair, sender.keyPair.toAddress, unitoken, 100L, unitoken, minFee, ByteStr.empty, System.currentTimeMillis()).explicitGet()
    val json = tx.json() ++ Json.obj("assetId" -> "", "feeAssetId" -> "")
    sender.signedBroadcast(json, waitForTx = true)
  }

  test("asset transfer changes sender's and recipient's asset balance; issuer's.unitoken balance is decreased by fee") {
  ...
  test("unitoken transfer changes unitoken balances and eff.b.") {
    for (v <- transferTxSupportedVersions) {
      val (firstBalance, firstEffBalance)   = miner.accountBalances(firstAddress)
      val (secondBalance, secondEffBalance) = miner.accountBalances(secondAddress)
  ...
  test("invalid signed unitoken transfer should not be in UTX or blockchain") {
    def invalidTx(timestamp: Long = System.currentTimeMillis, fee: Long = 100000): TransferTransaction =
      TransferTransaction
        .selfSigned(1.toByte, sender.keyPair, AddressOrAlias.fromString(sender.address).explicitGet(), unitoken, 1, unitoken, fee, ByteStr.empty, timestamp)
        .explicitGet()
  ...
  test("can not make transfer without having enough balance") {
    for (v <- transferTxSupportedVersions) {
      ...
      assertBadRequestAndResponse(
        sender.transfer(secondKeyPair, firstAddress, secondBalance + 1.unitoken, minFee, version = v),
