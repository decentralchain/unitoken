...
import com.decentralchain.api.http.ApiError.{CustomValidationError, InvalidName, NonPositiveAmount, TooBigArrayAllocation}
...
class IssueTransactionSuite extends BaseTransactionSuite with TableDrivenPropertyChecks {
  test("asset issue changes issuer's asset balance; issuer's unitoken balance is decreased by fee") {
  ...
  test("Not able to create asset when insufficient funds") {
    for (v <- issueTxSupportedVersions) {
      ...
      val bigAssetFee      = eff1 + 1.unitoken
