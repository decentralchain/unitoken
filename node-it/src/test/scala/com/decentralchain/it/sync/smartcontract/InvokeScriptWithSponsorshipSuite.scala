...
class InvokeScriptWithSponsorshipSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("_send unitoken to dApp and caller accounts") {
    sender.transfer(sender.keyPair, dAppAddress, 5.unitoken, minFee, waitForTx = true).id
    sender.transfer(sender.keyPair, callerAddress, 5.unitoken, minFee, waitForTx = true).id
  ...
  test("required fee in sponsored assets considers scripts count") {
    ...
    assertBadRequestAndMessage(
      ...
      s"does not exceed minimal value of 900000 unitoken or $feeAmount"
    )
    ...
    sender.accountBalances(dAppAddress)._1 shouldBe dAppInitBalance - 0.009.unitoken - 0.053.unitoken
  ...
  test("dApp caller is dApp address") {
    ...
    val dAppunitokenBalance = sender.accountBalances(dAppAddress)._1
    ...
    sender.accountBalances(dAppAddress)._1 shouldBe dAppunitokenBalance - 0.009.unitoken
  }
