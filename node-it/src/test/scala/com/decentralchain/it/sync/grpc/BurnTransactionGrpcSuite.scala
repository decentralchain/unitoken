...
class BurnTransactionGrpcSuite extends GrpcBaseTransactionSuite {
  ...
  test("burning assets changes issuer's asset balance; issuer's unitoken balance is decreased by fee") {
    for (v <- burnTxSupportedVersions) {
      ...
      val balance = sender.unitokenBalance(firstAddress)
      sender.broadcastBurn(firstAcc, issuedAssetId, issueAmount / 2, minFee, version = v, waitForTx = true)

      sender.unitokenBalance(firstAddress).available shouldBe balance.available - minFee
      sender.unitokenBalance(firstAddress).effective shouldBe balance.effective - minFee
