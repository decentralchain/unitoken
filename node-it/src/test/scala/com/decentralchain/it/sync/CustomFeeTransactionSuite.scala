...
class CustomFeeTransactionSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  private val assetFee    = 1.unitoken
  ...
  test("make transfer with sponsored asset") {
    ...
    val sponsorship = Sponsorship.tounitoken(transferFee, assetToken)
...
object CustomFeeTransactionSuite {
  ...
  private val minerConfig = ConfigFactory.parseString(s"""unitoken.fees.transfer.$assetId = 100000
                                                         |unitoken.blockchain.custom.functionality {
  ...
  private val notMinerConfig = ConfigFactory.parseString("unitoken.miner.enable=no").withFallback(minerConfig)
