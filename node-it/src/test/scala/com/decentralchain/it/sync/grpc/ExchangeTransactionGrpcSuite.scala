...
import com.decentralchain.common.utils.{Base64, EitherExt2}
...
class ExchangeTransactionGrpcSuite extends GrpcBaseTransactionSuite with NTPTime {
  ...
  test("exchange tx with orders v1,v2") {
    val exchAsset          = sender.broadcastIssue(buyer, Base64.encode("exchAsset".utf8Bytes), someAssetAmount, 8, reissuable = true, 1.unitoken, waitForTx = true)
    ...
    val pair               = AssetPair.createAssetPair("unitoken", exchAssetId).get
    for ((o1ver, o2ver, tver) <- versions) {
      ...
      val buyerunitokenBalanceBefore  = sender.unitokenBalance(buyerAddress).available
      val sellerunitokenBalanceBefore = sender.unitokenBalance(sellerAddress).available
      ...
      sender.unitokenBalance(buyerAddress).available shouldBe buyerunitokenBalanceBefore + amount - matcherFee
      sender.unitokenBalance(sellerAddress).available shouldBe sellerunitokenBalanceBefore - amount - matcherFee
  ...
  test("exchange tx with orders v3") {
    val feeAsset           = sender.broadcastIssue(buyer, "feeAsset", someAssetAmount, 8, reissuable = true, 1.unitoken, waitForTx = true)
    ...
    for ((o1ver, o2ver, matcherFeeOrder1, matcherFeeOrder2, buyerunitokenDelta, sellerunitokenDelta, buyerAssetDelta, sellerAssetDelta) <- Seq(
           (1: Byte, 3: Byte, unitoken, IssuedAsset(feeAssetId), amount - matcherFee, -amount, -priceAssetSpending, priceAssetSpending - matcherFee),
           (1: Byte, 3: Byte, unitoken, unitoken, amount - matcherFee, -amount - matcherFee, -priceAssetSpending, priceAssetSpending),
           (2: Byte, 3: Byte, unitoken, IssuedAsset(feeAssetId), amount - matcherFee, -amount, -priceAssetSpending, priceAssetSpending - matcherFee),
           (3: Byte, 1: Byte, IssuedAsset(feeAssetId), unitoken, amount, -amount - matcherFee, -priceAssetSpending - matcherFee, priceAssetSpending),
           (2: Byte, 3: Byte, unitoken, unitoken, amount - matcherFee, -amount - matcherFee, -priceAssetSpending, priceAssetSpending),
           (3: Byte, 2: Byte, IssuedAsset(feeAssetId), unitoken, amount, -amount - matcherFee, -priceAssetSpending - matcherFee, priceAssetSpending)
         )) {
      if (matcherFeeOrder1 == unitoken && matcherFeeOrder2 != unitoken) {
        sender.broadcastTransfer(
          buyer,
          Recipient().withPublicKeyHash(sellerAddress),
          100000,
      ...
      val ts                  = ntpTime.correctedTime()
      val expirationTimestamp = ts + Order.MaxLiveTime
      val assetPair           = AssetPair.createAssetPair("unitoken", feeAssetId.toString).get
      ...
      sender.unitokenBalance(buyerAddress).available shouldBe (buyerunitokenBalanceBefore + buyerunitokenDelta)
      sender.unitokenBalance(sellerAddress).available shouldBe (sellerunitokenBalanceBefore + sellerunitokenDelta)
  ...
  test("cannot exchange non-issued assets") {
    ...
    for ((o1ver, o2ver, tver) <- versions) {
      ...
      val amount              = 1
      val pair                = AssetPair.createAssetPair("unitoken", assetId).get
      val buy                 = Order.buy(o1ver, buyer, matcher.publicKey, pair, amount, price, ts, expirationTimestamp, matcherFee)
      val sell                = Order.sell(o2ver, seller, matcher.publicKey, pair, amount, price, ts, expirationTimestamp, matcherFee)
