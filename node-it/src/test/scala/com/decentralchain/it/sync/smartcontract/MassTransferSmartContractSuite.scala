...
class MassTransferSmartContractSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("airdrop emulation via MassTransfer") {
    ...
    val unsigned =
      MassTransferTransaction
        .create(1.toByte, notMiner.publicKey, unitoken, transfers, calcMassTransferFee(2) + smartFee, currTime, ByteStr.empty, Proofs.empty)
        .explicitGet()
    ...
    val unsignedToGov =
      MassTransferTransaction
        .create(1.toByte, notMiner.publicKey, unitoken, transfersToGov, calcMassTransferFee(2) + smartFee, currTime, ByteStr.empty, Proofs.empty)
        .explicitGet()
