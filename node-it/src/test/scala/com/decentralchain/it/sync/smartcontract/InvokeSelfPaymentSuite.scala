...
class InvokeSelfPaymentSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("prerequisite: set contract") {
    ...
    val sourceV4 =
      """{-# STDLIB_VERSION 4 #-}
        ...
        |  let id = if asset == "unitoken" then unit else fromBase58String(asset)
  ...
  test("V4: can't invoke itself with payment") {
    for (payment <- List(
           Seq(InvokeScriptTransaction.Payment(1, unitoken)),
           Seq(InvokeScriptTransaction.Payment(1, asset1)),
           Seq(InvokeScriptTransaction.Payment(1, unitoken), InvokeScriptTransaction.Payment(1, asset1))
         )) {
      assertApiError(
  ...
  test("V4: can't send tokens to itself from a script") {
    for (args <- List(
           List(CONST_STRING("unitoken").explicitGet()),
  ...
  test("V3: still can invoke itself") {
    ...
    sender.invokeScript(
      dAppV3,
      dAppV3Address,
      payment = Seq(InvokeScriptTransaction.Payment(1, unitoken)),
  ...
  test("V3: still can pay itself") {
    sender.invokeScript(caller, dAppV3Address, Some("paySelf"), List(CONST_STRING("unitoken").explicitGet()), waitForTx = true)
