...
import com.decentralchain.it.sync.{calcMassTransferFee, setScriptFee, _}
...
class TransferNFTSuite extends BaseTransactionSuite with NTPTime {
  ...
  test("NFT should be correctly transferred via transfer transaction") {
    val nftAsset = sender.issue(caller, assetName, assetDescription, 1, 0, reissuable = false, 1.unitoken / 1000, waitForTx = true).id
  ...
  test("NFT should correctly be transferred via exchange transaction") {
    ...
    val transfers = List(
      Transfer(buyer.toAddress.toString, 10.unitoken),
      Transfer(seller.toAddress.toString, 10.unitoken),
      Transfer(matcher.toAddress.toString, 10.unitoken)
    )
    sender.massTransfer(caller, transfers, calcMassTransferFee(transfers.size), waitForTx = true)

    val nftAsset =
      sender.broadcastIssue(seller, assetName, assetDescription, 1, 0, reissuable = false, 1.unitoken / 1000, waitForTx = true, script = None).id
    val pair = AssetPair.createAssetPair(nftAsset, "unitoken")
