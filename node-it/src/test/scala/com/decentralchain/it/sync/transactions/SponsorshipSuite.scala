...
import com.decentralchain.it.{IntegrationSuiteWithThreeAddresses, NodeConfigs, ReportingTestName}
...
class SponsorshipSuite
  ...
  override def nodeConfigs: Seq[Config] =
    NodeConfigs.newBuilder
      ...
      .overrideBase(_.raw("unitoken.blockchain.custom.functionality.blocks-for-feature-activation=1"))
      .overrideBase(_.raw("unitoken.blockchain.custom.functionality.feature-check-blocks-period=1"))
      .withDefault(1)
  ...
  var sponsorunitokenBalance                   = 0L
  var minerunitokenBalance                     = 0L
  var minerunitokenBalanceAfterFirstXferTest   = 0L
  var sponsorunitokenBalanceAfterFirstXferTest = 0L
  ...
  protected override def beforeAll(): Unit = {
    ...
    sponsorunitokenBalance = sender.accountBalances(sponsorAddress)._2
    minerunitokenBalance = sender.accountBalances(miner.address)._2
    minerunitokenBalanceAfterFirstXferTest = minerunitokenBalance + 2 * issueFee + 2 * sponsorReducedFee + 2 * minFee + 2 * FeeValidation.FeeUnit * SmallFee / minSponsorFee
    sponsorunitokenBalanceAfterFirstXferTest = sponsorunitokenBalance - 2 * issueFee - 2 * sponsorReducedFee - 2 * minFee - 2 * FeeValidation.FeeUnit * SmallFee / minSponsorFee
  ...
  "Fee in sponsored asset works fine for transaction" - {
    ...
    "fee should be written off in issued asset" - {
      ...
      "sponsor should receive sponsored asset as fee, unitoken should be written off" in {
        ...
        miner.assertBalances(sponsorAddress, sponsorunitokenBalanceAfterFirstXferTest)
      }

      "miner unitoken balance should be changed" in {
        miner.assertBalances(miner.address, minerunitokenBalanceAfterFirstXferTest)
      }
    }

    "assets balance should contain sponsor fee info and sponsor balance" in {
      val sponsorLeaseSomeunitoken = sender.lease(sponsor, bobAddress, leasingAmount, leasingFee).id
      nodes.waitForHeightAriseAndTxPresent(sponsorLeaseSomeunitoken)
    ...
    "unitoken fee depends on sponsor fee and sponsored token decimals" in {
      val transferTxCustomLargeFeeAlice1 = sender.transfer(alice, bobAddress, 1.unitoken, LargeFee, None, Some(firstSponsorAssetId)).id
      val transferTxCustomLargeFeeAlice2 = sender.transfer(alice, bobAddress, 1.unitoken, LargeFee, None, Some(secondSponsorAssetId)).id
      ...
      sender.assertBalances(
        sponsorAddress,
        sponsorunitokenBalanceAfterFirstXferTest - FeeValidation.FeeUnit * 2 * LargeFee / Token - leasingFee,
        sponsorunitokenBalanceAfterFirstXferTest - FeeValidation.FeeUnit * 2 * LargeFee / Token - leasingFee - leasingAmount
      )
      miner.assertBalances(miner.address, minerunitokenBalanceAfterFirstXferTest + FeeValidation.FeeUnit * 2 * LargeFee / Token + leasingFee)
    }
    ...
    "cancel sponsorship" - {
      ...
      "check sponsor and miner balances after cancel" in {
        sender.assertBalances(
          sponsorAddress,
          sponsorunitokenBalanceAfterFirstXferTest - FeeValidation.FeeUnit * 2 * LargeFee / Token - leasingFee - 2 * issueFee,
          sponsorunitokenBalanceAfterFirstXferTest - FeeValidation.FeeUnit * 2 * LargeFee / Token - leasingFee - leasingAmount - 2 * issueFee
        )
        miner.assertBalances(
          miner.address,
          minerunitokenBalanceAfterFirstXferTest + FeeValidation.FeeUnit * 2 * LargeFee / Token + leasingFee + 2 * issueFee
        )
      }
    ...
    "set sponsopship again" - {
      ...
      "make transfer with new min sponsored fee" in {
        ...
        val aliceunitokenBalance         = sender.accountBalances(aliceAddress)
        ...
        val bobunitokenBalance           = sender.accountBalances(bobAddress)
        ...
        val transferTxCustomFeeAlice1 = sender.transfer(alice, bobAddress, 1.unitoken, TinyFee, None, Some(firstSponsorAssetId)).id
        val transferTxCustomFeeAlice2 = sender.transfer(alice, bobAddress, 1.unitoken, TinyFee, None, Some(secondSponsorAssetId)).id
        nodes.waitForHeight(
          math.max(
        ...
        val unitokenFee = FeeValidation.FeeUnit * 2 * TinyFee / TinyFee
        sender.assertBalances(sponsorAddress, sponsoredBalance._1 - unitokenFee, sponsoredBalance._2 - unitokenFee)
        ...
        sender.assertBalances(aliceAddress, aliceunitokenBalance._1 - 2.unitoken, aliceunitokenBalance._2 - 2.unitoken)
        sender.assertBalances(bobAddress, bobunitokenBalance._1 + 2.unitoken, bobunitokenBalance._2 + 2.unitoken)
        ...
        miner.assertBalances(miner.address, minerBalance._2 + unitokenFee)
      ...
      "transfer tx sponsored fee is less then new minimal" in {
        assertBadRequestAndResponse(
          sender
            ...
            .id,
          s"Fee for TransferTransaction \\($SmallFee in ${Some(firstSponsorAssetId).get}\\) does not exceed minimal value of 100000 unitoken or $LargeFee ${Some(firstSponsorAssetId).get}"
        )
        assertBadRequestAndResponse(
          sender
            .transfer(
            ...
            .id,
          s"Fee for TransferTransaction \\($SmallFee in ${Some(secondSponsorAssetId).get}\\) does not exceed minimal value of 100000 unitoken or $LargeFee ${Some(secondSponsorAssetId).get}"
        )
      }
      ...
      "make transfer with updated min sponsored fee" in {
        ...
        val transferTxCustomFeeAlice1 = sender.transfer(alice, bobAddress, 1.unitoken, LargeFee, None, Some(firstSponsorAssetId)).id
        val transferTxCustomFeeAlice2 = sender.transfer(alice, bobAddress, 1.unitoken, LargeFee, None, Some(secondSponsorAssetId)).id
        nodes.waitForHeightArise()
        nodes.waitForTransaction(transferTxCustomFeeAlice1)
        nodes.waitForTransaction(transferTxCustomFeeAlice2)
        val unitokenFee = FeeValidation.FeeUnit * 2 * LargeFee / LargeFee
        nodes.waitForHeightArise()
        ...
        sender.assertBalances(aliceAddress, aliceunitokenBalance._1 - 2.unitoken, aliceunitokenBalance._2 - 2.unitoken)
        sender.assertBalances(bobAddress, bobunitokenBalance._1 + 2.unitoken, bobunitokenBalance._2 + 2.unitoken)
        miner.assertBalances(miner.address, minerBalance._1 + unitokenFee, minerBalance._2 + unitokenFee)
      }
    ...
    "issue asset make sponsor and burn and reissue" in {
      ...
      val aliceTransferunitoken1 = sender.transfer(alice, bobAddress, transferAmount, SmallFee, None, Some(firstSponsorAssetId2), waitForTx = true).id
      val aliceTransferunitoken2 = sender.transfer(alice, bobAddress, transferAmount, SmallFee, None, Some(secondSponsorAssetId2), waitForTx = true).id
      nodes.waitForHeightAriseAndTxPresent(aliceTransferunitoken1)
      nodes.waitForHeightAriseAndTxPresent(aliceTransferunitoken2)

      val totalunitokenFee = FeeValidation.FeeUnit * 2 * SmallFee / Token + 2 * issueFee + 2 * sponsorReducedFee + 2 * burnFee + 2 * minFee + 2 * issueFee
      miner.assertBalances(miner.address, minerBalance._1 + totalunitokenFee, minerBalance._2 + totalunitokenFee)
      sender.assertBalances(sponsorAddress, sponsorBalance._1 - totalunitokenFee, sponsorBalance._2 - totalunitokenFee)
    ...
    "miner is sponsor" in {
      ...
      val aliceFirstTransferunitokenId  = sender.transfer(alice, bobAddress, transferAmount, SmallFee, None, Some(firstMinersAsset)).id
      val aliceSecondTransferunitokenId = sender.transfer(alice, bobAddress, transferAmount, SmallFee, None, Some(secondMinersAsset)).id
      nodes.waitForHeightAriseAndTxPresent(aliceFirstTransferunitokenId)
      nodes.waitForHeightAriseAndTxPresent(aliceSecondTransferunitokenId)
