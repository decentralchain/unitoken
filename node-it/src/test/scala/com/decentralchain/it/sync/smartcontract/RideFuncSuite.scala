...
class RideFuncSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("assetBalance() verification") {
    ...
    sender.transfer(firstKeyPair, newAddress.toAddress.toString, 10.unitoken, minFee, waitForTx = true)
    ...
    assertBadRequestAndResponse(
      sender.signedBroadcast(
        TransferTransaction.selfSigned(2.toByte, pkNewAddress, pkNewAddress.toAddress, unitoken, 1.unitoken, unitoken, smartMinFee, ByteStr.empty,  System.currentTimeMillis())
          .explicitGet()
          .json()),
    ...
    sender.signedBroadcast(
      TransferTransaction.selfSigned(2.toByte, firstKeyPair, pkNewAddress.toAddress, IssuedAsset(ByteStr.decodeBase58(asset).get), 100000000, unitoken, smartMinFee, ByteStr.empty,  System.currentTimeMillis())
        .explicitGet()
        .json(),
      waitForTx = true
    )
    ...
    val udpatedScript =
      s"""
         |match tx {
         |  case _: SetScriptTransaction => true
         |  case _ => assetBalance(tx.sender, base58'$asset') >= 900000000 && unitokenBalance(tx.sender) >500000000
         |}
      """.stripMargin
    ...
    sender.signedBroadcast(
      TransferTransaction.selfSigned(2.toByte, firstKeyPair, pkNewAddress.toAddress, IssuedAsset(ByteStr.decodeBase58(asset).get), 800000000, unitoken, smartMinFee, ByteStr.empty,  System.currentTimeMillis())
        .explicitGet()
        .json(),
      waitForTx = true
    )
  ...
  test("split around empty string") {
    ...
    val transfer = TransferTransaction.selfSigned(2.toByte, pkNewAddress, pkNewAddress.toAddress, unitoken, 1.unitoken, unitoken, smartMinFee, ByteStr.empty,  System.currentTimeMillis())
  ...
  test("lastBlock and blockInfoByHeight(last) must return liquid block") {
    ...
    val transfer = sender.transfer(newAddress, newAddress.toAddress.toString, 1.unitoken, minFee + (2 * smartFee))
    nodes.waitForHeightAriseAndTxPresent(transfer.id)
  }
}
