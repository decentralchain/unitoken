...
class InvokeScriptTransactionStateChangesSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("prepare") {
    ...
    sender.massTransfer(contract, List(Transfer(callerAddress, 3000), Transfer(recipientAddress, 3000)), 0.01.unitoken, assetId = Some(simpleAsset))
    sender.massTransfer(
      contract,
    ...
    val script = ScriptCompiler
      .compile(
        ...
        |
        |@Callable(i)
        |func writeAndSendunitoken(value: Int, recipient: String, amount: Int) = {
        |    ScriptResult(
        |        WriteSet([DataEntry("result", value)]),
  ...
  test("sponsored by recipient") {
    val invokeTx = sender.invokeScript(
      caller,
      contractAddress,
      func = Some("writeAndSendunitoken"),
  ...
  test("state changes order") {
    val script = ScriptCompiler
      ...
      .bytes()
      .base64
    sender.setScript(contract, Some(script), setScriptFee + 0.4.unitoken, waitForTx = true)
