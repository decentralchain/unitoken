...
import com.decentralchain.it.api.AsyncNetworkApi._
...
class SimpleTransactionsSuite extends BaseTransactionSuite with Matchers {
  ...
  test("valid tx send by network to node should be in blockchain") {
    val tx = TransferTransaction.selfSigned(1.toByte, node.keyPair, Address.fromString(node.address).explicitGet(), unitoken, 1L, unitoken, minFee, ByteStr.empty,  System.currentTimeMillis())
      .explicitGet()
