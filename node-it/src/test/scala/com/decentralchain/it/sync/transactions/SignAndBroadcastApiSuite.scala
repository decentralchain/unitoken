...
import com.decentralchain.transaction.assets.exchange.{AssetPair, Order, _}
...
class SignAndBroadcastApiSuite extends BaseTransactionSuite with NTPTime with BeforeAndAfterAll {
  test("height should always be reported for transactions") {
    val txId = sender.transfer(firstKeyPair, secondAddress, 1.unitoken, fee = minFee).id
  ...
  test("/transactions/sign should produce issue/reissue/burn/transfer transactions that are good for /transactions/broadcast") {
    for (v <- supportedVersions) {
      ...
      signBroadcastAndCalcFee(
        Json.obj("type" -> ReissueTransaction.typeId, "quantity" -> 200.unitoken, "assetId" -> issueId, "sender" -> firstAddress, "reissuable" -> false),
        usesProofs = isProof,
        version = v
      )
      ...
      signBroadcastAndCalcFee(
        Json.obj("type" -> BurnTransaction.typeId, "amount" -> 100.unitoken, "assetId" -> issueId, "sender" -> firstAddress),
        usesProofs = isProof,
        version = v
      )
  ...
  test("/transactions/sign should produce mass transfer transaction that is good for /transactions/broadcast") {
    signBroadcastAndCalcFee(
      Json.obj(
        ...
        "version"    -> 1,
        "sender"     -> firstAddress,
        "transfers"  -> Json.toJson(Seq(Transfer(secondAddress, 1.unitoken), Transfer(thirdAddress, 2.unitoken))),
  ...
  test("/transactions/broadcast should produce ExchangeTransaction with custom asset") {
    ...
    val versionsWithunitokenFee =
      (transactionV1versions +: transactionV2versions)
        .map { case (o1ver, o2ver, tver) => (o1ver, o2ver, tver, unitoken, unitoken) }

    val versionsWithAssetFee = for {
      o2ver <- 1 to 3
      buyMatcherFeeAssetId  = assetId
      sellMatcherFeeAssetId = unitoken
    } yield (3.toByte, o2ver.toByte, 2.toByte, buyMatcherFeeAssetId, sellMatcherFeeAssetId)

    for ((o1ver, o2ver, tver, matcherFeeOrder1, matcherFeeOrder2) <- versionsWithunitokenFee ++ versionsWithAssetFee) {
      ...
      val buyAmount           = 2
      val sellAmount          = 3
      val assetPair           = AssetPair.createAssetPair("unitoken", issueTx).get
