...
class TransferTxFromProtoSuite extends BaseTransactionSuite {
  ...
  private val scriptText =
    s"""
      ...
      |    let assetId = if (!transferTx.assetId.isDefined()) then {"unitoken"} else {transferTx.assetId.value().toBase58String()}
      |    let feeAssetId = if (!transferTx.feeAssetId.isDefined()) then {"unitoken"} else {transferTx.feeAssetId.value().toBase58String()}
      |[
  ...
  test("TransferTransaction with unitoken from proto bytes") {
    ...
    val transferTx = TransferTransaction
      .selfSigned(
        ...
        feeAsset = unitoken,
        fee = minFee,
        attachment = ByteStr("unitoken transfer".getBytes),
    ...
    sender.getDataByKey(dAppAddress, "assetId").value shouldBe "unitoken"
    sender.getDataByKey(dAppAddress, "feeAssetId").value shouldBe "unitoken"
    sender.getDataByKey(dAppAddress, "attachment").value shouldBe Base58.encode("unitoken transfer".getBytes)
