...
class VRFProtobufActivationSuite extends BaseTransactionSuite {
  ...
  override protected def nodeConfigs: Seq[Config] =
    NodeConfigs
      ...
      .overrideBase(_.raw(s"unitoken.blockchain.custom.functionality.min-asset-info-update-interval = $updateInterval"))
      .buildNonConflicting()
  ...
  test("able to broadcast tx of new version after activation") {
    val senderunitokenBalance    = sender.balanceDetails(senderAcc.toAddress.toString)
    val recipientunitokenBalance = sender.balanceDetails(recipientAcc.toAddress.toString)
    sender.transfer(senderAcc, recipientAcc.toAddress.toString, transferAmount, version = TxVersion.V3, waitForTx = true)

    sender.balanceDetails(senderAcc.toAddress.toString).available shouldBe senderunitokenBalance.available - transferAmount - minFee
    sender.balanceDetails(recipientAcc.toAddress.toString).available shouldBe recipientunitokenBalance.available + transferAmount
  }
  ...
  private def mkOrders: (Order, Order) = {
    ...
    val buyOrder = Order.buy(
      ...
      sender.keyPair,
      sender.publicKey,
      AssetPair.createAssetPair("unitoken", assetId).get,
      amount,
      price,
      ts,
