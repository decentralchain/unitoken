...
class ExchangeTransactionSuite extends BaseTransactionSuite with NTPTime {
  ...
  test("negative - check orders v2 and v3 with exchange tx v1") {
    ...
    val pair = AssetPair.createAssetPair("unitoken", exchAsset.id().toString).get
  ...
  test("exchange tx with orders v3") {
    ...
    for ((o1ver, o2ver, matcherFeeOrder1, matcherFeeOrder2) <- Seq(
           (1: Byte, 3: Byte, unitoken, IssuedAsset(assetId)),
           (1: Byte, 3: Byte, unitoken, unitoken),
           (2: Byte, 3: Byte, unitoken, IssuedAsset(assetId)),
           (3: Byte, 1: Byte, IssuedAsset(assetId), unitoken),
           (2: Byte, 3: Byte, unitoken, unitoken),
           (3: Byte, 2: Byte, IssuedAsset(assetId), unitoken)
         )) {
      ...
      val assetPair  = AssetPair.createAssetPair("unitoken", assetId.toString).get
  ...
  test("exchange tx with orders v4 can use price that is impossible for orders v3/v2/v1") {
    ...
    sender.transfer(sender.keyPair, firstAddress, 1000.unitoken, waitForTx = true)
    ...
    val expirationTimestamp = ts + Order.MaxLiveTime
    val amount              = 1
    val nftunitokenPrice       = 1000 * math.pow(10, 8).toLong
    val nftForAssetPrice    = 1 * math.pow(10, 8).toLong

    val nftunitokenPair      = AssetPair.createAssetPair(nftAsset, "unitoken").get
    val nftOtherAssetPair = AssetPair.createAssetPair(nftAsset, dec6AssetId).get

    val sellNftForunitoken =
      Order.sell(4.toByte, seller, matcher.publicKey, nftunitokenPair, amount, nftunitokenPrice, ts, expirationTimestamp, matcherFee, unitoken)
    val buyNftForunitoken =
      Order.buy(4.toByte, buyer, matcher.publicKey, nftunitokenPair, amount, nftunitokenPrice, ts, expirationTimestamp, matcherFee, unitoken)

    val sellNftForOtherAsset =
      Order.sell(4.toByte, buyer, matcher.publicKey, nftOtherAssetPair, amount, nftForAssetPrice, ts, expirationTimestamp, matcherFee, unitoken)
    val buyNftForOtherAsset =
      Order.buy(4.toByte, seller, matcher.publicKey, nftOtherAssetPair, amount, nftForAssetPrice, ts, expirationTimestamp, matcherFee, unitoken)
    ...
    val tx =
      ExchangeTransaction
        .signed(
          3.toByte,
          matcher = matcher.privateKey,
          order1 = buyNftForunitoken,
          order2 = sellNftForunitoken,
          amount = amount,
          price = nftunitokenPrice,
          buyMatcherFee = (BigInt(matcherFee) * amount / sellNftForunitoken.amount).toLong,
          sellMatcherFee = (BigInt(matcherFee) * amount / sellNftForunitoken.amount).toLong,
          fee = matcherFee,
          timestamp = ntpTime.correctedTime()
        )
        .explicitGet()
    ...
    sender.balanceDetails(sellerAddress).regular shouldBe sellerBalance + nftunitokenPrice - matcherFee
    sender.balanceDetails(buyerAddress).regular shouldBe buyerBalance - nftunitokenPrice - matcherFee
