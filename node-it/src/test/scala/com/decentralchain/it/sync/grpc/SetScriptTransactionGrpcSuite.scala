...
import com.decentralchain.protobuf.transaction.{PBTransactions, Recipient, SetScriptTransactionData, SignedTransaction, TransferTransactionData, Transaction => PBTransaction}
...
class SetScriptTransactionGrpcSuite extends GrpcBaseTransactionSuite {
  ...
  test("able to broadcast tx if that is allowed by account-script") {
    for (v <- setScrTxSupportedVersions) {
      val (contract, contractAddr) = if (v < 2) (firstAcc, firstAddress) else (secondAcc, secondAddress)
      val firstBalance             = sender.unitokenBalance(contractAddr).available
      val thirdBalance             = sender.unitokenBalance(thirdAddress).available
      ...
      sender.unitokenBalance(contractAddr).available shouldBe firstBalance - transferAmount - transferFee
      sender.unitokenBalance(thirdAddress).available shouldBe thirdBalance + transferAmount
    }
  }
  ...
  test("able to clear script from scripted account") {
    for (v <- setScrTxSupportedVersions) {
      ...
      val contractBalance = sender.unitokenBalance(contractAddr).available
      val thirdBalance    = sender.unitokenBalance(thirdAddress).available

      sender.broadcastTransfer(contract, Recipient().withPublicKeyHash(thirdAddress), transferAmount, minFee, waitForTx = true)

      sender.unitokenBalance(contractAddr).available shouldBe contractBalance - transferAmount - minFee
      sender.unitokenBalance(thirdAddress).available shouldBe thirdBalance + transferAmount
    }
  }
  ...
  test("not able to broadcast tx from scripted acc if tx fee doesn't include smart fee") {
    for (v <- setScrTxSupportedVersions) {
      ...
      val contractBalance    = sender.unitokenBalance(contractAddr).available
      val contractEffBalance = sender.unitokenBalance(contractAddr).effective
      val thirdBalance       = sender.unitokenBalance(thirdAddress).available
      val thirdEffBalance    = sender.unitokenBalance(thirdAddress).effective
      ...
      sender.unitokenBalance(contractAddr).available shouldBe contractBalance
      sender.unitokenBalance(contractAddr).effective shouldBe contractEffBalance
      sender.unitokenBalance(thirdAddress).available shouldBe thirdBalance
      sender.unitokenBalance(thirdAddress).effective shouldBe thirdEffBalance
    }
  }
}
