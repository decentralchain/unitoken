...
import com.decentralchain.it._
import com.decentralchain.it.api.AsyncHttpApi._
import com.decentralchain.it.api._
  ...
  protected override def beforeAll(): Unit = {
    ...
    val fundAndIssue =
      for {
        ...
        fundTx <- node.transfer(node.address, issuer.toAddress.toString, 1000.unitoken, 0.001.unitoken)
  ...
  "after activation" - {
    ...
    "returns only nft with balance > 0 on /nft/{address}/limit/{limit}" in {
      ...
      val transfer = TransferTransaction
        .selfSigned(1.toByte, issuer, other.toAddress, randomTokenToTransfer, 1, unitoken, 0.001.unitoken, ByteStr.empty, System.currentTimeMillis())
        .explicitGet()
    ...
    val transactions =
      Future.sequence(addrs map { addr =>
        NodeAsyncHttpApi(faucet)
          .transfer(faucet.address, addr, 1000.unitoken, 0.001.unitoken)
          .flatMap { tx =>
            NodeAsyncHttpApi(faucet)
