...
class ReissueTransactionGrpcSuite extends GrpcBaseTransactionSuite with NTPTime {
  ...
  test("asset reissue changes issuer's asset balance; issuer's unitoken balance is decreased by fee") {
    for (v <- reissueTxSupportedVersions) {
      val reissuerBalance = sender.unitokenBalance(reissuerAddress).available
      val reissuerEffBalance = sender.unitokenBalance(reissuerAddress).effective
      ...
      sender.unitokenBalance(reissuerAddress).available shouldBe reissuerBalance - issueFee - reissueFee
      sender.unitokenBalance(reissuerAddress).effective shouldBe reissuerEffBalance - issueFee - reissueFee
  ...
  test("can't reissue not reissuable asset") {
    for (v <- reissueTxSupportedVersions) {
      ...
      sender.unitokenBalance(reissuerAddress).available shouldBe reissuerBalance - issueFee
      sender.unitokenBalance(reissuerAddress).effective shouldBe reissuerEffBalance - issueFee
  ...
  test("not able to reissue if cannot pay fee - insufficient funds") {
    for (v <- reissueTxSupportedVersions) {
      val reissuerBalance = sender.unitokenBalance(reissuerAddress).available
      val reissuerEffBalance = sender.unitokenBalance(reissuerAddress).effective
      val hugeReissueFee = reissuerEffBalance + 1.unitoken
  ...
  test("able to transfer new reissued amount of assets") {
    for (v <- reissueTxSupportedVersions) {
      ...
      sender.unitokenBalance(reissuerAddress).available shouldBe reissuerBalance - issueFee - reissueFee - minFee
      sender.unitokenBalance(reissuerAddress).effective shouldBe reissuerEffBalance - issueFee - reissueFee - minFee
