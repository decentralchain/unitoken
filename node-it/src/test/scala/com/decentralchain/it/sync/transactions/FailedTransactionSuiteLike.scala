...
import com.decentralchain.api.grpc.{ApplicationStatus, TransactionsByIdRequest, TransactionStatus => PBTransactionStatus}
...
trait FailedTransactionSuiteLike[T] extends ScorexLogging { _: Matchers =>
  ...
  def sendTxsAndThenPriorityTx[S](t: Int => T, pt: () => T)(
    ...
    val maxTxsInMicroBlock = sender.config.getInt("unitoken.miner.max-transactions-in-micro-block")
...
object FailedTransactionSuiteLike {
  ...
  val configForMinMicroblockAge: Config = ConfigFactory.parseString(s"""
     |unitoken.miner.min-micro-block-age = 7
     |unitoken.miner.max-transactions-in-micro-block = 1
     |""".stripMargin)

  val Configs: Seq[Config] =
    NodeConfigs.newBuilder
      .overrideBase(_.quorum(0))
      .overrideBase(_.raw(s"unitoken.miner.max-transactions-in-micro-block = 50"))
      .withDefault(1)
