...
class IssueTransactionGrpcSuite extends GrpcBaseTransactionSuite with NTPTime with TableDrivenPropertyChecks {
  ...
  test("asset issue changes issuer's asset balance") {
    for (v <- issueTxSupportedVersions) {
      ...
      val issuerBalance    = sender.unitokenBalance(issuerAddress).available
      val issuerEffBalance = sender.unitokenBalance(issuerAddress).effective
      ...
      sender.unitokenBalance(issuerAddress).available shouldBe issuerBalance - issueFee
      sender.unitokenBalance(issuerAddress).effective shouldBe issuerEffBalance - issueFee
  ...
  test("not able to issue asset with fee less then issueFee (minFee for NFT)") {
    for (v <- issueTxSupportedVersions) {
      ...
      sender.unitokenBalance(issuerAddress).available shouldBe issuerBalance
      sender.unitokenBalance(issuerAddress).effective shouldBe issuerEffBalance
    }
  }
  ...
  test("Not able to create asset when insufficient funds") {
    ...
    val issuerEffBalance = sender.unitokenBalance(issuerAddress).effective
    val bigAssetFee      = issuerEffBalance + 1.unitoken
