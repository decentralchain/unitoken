...
class SetAssetScriptGrpcSuite extends GrpcBaseTransactionSuite {
  ...
  test("sender's unitoken balance is decreased by fee") {
    ...
    sender.unitokenBalance(firstAddress).available shouldBe firstBalance - setAssetScriptFee
    sender.unitokenBalance(firstAddress).effective shouldBe firstEffBalance - setAssetScriptFee
  }

  test("not able set script without having enough unitoken") {
    val firstBalance    = sender.unitokenBalance(firstAddress).available
    val firstEffBalance = sender.unitokenBalance(firstAddress).effective
    assertGrpcError(
  ...
  test("try to make SetAssetScript for asset v1") {
    ...
    val balance    = sender.unitokenBalance(thirdAddress).available
    val effBalance = sender.unitokenBalance(thirdAddress).effective
    ...
    sender.unitokenBalance(thirdAddress).available shouldBe balance
    sender.unitokenBalance(thirdAddress).effective shouldBe effBalance
