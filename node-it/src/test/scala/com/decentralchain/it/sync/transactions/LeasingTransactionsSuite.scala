...
class LeasingTransactionsSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("can not make leasing without having enough balance") {
    for (v <- leaseTxSupportedVersions) {
      ...
      assertBadRequestAndResponse(sender.lease(secondKeyPair, firstAddress, balance2 + 1.unitoken, minFee, version = v), errorMessage)
  ...
  test("can not make leasing to yourself") {
    for (v <- leaseTxSupportedVersions) {
      val (balance1, eff1) = miner.accountBalances(firstAddress)
      assertBadRequestAndResponse(sender.lease(firstKeyPair, firstAddress, balance1 + 1.unitoken, minFee, v), "Transaction to yourself")
      nodes.waitForHeightArise()
