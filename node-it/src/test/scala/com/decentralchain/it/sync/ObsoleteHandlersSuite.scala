...
import com.decentralchain.api.http.requests.{CreateAliasRequest, DataRequest, LeaseCancelRequest, LeaseRequest, MassTransferRequest, SponsorFeeRequest, TransferRequest}
...
class ObsoleteHandlersSuite extends BaseTransactionSuite with BeforeAndAfterAll {
  ...
  test("assets masstransfer") {
    ...
    val transfers = List(Transfer(secondAddress, 1.unitoken), Transfer(thirdAddress, 2.unitoken))
