...
import com.decentralchain.it.sync.smartcontract.{cryptoContextScript, pureContextScript, unitokenContextScript, _}
...
class ExchangeSmartAssetsSuite extends BaseTransactionSuite with CancelAfterFailure with NTPTime {
  ...
  test("require using a certain matcher with smart accounts") {
    ...
    val smartPair = AssetPair(IssuedAsset(ByteStr.decodeBase58(sAsset).get), unitoken)
  ...
  test("AssetPair from smart assets") {
    ...
    withClue("check fee for smart accounts and smart AssetPair - extx.fee == 0.015.unitoken") {
      ...
      assertBadRequestAndMessage(
        ...
        "does not exceed minimal value of 1500000 unitoken"
      )
  ...
  test("use all functions from RIDE for asset script") {
    ...
    val script3 = Some(ScriptCompiler(unitokenContextScript(dtx, false), isAssetScript = true, estimator).explicitGet()._1.bytes().base64)
    ...
    List(script1, script2, script3)
      .map { i =>
        ...
        val smartPair = AssetPair(IssuedAsset(ByteStr.decodeBase58(asset).get), unitoken)
