...
class InvokeScriptWithSmartAccountAndAssetSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("invoke by smart account requires just 1 extra fee") {
    assertBadRequestAndMessage(
      sender.invokeScript(
        ...
        dAppAddress,
        Some("justWriteData"),
        fee = 0.00899999.unitoken
      ),
      s"does not exceed minimal value of 900000 unitoken"
    )
  }

  test("max fee is 0.053 unitoken (0.005 + extraFee(1 smart caller + 1 payment + 10 transfers))") {
    ...
    assertApiError(
      sender
        .invokeScript(
          ...
          fee = 0.05299999.unitoken
        ),
      AssertiveApiError(
        ScriptExecutionError.Id,
        "Error while executing account-script: Fee in unitoken for InvokeScriptTransaction (5299999 in unitoken) with 12 total scripts invoked does not exceed minimal value of 5300000 unitoken."
      )
    )
  ...
  test("can't invoke with small fee for both smart assets") {
    ...
    assertBadRequestAndMessage(
      sender
        ...
        ._1
        .id,
      "does not exceed minimal value of 900000 unitoken"
    )
  }
  ...
  test("can't invoke with small fee for one of smart assets") {
    ...
    assertApiError(
      ...
      AssertiveApiError(
        ScriptExecutionError.Id,
        "Error while executing account-script: Fee in unitoken for InvokeScriptTransaction (900000 in unitoken) with 2 total scripts invoked does not exceed minimal value of 1300000 unitoken."
      )
    )
  }
  ...
  protected override def beforeAll(): Unit = {
    ...
    withClue("send unitoken to dApp and caller accounts") {
      val dAppTransferId        = sender.transfer(sender.keyPair, dAppAddress, 5.unitoken, minFee).id
      val callerTransferId      = sender.transfer(sender.keyPair, callerAddress, 5.unitoken, minFee).id
      val smartCallerTransferId = sender.transfer(sender.keyPair, smartCallerAddress, 5.unitoken, minFee).id
