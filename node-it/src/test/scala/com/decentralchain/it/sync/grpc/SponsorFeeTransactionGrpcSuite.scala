...
class SponsorFeeTransactionGrpcSuite extends GrpcBaseTransactionSuite {
  ...
  test("able to make transfer with sponsored fee") {
    for (v <- sponsorFeeTxSupportedVersions) {
      val minerunitokenBalance = sender.unitokenBalance(ByteString.copyFrom(Base58.decode(miner.address)))
      ...
      val aliceunitokenBalance = sender.unitokenBalance(aliceAddress)
      val bobunitokenBalance = sender.unitokenBalance(bobAddress)
      val sponsorunitokenBalance = sender.unitokenBalance(sponsorAddress)
      ...
      sender.unitokenBalance(aliceAddress).available shouldBe aliceunitokenBalance.available
      sender.unitokenBalance(bobAddress).available shouldBe bobunitokenBalance.available
      sender.unitokenBalance(sponsorAddress).available shouldBe sponsorunitokenBalance.available - FeeValidation.FeeUnit * smallFee / minSponsorFee
      ...
      sender.unitokenBalance(ByteString.copyFrom(Base58.decode(miner.address))).available shouldBe
        minerunitokenBalance.available + reward + sponsorReducedFee + issueFee + minFee + FeeValidation.FeeUnit * smallFee / minSponsorFee
    }
  }
  ...
  test("sponsor is able to update amount of sponsored fee") {
    for (v <- sponsorFeeTxSupportedVersions) {
      ...
      assertGrpcError(
        sender.broadcastTransfer(alice, Recipient().withPublicKeyHash(bobAddress), 10 * token, smallFee, assetId = sponsoredAssetId, feeAssetId = sponsoredAssetId, waitForTx = true),
        s"does not exceed minimal value of $minFee unitoken or $largeFee $sponsoredAssetId",
        Code.INVALID_ARGUMENT
      )
      val aliceunitokenBalance = sender.unitokenBalance(aliceAddress)
      val bobunitokenBalance = sender.unitokenBalance(bobAddress)
      ...
      sender.unitokenBalance(aliceAddress).available shouldBe aliceunitokenBalance.available
      sender.unitokenBalance(bobAddress).available shouldBe bobunitokenBalance.available
      sender.unitokenBalance(sponsorAddress).available shouldBe sponsorunitokenBalance.available - FeeValidation.FeeUnit * largeFee / largeFee
