...
import com.decentralchain.api.http.ApiError.{ScriptExecutionError, TransactionNotAllowedByAccountScript, TransactionNotAllowedByAssetScript}
import com.decentralchain.common.state.ByteStr
import com.decentralchain.common.utils.EitherExt2
import com.decentralchain.http.DebugMessage
import com.decentralchain.it.api.SyncHttpApi._
import com.decentralchain.it.api.{DebugStateChanges, TransactionStatus}
...
class FailedTransactionSuite extends BaseTransactionSuite with CancelAfterFailure with FailedTransactionSuiteLike[String] with OverflowBlock {
  ...
  protected override def beforeAll(): Unit = {
    ...
    sender.transfer(sender.keyPair, contractAddress, 100.unitoken, minFee, waitForTx = true)
  ...
  test("InvokeScriptTransaction: insufficient action fees propagates failed transaction") {
    ...
    for (typeName <- Seq("transfer", "issue", "reissue", "burn")) {
      ...
      sendTxsAndThenPriorityTx(
        ...
        val text = s"Fee in unitoken for InvokeScriptTransaction ($invokeFee in unitoken)" +
          s" with $scriptInvoked total scripts invoked does not exceed minimal value of $minFee unitoken."
  ...
  test("InvokeScriptTransaction: invoke script error in action asset propagates failed transaction") {
    val invokeFee            = 0.005.unitoken + smartFee
    val setAssetScriptMinFee = setAssetScriptFee + smartFee
    val priorityFee          = setAssetScriptMinFee + invokeFee
  ...
  test("InvokeScriptTransaction: sponsored fee on failed transaction should be charged correctly") {
    val invokeFee            = 0.005.unitoken + smartFee
    val invokeFeeInAsset     = invokeFee / 100000 // assetFee = feeInunitoken / feeUnit * sponsorship
    val setAssetScriptMinFee = setAssetScriptFee + smartFee
    val priorityFee          = setAssetScriptMinFee + invokeFee
  ...
  test("ExchangeTransaction: transaction validates as failed when asset script fails") {
    ...
    val fee            = 0.003.unitoken + 4 * smartFee
  ...
  test("ExchangeTransaction: invalid exchange tx when account script fails") {
    ...
    val assetPair      = AssetPair.createAssetPair(amountAsset, priceAsset).get
    val fee            = 0.003.unitoken + smartFee
    val sellMatcherFee = fee / 100000L
    val buyMatcherFee  = fee / 100000L
  ...
  test("InvokeScriptTransaction: revalidate transactions returned to UTXPool because of `min-micro-block-age`") {
    ...
    sender.transfer(sender.keyPair, caller.toAddress.toString, 100.unitoken, minFee, waitForTx = true)
  ...
  private def exchangePreconditions(initScript: Option[String]): Precondition = {
    val transfers = Seq(
      sender.transfer(sender.keyPair, sellerAddress.toAddress.toString, 100.unitoken).id,
      sender.transfer(sender.keyPair, buyerAddress.toAddress.toString, 100.unitoken).id,
      sender.transfer(sender.keyPair, matcherAddress.toAddress.toString, 100.unitoken).id
    )
