...
import com.decentralchain.it.sync.{calcDataFee, minFee, _}
...
class DataTransactionSuite extends BaseTransactionSuite with EitherValues {
  ...
  protected override def beforeAll(): Unit = {
    ...
    sender.postForm("/addresses")
    sender.transfer(firstKeyPair, fourthAddress, 10.unitoken, minFee, waitForTx = true)
  }
  ...
  test("cannot broadcast data without having enough unitoken") {
    for (v <- dataTxSupportedVersions) {
      val (balance1, eff1) = miner.accountBalances(firstAddress)
  ...
  test("try to send tx above limits of key, value and entries count") {
    for (v <- dataTxSupportedVersions) {
      ...
      assertBadRequestAndResponse(postDataTxJson(firstKeyPair, extraValueData, 1.unitoken, version = v), TooBig)
      nodes.waitForHeightArise()

      val largeBinData = List.tabulate(5)(n => BinaryDataEntry(extraKey, ByteStr(Array.fill(maxValueSize)(n.toByte))))
      assertBadRequestAndResponse(postDataTxJson(firstKeyPair, largeBinData, 1.unitoken, version = v), TooBig)
      nodes.waitForHeightArise()

      val largeStrData = List.tabulate(5)(n => StringDataEntry(extraKey, "A" * maxValueSize))
      assertBadRequestAndResponse(postDataTxJson(firstKeyPair, largeStrData, 1.unitoken, version = v), TooBig)
      nodes.waitForHeightArise()

      val tooManyEntriesData = List.fill(maxEntryCount + 1)(IntegerDataEntry("key", 88))
      assertBadRequestAndResponse(postDataTxJson(firstKeyPair, tooManyEntriesData, 1.unitoken, version = v), TooBig)
      nodes.waitForHeightArise()
    }
  }
  ...
  test("put data in liquid block") {
    ...
    sender.transfer(firstKeyPair, newAddress.toAddress.toString, 2 unitoken, 1 unitoken, waitForTx = true)
    sender.broadcastData(newAddress, entries, 0.1 unitoken, waitForTx = true)
