...
import com.decentralchain.protobuf.transaction.{PBSignedTransaction, PBTransactions, Recipient}
...
class InvokeScriptPayAndTransferAssetGrpcSuite extends GrpcBaseTransactionSuite {
  ...
  test("set script to dApp account and transfer out all unitoken") {
    val dAppBalance = sender.unitokenBalance(dAppAddress)
    ...
    val dAppScript = ScriptCompiler
      .compile(
        s"""
         ...
         |  else throw("need payment in unitoken or any Asset")
         |}
        """.stripMargin,
        estimator
      )
  ...
  test("dApp can transfer payed asset if its own balance is 0") {
    val dAppInitBalance     = sender.unitokenBalance(dAppAddress)
    val callerInitBalance   = sender.unitokenBalance(callerAddress)
    val receiverInitBalance = sender.unitokenBalance(receiverAddress)

    val paymentAmount = 10

    invoke("resendPayment", paymentAmount, assetId)

    sender.unitokenBalance(dAppAddress).regular shouldBe dAppInitBalance.regular
    sender.unitokenBalance(callerAddress).regular shouldBe callerInitBalance.regular - smartMinFee
    sender.unitokenBalance(receiverAddress).regular shouldBe receiverInitBalance.regular
  ...
  test("dApp can transfer payed smart asset if its own balance is 0") {
    ...
    sender.unitokenBalance(dAppAddress).regular shouldBe dAppInitBalance.regular
    sender.unitokenBalance(callerAddress).regular shouldBe callerInitBalance.regular - fee
    sender.unitokenBalance(receiverAddress).regular shouldBe receiverInitBalance.regular
  ...
  test("dApp can't transfer payed smart asset if it rejects transfers and its own balance is 0") {
    ...
    sender.unitokenBalance(dAppAddress).regular shouldBe dAppInitBalance.regular
    sender.unitokenBalance(callerAddress).regular shouldBe callerInitBalance.regular
    sender.unitokenBalance(receiverAddress).regular shouldBe receiverInitBalance.regular
  ...
  test("dApp can transfer payed unitoken if its own balance is 0") {
    val dAppInitBalance     = sender.unitokenBalance(dAppAddress)
    val callerInitBalance   = sender.unitokenBalance(callerAddress)
    val receiverInitBalance = sender.unitokenBalance(receiverAddress)
    ...
    sender.unitokenBalance(dAppAddress).regular shouldBe dAppInitBalance.regular + paymentAmount - 1
    sender.unitokenBalance(callerAddress).regular shouldBe callerInitBalance.regular - paymentAmount - smartMinFee
    sender.unitokenBalance(receiverAddress).regular shouldBe receiverInitBalance.regular + 1
  }

  def invoke(func: String, amount: Long, assetId: String = "unitoken", fee: Long = 500000): PBSignedTransaction = {
    val assetIdByteSting = if (assetId == "unitoken") ByteString.EMPTY else ByteString.copyFrom(Base58.decode(assetId))
