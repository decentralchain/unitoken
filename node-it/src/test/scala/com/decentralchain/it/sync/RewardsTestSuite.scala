...
class RewardsTestSuite
  ...
  private lazy val initialAmount    = BigInt(Constants.Totalunitoken) * BigInt(Constants.UnitsInWave)
  ...
  "reward changes accordingly node's votes and miner's balance changes by reward amount after block generation" - {
    "when miner votes for increase" in {
      ...
      rewardAtActivation.totalunitokenAmount shouldBe initialAmount + initial
      ...
      rewardAfterFirstVote.totalunitokenAmount shouldBe initialAmount + BigInt(initial) * BigInt(votingStartHeight - activationHeight + 1)
      ...
      rewardAtTermEnd.totalunitokenAmount shouldBe amountAfterTerm
      ...
      rewardSecVoting.totalunitokenAmount shouldBe amountAfterTerm + newReward * BigInt(secondVotingStartHeightPlusTwo - termEndHeight)
    }
