...
import com.decentralchain.protobuf.transaction.DataTransactionData.DataEntry
import com.decentralchain.protobuf.transaction.{DataTransactionData, PBRecipients, PBTransactions, Recipient}
...
class DataTransactionGrpcSuite extends GrpcBaseTransactionSuite {
  ...
  protected override def beforeAll(): Unit = {
    super.beforeAll()
    sender.broadcastTransfer(firstAcc, Recipient().withPublicKeyHash(fourthAddress), 10.unitoken, minFee, waitForTx = true)
  }

  test("sender's unitoken balance is decreased by fee.") {
    for (v <- dataTxSupportedVersions) {
      val firstBalance = sender.unitokenBalance(firstAddress).available
      val firstEffBalance = sender.unitokenBalance(firstAddress).effective
      ...
      sender.unitokenBalance(firstAddress).available shouldBe firstBalance - fee
      sender.unitokenBalance(firstAddress).effective shouldBe firstEffBalance - fee
    }
  }

  test("cannot put data without having enough unitoken") {
    for (v <- dataTxSupportedVersions) {
      val firstBalance = sender.unitokenBalance(firstAddress).available
      val firstEffBalance = sender.unitokenBalance(firstAddress).effective
      ...
      sender.unitokenBalance(firstAddress).available shouldBe firstBalance
      sender.unitokenBalance(firstAddress).effective shouldBe firstEffBalance
    }
  }
  ...
  test("data definition and retrieval") {
    for (v <- dataTxSupportedVersions) {
      ...
      val firstBalance = sender.unitokenBalance(txSenderAddress).available
      val firstEffBalance = sender.unitokenBalance(txSenderAddress).effective
      ...
      sender.unitokenBalance(txSenderAddress).available shouldBe firstBalance - fee
      sender.unitokenBalance(txSenderAddress).effective shouldBe firstEffBalance - fee
    }
  }
  ...
  test("try to send tx above limits of key, value and size") {
    for (v <- dataTxSupportedVersions) {
      ...
      assertGrpcError(sender.putData(firstAcc, List(DataEntry("", DataEntry.Value.BoolValue(false))), 1.unitoken, version = v),
        "Empty key found", Code.INVALID_ARGUMENT)
      assertGrpcError(
        sender.putData(firstAcc, List(DataEntry("abc", DataEntry.Value.BoolValue(false)), DataEntry("abc", DataEntry.Value.BoolValue(false))), 1.unitoken, version = v),
