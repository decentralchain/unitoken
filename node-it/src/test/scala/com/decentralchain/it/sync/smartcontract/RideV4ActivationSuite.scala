...
class RideV4ActivationSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("declines reduced fee for reissue transaction before activation") {
    ...
    assertApiError(sender.reissue(callerAcc, issuedAssetId, someAssetAmount, reissuable = true, fee = reissueReducedFee)) { error =>
      ...
      error.message should include(s"Fee for ReissueTransaction ($reissueReducedFee in unitoken) does not exceed minimal value of $reissueFee unitoken.")
    }
  }
  ...
  test("can invoke V4 contract from V3 scripted account with 0 or 1 payments") {
    ...
    sender.invokeScript(
      ...
      payment = Seq(Payment(1, unitoken)),
  ...
  test("can't invoke V4 contract from V3 scripted account with 2 payments") {
    ...
    assertApiError(
      sender.invokeScript(
        smartAccV3,
        smartAccV4.toAddress.toString,
        payment = Seq(Payment(1, unitoken), Payment(1, unitoken)),
        fee = smartMinFee + smartFee,
        waitForTx = true
      )._1.id) { error =>
      error.statusCode shouldBe 400
  ...
  test("can't invoke V3 DApp with multiple payments") {
    ...
    assertApiError(
      sender.invokeScript(callerAcc, smartAccV3.toAddress.toString, payment = Seq(Payment(1, unitoken), Payment(1, unitoken)), waitForTx = true)
    ) { error =>
      error.statusCode shouldBe 400
  ...
  test("can't attach unavailable payment even if V4 DApp returns its enough amount") {
    ...
    assertApiError(
      sender.invokeScript(callerAcc, smartAccV4.toAddress.toString, Some("payBack"), payment = Seq(Payment(balance + 1, unitoken)))
    ) { error =>
      error.message should include("Transaction application leads to negative unitoken balance")
      error.id shouldBe StateCheckFailed.Id
      error.statusCode shouldBe 400
    }
  }
  ...
  test("still can't attach unavailable payment if V3 DApp returns its enough amount") {
    ...
    assertApiError(
      sender.invokeScript(callerAcc, smartAccV3.toAddress.toString, Some("payBack"), payment = Seq(Payment(balance, unitoken)), waitForTx = true)
    )(_.statusCode shouldBe 400)
  }
