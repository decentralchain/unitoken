...
import com.decentralchain.api.grpc.TransactionsRequest
...
import com.decentralchain.protobuf.transaction.{PBRecipients, PBSignedTransaction, PBTransactions, Recipient}
...
class FailedTransactionGrpcSuite extends GrpcBaseTransactionSuite with FailedTransactionSuiteLike[PBSignedTransaction] {
  ...
  protected override def beforeAll(): Unit = {
    ...
    sender.broadcastTransfer(sender.keyPair, Recipient().withPublicKeyHash(contractAddr), 100.unitoken, minFee, waitForTx = true)
  ...
  test("InvokeScriptTransaction: sponsored fee on failed transaction should be charged correctly") {
    ...
    val prevBalance = sender.unitokenBalance(contractAddr).regular
    ...
    sendTxsAndThenPriorityTx(
      ...
      sender.unitokenBalance(contractAddr).regular shouldBe prevBalance - invokeFee * txs.size - priorityFee
      assertFailedTxs(txs)
    }
  }
  ...
  test("InvokeScriptTransaction: reject transactions if account script failed") {
    ...
    overflowBlock()
    val prevBalance = sender.unitokenBalance(callerAddr).regular
    sendTxsAndThenPriorityTx(
      _ =>
        sender.broadcastInvokeScript(
          caller,
      ...
      sender.unitokenBalance(callerAddr).regular shouldBe prevBalance - (txs.size - invalid.size) * invokeFee - priorityFee
      invalid
    }
  }
