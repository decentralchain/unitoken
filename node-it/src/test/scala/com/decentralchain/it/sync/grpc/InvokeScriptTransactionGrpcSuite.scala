...
class InvokeScriptTransactionGrpcSuite extends GrpcBaseTransactionSuite {
  ...
  protected override def beforeAll(): Unit = {
    ...
    sender.broadcastTransfer(firstAcc, Recipient().withPublicKeyHash(thirdContractAddr), 10.unitoken, minFee, waitForTx = true)
  ...
  test("verifier works") {
    for (v <- invokeScrTxSupportedVersions) {
      val contract    = if (v < 2) firstContractAddr else secondContractAddr
      val dAppBalance = sender.unitokenBalance(contract)
      assertGrpcError(
        sender.broadcastTransfer(firstAcc, Recipient().withPublicKeyHash(contract), transferAmount, 1.unitoken),
        "Transaction is not allowed by account-script",
        Code.INVALID_ARGUMENT
      )
      sender.unitokenBalance(contract) shouldBe dAppBalance
    }
  }
