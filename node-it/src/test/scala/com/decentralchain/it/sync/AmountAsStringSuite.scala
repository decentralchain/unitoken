...
import com.decentralchain.transaction.{CreateAliasTransaction, TxVersion}
...
class AmountAsStringSuite extends BaseTransactionSuite with OverflowBlock {
  ...
  test("amount as string in exchange transaction") {
    ...
    val buyOrder = Order.buy(
      ...
      exchanger,
      exchanger.publicKey,
      AssetPair.createAssetPair("unitoken", exchAssetId).get,
      amount,
      price,
      ts,
  ...
  test("amount as string in rewards api") {
    ...
    rewards.totalunitokenAmount shouldBe rewardsAsInteger.totalunitokenAmount
    rewards.currentReward shouldBe rewardsAsInteger.currentReward
    rewards.minIncrement shouldBe rewardsAsInteger.minIncrement
    rewardsByHeight.totalunitokenAmount shouldBe rewardsAsInteger.totalunitokenAmount
  ...
  test("amount as string in debug api") {
    ...
    val stateunitokenOnHeight = sender.getWithCustomHeader(
      s"/debug/stateunitoken/${sender.height}",
      headerValue = "application/json;large-significand-format=string",
      withApiKey = true
    )
    (parseResponse(stateunitokenOnHeight) \ firstAddress).get shouldBe JsString(firstBalance.toString)
  }
