...
class AddressApiSuite extends BaseTransactionSuite with NTPTime {
  ...
  test("balances for unitoken should be correct") {
    assertBalances(None)
  }
  ...
  test("limit violation requests should be handled correctly") {
    val limit     = miner.config.getInt("unitoken.rest-api.transactions-by-address-limit")
  ...
  override protected def nodeConfigs: Seq[Config] =
    NodeConfigs.newBuilder
      ...
      .overrideBase(_.raw("unitoken.rest-api.transactions-by-address-limit = 20"))
      .withDefault(1)
