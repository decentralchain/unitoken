...
import com.decentralchain.it.{Node, NodeConfigs, ReportingTestName, TransferSending}
    ...
    def tt(tx: Transaction) =
      TransferTransaction
        .create(
          ...
          unitoken /* not support tx.asset.fold(unitoken)(v => IssuedAsset(Base58.decode(v))) */,
          tx.amount.get,
          unitoken /* not support tx.feeAsset.fold(unitoken)(v => Issued(Base58.decode(v))) */,
          tx.fee,
