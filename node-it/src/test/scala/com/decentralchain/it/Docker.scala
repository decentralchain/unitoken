...
import com.decentralchain.it.api.AsyncHttpApi._
...
class Docker(suiteConfig: Config = empty, tag: String = "", enableProfiling: Boolean = false, imageName: String = Docker.NodeImageName)
  ...
  private val logDir: Coeval[Path] = Coeval.evalOnce {
    val r = Option(System.getProperty("unitoken.it.logging.dir"))
      .map(Paths.get(_))
  ...
  private lazy val unitokenNetwork: Network = {
    val id          = Random.nextInt(Int.MaxValue)
    val networkName = s"unitoken-$id"
  ...
  def createNetwork: Network = unitokenNetwork
  ...
  private def startNodeInternal(nodeConfig: Config, autoConnect: Boolean = true): DockerNode =
    try {
      val nodeName = nodeConfig.getString("unitoken.network.node-name")
      ...
      val networkPort = actualConfig.getString("unitoken.network.port")
      ...
      val configOverrides: String = {
        val ntpServer    = Option(System.getenv("NTP_SERVER")).fold("")(x => s"-Dunitoken.ntp-server=$x ")
        val maxCacheSize = Option(System.getenv("MAX_CACHE_SIZE")).fold("")(x => s"-Dunitoken.max-cache-size=$x ")

        var config = s"$javaOptions ${renderProperties(asProperties(overrides))} " +
          s"-Dlogback.stdout.level=TRACE -Dlogback.file.level=OFF -Dunitoken.network.declared-address=$ip:$networkPort $ntpServer $maxCacheSize"
        ...
        if (enableProfiling) {
          ...
          config += s"-agentpath:/usr/local/YourKit-JavaProfiler-2019.8/bin/linux-x86-64/libyjpagent.so=port=$ProfilerPort,listen=all," +
            s"sampling,monitors,sessionname=unitokenNode,dir=$ContainerRoot/profiler,logdir=$ContainerRoot,onexit=snapshot "
        }
      ...
      val containerConfig = ContainerConfig
        .builder()
        .image(imageName)
        .networkingConfig(ContainerConfig.NetworkingConfig.create(Map(unitokenNetwork.name() -> endpointConfigFor(nodeName)).asJava))
        .hostConfig(hostConfig)
        .env(s"unitoken_OPTS=$configOverrides")
        .build()

      val containerId = {
        val jenkinsJobIdFromEnv = sys.env.get("JENKINS_JOB_ID").fold("")(s => s"-$s")
        val containerName = s"${unitokenNetwork.name()}-$nodeName$jenkinsJobIdFromEnv"
        dumpContainers(
      ...
      val node = new DockerNode(actualConfig, containerId, getNodeInfo(containerId, unitokenSettings.fromRootConfig(actualConfig)))
      nodes.add(node)
  ...
  private def getNodeInfo(containerId: String, settings: unitokenSettings): NodeInfo = {
    ...
    val unitokenIpAddress = containerInfo.networkSettings().networks().get(unitokenNetwork.name()).ipAddress()

    NodeInfo(restApiPort, networkPort, unitokenIpAddress, containerInfo.networkSettings().ports())
  }

  private def inspectContainer(containerId: String): ContainerInfo = {
    val containerInfo = client.inspectContainer(containerId)
    if (containerInfo.networkSettings().networks().asScala.contains(unitokenNetwork.name())) containerInfo
    else {
      log.debug(s"Container $containerId has not connected to the network ${unitokenNetwork.name()} yet, retry")
      Thread.sleep(1000)
  ...
  def restartNode(node: DockerNode, configUpdates: Config = empty): DockerNode = {
    ...
    if (configUpdates != empty) {
      ...
      val shPath = "/opt/unitoken/start-unitoken.sh"
      val scriptCmd: Array[String] =
        Array("sh", "-c", s"sed -i 's|$$unitoken_OPTS.*-cp|$$unitoken_OPTS $renderedConfig -cp|' $shPath && chmod +x $shPath")
  ...
  override def close(): Unit = {
    if (isStopped.compareAndSet(false, true)) {
      ...
      try {
        client.removeNetwork(unitokenNetwork.id)
      } catch {
        case NonFatal(e) =>
          // https://github.com/moby/moby/issues/17217
          log.warn(s"Can not remove network ${unitokenNetwork.name()}", e)
      }
  ...
  private def disconnectFromNetwork(containerId: String): Unit = {
    ...
    client.disconnectFromNetwork(containerId, unitokenNetwork.id())
  }
...
object Docker {
  ...
  private val ContainerRoot = Paths.get("/opt/unitoken")
  ...
  def genesisOverride: Config = {
    ...
    val timestampOverrides = parseString(s"""unitoken.blockchain.custom.genesis {
                                            |  timestamp = $genesisTs
    ...
    val gs               = genesisConfig.as[GenesisSettings]("unitoken.blockchain.custom.genesis")
    val genesisSignature = Block.genesis(gs).explicitGet().id()

    parseString(s"unitoken.blockchain.custom.genesis.signature = $genesisSignature").withFallback(timestampOverrides)
  }

  AddressScheme.current = new AddressScheme {
    override val chainId: Byte = configTemplate.as[String]("unitoken.blockchain.custom.address-scheme-character").charAt(0).toByte
  }
  ...
  case class NodeInfo(restApiPort: Int, networkPort: Int, unitokenIpAddress: String, ports: JMap[String, JList[PortBinding]]) {
    ...
    val containerNetworkAddress: InetSocketAddress = new InetSocketAddress(unitokenIpAddress, networkPort)
