...
object NodeConfigs {
  ...
  object Templates {
    ...
    def quorum(n: Int): String = s"unitoken.miner.quorum = $n"
    def preactivatedFeatures(f: (Int, Int)*): String = {
      s"""
         |unitoken.blockchain.custom.functionality.pre-activated-features {
         ${f.map {case (id, height) => s"|  $id = $height"}.mkString("\n")}
         |}""".stripMargin
    }
    def minAssetInfoUpdateInterval(blocks: Int): String =
      s"unitoken.blockchain.custom.functionality.min-asset-info-update-interval = $blocks"

    val nonMiner: String = "unitoken.miner.enable = no"
  }
