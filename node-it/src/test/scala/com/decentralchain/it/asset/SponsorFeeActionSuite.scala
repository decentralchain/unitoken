...
import com.decentralchain.it.BaseSuite
...
class SponsorFeeActionSuite extends BaseSuite {
  private val initialunitokenBalance = 100.unitoken
  ...
  "State changes" - {
    ...
    "Use sponsored asset as fee" in {
      ...
      val dAppunitokenOutgo = smartMinFee + Sponsorship.tounitoken(assetFee, minSponsoredAssetFee)
      ...
      miner.balanceAtHeight(globalDAppAddress, lastCheckHeight) shouldBe startDAppBalance - dAppunitokenOutgo

      miner.balanceAtHeight(miner.address, lastCheckHeight) shouldBe
        startMinerBalance + dAppunitokenOutgo + blockReward * (lastCheckHeight - firstCheckHeight)
    }
  ...
  "Restrictions" - {
    ...
    "SponsorFee is available for assets issued via transaction" in {
      ...
      miner.transfer(sender.keyPair, dApp.toAddress.toString, initialunitokenBalance, minFee, waitForTx = true)
    ...
    "Negative fee is not available" in {
      val dApp        = miner.createKeyPair()
      val dAppAddress = dApp.toAddress.toString
      miner.transfer(sender.keyPair, dAppAddress, initialunitokenBalance, minFee, waitForTx = true)
      val assetId = miner.issue(dApp, waitForTx = true).id
    ...
    "SponsorFee is available only for assets issuing from current address" in {
      ...
      miner.transfer(sender.keyPair, issuer.toAddress.toString, initialunitokenBalance, minFee, waitForTx = true)
  ...
  private def createDApp(script: String, address: KeyPair = miner.createKeyPair()): KeyPair = {
    ...
    miner.transfer(sender.keyPair, address.toAddress.toString, initialunitokenBalance, minFee, waitForTx = true)
