...
import com.decentralchain.api.http.ApiError.{CustomValidationError, TransactionDoesNotExist}
...
class IssueReissueBurnAssetSuite extends BaseSuite {
  ...
  private val setScriptPrice      = 0.01.unitoken
  ...
  for (method <- Seq(CallableMethod, TransactionMethod)) s"Asset Issue/Reissue/Burn via $method" - {
    ...
    for (data <- Seq(simpleNonreissuableAsset, simpleReissuableAsset, nftAsset, longMaxAsset, longMinAsset))
      s"${data.assetType} asset could be issued in callable" in {
        ...
        sender.balanceDetails(acc.toAddress.toString).regular shouldBe (initialunitokenBalance - setScriptPrice - fee)
      }
  ...
  def createDapp(scriptParts: String*): KeyPair = {
    ...
    miner.transfer(sender.keyPair, address.toAddress.toString, initialunitokenBalance, setScriptFee * 2, waitForTx = true)
  ...
  def invocationCost(aCount: Int, isSmartAcc: Boolean = true, sPCount: Int = 0, sAinActions: Int = 0): Long = {
    0.005.unitoken + (if (isSmartAcc) 0.004.unitoken else 0L) + 0.004.unitoken * sPCount + 0.004.unitoken * sAinActions + 1.unitoken * aCount
  }
