...
class GrpcSponsorFeeActionSuite extends FreeSpec with GrpcBaseTransactionSuiteLike {
  ...
  "State changes" - {
    ...
    "Use sponsored asset as fee" in {
      ...
      miner.balance(dAppAddress).balance shouldBe startDAppBalance - dAppunitokenOutgo
      miner.balance(miner.address).balance shouldBe startMinerBalance + dAppunitokenOutgo + blockReward
    }
  ...
  private def createDApp(script: String, address: KeyPair = miner.createKeyPair()): KeyPair = {
    ...
    miner.transfer(sender.keyPair, address.publicKey.toAddress.toString, initialunitokenBalance, minFee, waitForTx = true)
