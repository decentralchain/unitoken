...
trait GrpcIntegrationSuiteWithThreeAddress
  ...
  abstract protected override def beforeAll(): Unit = {
    ...
    val defaultBalance: Long = 100.unitoken

    def dumpBalances(node: Node, accounts: Seq[ByteString], label: String): Unit = {
      accounts.foreach(acc => {
        val balance = miner.unitokenBalance(acc).available
        val eff     = miner.unitokenBalance(acc).effective
    ...
    def correctStartBalancesFuture(): Unit = {
      ...
      accounts.foreach(acc => miner.unitokenBalance(acc).available shouldBe defaultBalance)
      accounts.foreach(acc => miner.unitokenBalance(acc).effective shouldBe defaultBalance)
    }
