...
import com.decentralchain.database.{DBExt, KeyTags, Keys}
import com.decentralchain.features.BlockchainFeatures
import com.decentralchain.lang.ValidationError
import com.decentralchain.protobuf.transaction.PBRecipients
import com.decentralchain.state.{AccountScriptInfo, AssetDescription, Blockchain, DataEntry, Diff, Height}
...
object CommonAccountsApi extends ScorexLogging {
  ...
  def apply(diff: => Diff, db: DB, blockchain: Blockchain): CommonAccountsApi = new CommonAccountsApi {
    ...
    override def balanceDetails(address: Address): BalanceDetails = {
      val portfolio = blockchain.unitokenPortfolio(address)
      BalanceDetails(
        portfolio.balance,
