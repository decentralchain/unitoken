...
import com.decentralchain.transaction.{Asset, TransactionFactory}
import com.decentralchain.utils.{Time, _}
...
case class AddressApiRoute(
  ...
  def balances: Route = (path("balance") & get & parameters(("height".as[Int].?, "address".as[String].*, "asset".?))) {
    (height, addresses, assetId) =>
      complete(
        balancesJson(height.getOrElse(blockchain.height), addresses.toSeq, assetId.fold(unitoken: Asset)(a => IssuedAsset(ByteStr.decodeBase58(a).get)))
      )
  }
  ...
  def balancesPost: Route = (path("balance") & (post & entity(as[JsObject]))) { request =>
    ...
    complete(balancesJson(height.getOrElse(blockchain.height), addresses, assetId.fold(unitoken: Asset)(a => IssuedAsset(ByteStr.decodeBase58(a).get))))
  }
