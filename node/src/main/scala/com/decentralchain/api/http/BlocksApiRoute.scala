...
import com.decentralchain.api.http.ApiError.{BlockDoesNotExist, TooBigArrayAllocation}
...
object BlocksApiRoute {
  ...
  private def transactionField(blockVersion: Byte, transactions: Seq[(Transaction, Boolean)]): JsObject = Json.obj(
    "fee" -> transactions.map(_._1.assetFee).collect { case (unitoken, feeAmt) => feeAmt }.sum,
