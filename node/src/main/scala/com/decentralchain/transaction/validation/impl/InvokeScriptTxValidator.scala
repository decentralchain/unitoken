...
import com.decentralchain.transaction.TxValidationError.{GenericError, NonPositiveAmount, TooBigArray}
...
object InvokeScriptTxValidator extends TxValidator[InvokeScriptTransaction] {
  override def validate(tx: InvokeScriptTransaction): ValidatedV[InvokeScriptTransaction] = {
    ...
    def checkAmounts(payments: Seq[Payment]): ValidatedNV = {
      ...
      if (invalid.nonEmpty)
        Invalid(NonEmptyList.fromListUnsafe(invalid.toList).map(p => NonPositiveAmount(p.amount, p.assetId.fold("unitoken")(_.toString))))
      else Valid(())
    }
