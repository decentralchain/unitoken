...
case class Order(
    ...
    matcherFee: TxAmount,
    matcherFeeAssetId: Asset = unitoken,
  ...
  def isValid(atTime: Long): Validation = {
    ...
    (matcherFeeAssetId == unitoken || version >= Order.V3) :| "matcherFeeAssetId should be unitoken"
  }
  ...
  override def toString: String = {
    val matcherFeeAssetIdStr = if (version == 3) s" matcherFeeAssetId=${matcherFeeAssetId.fold("unitoken")(_.toString)}," else ""
...
object Order {
  ...
  def selfSigned(
      ...
      expiration: TxTimestamp,
      matcherFee: TxAmount,
      matcherFeeAssetId: Asset = Asset.unitoken
  ): Order =
