...
import com.decentralchain.transaction.{Asset, Proofs, TxVersion}
...
object OrderJson {
  ...
  private val assetReads: Reads[Asset] = {
    case JsNull | JsString("") => JsSuccess(unitoken)
    case JsString(s) =>
  ...
  implicit val assetPairReads: Reads[AssetPair] = {
    val r = (JsPath \ "amountAsset").readWithDefault[Asset](unitoken)(assetReads) and
      (JsPath \ "priceAsset").readWithDefault[Asset](unitoken)(assetReads)
    r(AssetPair(_, _))
  }
