...
import com.decentralchain.serialization._
...
object InvokeScriptTxSerializer {
  def functionCallToJson(fc: Terms.FUNCTION_CALL): JsObject = {
    Json.obj(
      "function" -> JsString(fc.function.asInstanceOf[com.decentralchain.lang.v1.FunctionHeader.User].internalName),
      "args" -> JsArray(
        fc.args.map {
  ...
  def parseBytes(bytes: Array[Byte]): Try[InvokeScriptTransaction] = Try {
    def parsePayment(arr: Array[Byte]): Payment = {
      ...
      val asset             = maybeAssetId.fold[Asset](unitoken)(IssuedAsset)
      Payment(amt, asset)
    }
