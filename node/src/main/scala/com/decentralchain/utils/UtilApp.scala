...
object UtilApp {
  ...
  private[this] lazy val commandParser = {
    ...
    OParser.sequence(
      programName("unitoken util"),
      head("unitoken Util", Version.VersionString),
      OParser.sequence(
  ...
  private[this] object Actions {
    ...
    def doCompile(settings: unitokenSettings)(c: Command, str: Array[Byte]): ActionResult = {
    ...
    def doVerify(c: Command, data: Array[Byte]): ActionResult =
      Either.cond(com.decentralchain.crypto.verify(c.verifyOptions.signature, data, c.verifyOptions.publicKey), data, "Invalid signature")
