...
import com.decentralchain.api.common.{CommonAccountsApi, CommonAssetsApi, CommonTransactionsApi}
import com.decentralchain.api.http.TransactionsApiRoute.applicationStatus
import com.decentralchain.api.http._
import com.decentralchain.common.state.ByteStr
import com.decentralchain.features.BlockchainFeatures
import com.decentralchain.lang.ValidationError
import com.decentralchain.mining.{Miner, MinerDebugInfo}
import com.decentralchain.network.{PeerDatabase, PeerInfo, _}
import com.decentralchain.settings.{RestAPISettings, unitokenSettings}
import com.decentralchain.state.diffs.TransactionDiffer
import com.decentralchain.state.{Blockchain, LeaseBalance, NG, Portfolio, StateHash}
import com.decentralchain.transaction.TxValidationError.{GenericError, InvalidRequestSignature}
...
case class DebugApiRoute(
  ...
  private lazy val unitokenConfig: JsObject = Json.obj("unitoken" -> (fullConfig \ "unitoken").get)

  override val settings: RestAPISettings = ws.restAPISettings
  override lazy val route: Route = pathPrefix("debug") {
    stateChanges ~ balanceHistory ~ stateHash ~ validate ~ withAuth {
      state ~ info ~ stateunitoken ~ rollback ~ rollbackTo ~ blacklist ~ portfolios ~ minerInfo ~ configInfo ~ print
    }
  }
  ...
  private def distribution(height: Int): Route = optionalHeaderValueByType[Accept](()) { accept =>
    extractScheduler { implicit s =>
      complete(
        assetsApi
          .unitokenDistribution(height, None)
          .toListL
          .runToFuture
          .map {
  ...
  def stateunitoken: Route = (path("stateunitoken" / IntNumber) & get) { height =>
    distribution(height)
  }
  ...
  def configInfo: Route = (path("configInfo") & get & parameter("full".as[Boolean])) { full =>
    complete(if (full) fullConfig else unitokenConfig)
  }
