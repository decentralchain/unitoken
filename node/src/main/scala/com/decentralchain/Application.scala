...
import com.decentralchain.consensus.nxt.api.http.NxtConsensusApiRoute
import com.decentralchain.database.{DBExt, Keys, openDB}
import com.decentralchain.events.{BlockchainUpdateTriggersImpl, BlockchainUpdated, UtxEvent}
import com.decentralchain.extensions.{Context, Extension}
import com.decentralchain.features.EstimatorProvider._
import com.decentralchain.features.api.ActivationApiRoute
import com.decentralchain.history.{History, StorageFactory}
import com.decentralchain.http.{DebugApiRoute, NodeApiRoute}
import com.decentralchain.lang.ValidationError
import com.decentralchain.metrics.Metrics
import com.decentralchain.mining.{Miner, MinerDebugInfo, MinerImpl}
import com.decentralchain.network.RxExtensionLoader.RxExtensionLoaderShutdownHook
...
import com.decentralchain.state.{Blockchain, BlockchainUpdaterImpl, Diff, Height}
...
import com.decentralchain.utils.Schedulers._
...
class Application(val actorSystem: ActorSystem, val settings: unitokenSettings, configRoot: ConfigObject, time: NTP) extends ScorexLogging {
  app =>
  ...
  def run(): Unit = {
    ...
    val extensionContext: Context = new Context {
      override def settings: unitokenSettings                                                       = app.settings
    ...
    if (settings.restAPISettings.enable) {
      if (settings.restAPISettings.apiKeyHash == "H6nsiifwYKYEx6YzYD7woP1XCn72RVvx6tC1zjjLXqsu") {
        log.error(
          "Usage of the default api key hash (H6nsiifwYKYEx6YzYD7woP1XCn72RVvx6tC1zjjLXqsu) is prohibited, please change it in the unitoken.conf"
        )
      ...
      def loadBalanceHistory(address: Address): Seq[(Int, Long)] = db.readOnly { rdb =>
        rdb.get(Keys.addressId(address)).fold(Seq.empty[(Int, Long)]) { aid =>
          rdb.get(Keys.unitokenBalanceHistory(aid)).map { h =>
            h -> rdb.get(Keys.unitokenBalance(aid)(h))
          }
        }
      }
...
object Application extends ScorexLogging {
  private[decentralchain] def loadApplicationConfig(external: Option[File] = None): unitokenSettings = {
    ...
    System.setProperty("unitoken.directory", config.getString("unitoken.directory"))
    if (config.hasPath("unitoken.config.directory")) System.setProperty("unitoken.config.directory", config.getString("unitoken.config.directory"))
    ...
    val settings = unitokenSettings.fromRootConfig(config)
  ...
  private[decentralchain] def loadBlockAt(db: DB, blockchainUpdater: BlockchainUpdaterImpl)(height: Int): Option[(BlockMeta, Seq[Transaction])] =
    loadBlockInfoAt(db, blockchainUpdater)(height).map { case (meta, txs) => (meta, txs.map(_._1)) }

  private[decentralchain] def loadBlockInfoAt(db: DB, blockchainUpdater: BlockchainUpdaterImpl)(
      height: Int
  ...
  private[decentralchain] def loadBlockMetaAt(db: DB, blockchainUpdater: BlockchainUpdaterImpl)(height: Int): Option[BlockMeta] =
  ...
  def main(args: Array[String]): Unit = {
    ...
    args.headOption.getOrElse("") match {
      ...
      case "help" | "--help" | "-h" => println("Usage: unitoken <config> | export | import | explore | util")
