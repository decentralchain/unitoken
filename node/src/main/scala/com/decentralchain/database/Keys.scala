...
import com.decentralchain.protobuf.transaction.PBRecipients
...
object Keys {
  ...
  def unitokenBalanceHistory(addressId: AddressId): Key[Seq[Int]] = historyKey(unitokenBalanceHistory, addressId.toByteArray)

  def unitokenBalance(addressId: AddressId)(height: Int): Key[Long] =
    Key(unitokenBalance, hAddr(height, addressId), Option(_).fold(0L)(Longs.fromByteArray), Longs.toByteArray)
  ...
  def unitokenAmount(height: Int): Key[BigInt] = Key(unitokenAmount, h(height), Option(_).fold(BigInt(0))(BigInt(_)), _.toByteArray)
