...
import com.decentralchain.database.protobuf.TransactionMeta
import com.decentralchain.features.BlockchainFeatures
import com.decentralchain.lang.ValidationError
import com.decentralchain.protobuf.transaction.PBTransactions
import com.decentralchain.settings.{BlockchainSettings, Constants, DBSettings, unitokenSettings}
import com.decentralchain.state.reader.LeaseDetails
import com.decentralchain.state.{TxNum, _}
...
import com.decentralchain.utils.{LoggerFacade, ScorexLogging}
...
object LevelDBWriter extends ScorexLogging {
  ...
  def apply(db: DB, spendableBalanceChanged: Observer[(Address, Asset)], settings: unitokenSettings): LevelDBWriter with AutoCloseable = {
    ...
    val _unitokenBalanceFilter = load("unitoken-balances", KeyTags.unitokenBalanceHistory)
    ...
    new LevelDBWriter(db, spendableBalanceChanged, settings.blockchainSettings, settings.dbSettings) with AutoCloseable {
      ...
      override val unitokenBalanceFilter: BloomFilter = _unitokenBalanceFilter.getOrElse(BloomFilter.AlwaysEmpty)
      ...
      override def close(): Unit = {
        ...
        _unitokenBalanceFilter.foreach(_.save(lastHeight))
        _assetBalanceFilter.foreach(_.save(lastHeight))
      }
    }
  }

  def readOnly(db: DB, settings: unitokenSettings): LevelDBWriter = {
    val expectedHeight = loadHeight(db)
    ...
    new LevelDBWriter(db, Observer.stopped, settings.blockchainSettings, settings.dbSettings) {
      ...
      override val unitokenBalanceFilter: BloomFilter = loadFilter("unitoken-balances")
...
abstract class LevelDBWriter private[database] (
  ...
  def unitokenBalanceFilter: BloomFilter
  ...
  protected override def loadBalance(req: (Address, Asset)): Long =
    addressId(req._1).fold(0L) { addressId =>
      req._2 match {
        ...
        case unitoken =>
          val kwbh = Keys.unitokenBalanceHistory(addressId)
          if (unitokenBalanceFilter.mightContain(kwbh.suffix))
            writableDB.readOnly(_.fromHistory(kwbh, Keys.unitokenBalance(addressId))).getOrElse(0L)
          else 0L
      }
    }
  ...
  private[database] def loadLposPortfolio(db: ReadOnlyDB, addressId: AddressId) = Portfolio(
    db.fromHistory(Keys.unitokenBalanceHistory(addressId), Keys.unitokenBalance(addressId)).getOrElse(0L),
  ...
  override def unitokenAmount(height: Int): BigInt = readOnly { db =>
    if (db.has(Keys.unitokenAmount(height))) db.get(Keys.unitokenAmount(height))
    else BigInt(Constants.UnitsInWave * Constants.Totalunitoken)
  }
  ...
  private def appendBalances(
    ...
    for ((addressId, updatedBalances) <- balances) {
      for ((asset, balance) <- updatedBalances) {
        asset match {
          case unitoken =>
            rw.put(Keys.unitokenBalance(addressId)(height), balance)
            val kwbh = Keys.unitokenBalanceHistory(addressId)
            if (unitokenBalanceFilter.mightContain(kwbh.suffix))
              updateHistory(rw, kwbh, balanceThreshold, Keys.unitokenBalance(addressId)).foreach(rw.delete)
            else {
              rw.put(kwbh, Seq(height))
              unitokenBalanceFilter.put(kwbh.suffix)
            }
          case a: IssuedAsset =>
  ...
  override protected def doAppend(
    ...
    readWrite { rw =>
      ...
      reward.foreach { lastReward =>
        ...
        rw.put(Keys.unitokenAmount(height), unitokenAmount(height - 1) + lastReward)
      }
      ...
      val discardedBlocks: Seq[(Block, ByteStr)] = for (currentHeight <- height until targetHeight by -1) yield {
        ...
        val discardedBlock = readWrite { rw =>
          ...
          for ((addressId, address) <- changedAddresses) {
            ...
            balancesToInvalidate += (address -> unitoken)
            rw.delete(Keys.unitokenBalance(addressId)(currentHeight))
            rw.filterHistory(Keys.unitokenBalanceHistory(addressId), currentHeight)
          ...
          rw.delete(Keys.unitokenAmount(currentHeight))
  ...
  override def balanceAtHeight(address: Address, height: Int, assetId: Asset = unitoken): Option[(Int, Long)] = readOnly { db =>
    db.get(Keys.addressId(address)).flatMap { addressId =>
      assetId match {
        case unitoken =>
          closest(db.get(Keys.unitokenBalanceHistory(addressId)), height).map { wh =>
            val b: Long = db.get(Keys.unitokenBalance(addressId)(wh))
            (wh, b)
          }
        case asset @ IssuedAsset(_) =>
  ...
  override def balanceSnapshots(address: Address, from: Int, to: Option[BlockId]): Seq[BalanceSnapshot] = readOnly { db =>
    db.get(Keys.addressId(address)).fold(Seq(BalanceSnapshot(1, 0, 0, 0))) { addressId =>
      ...
      val wbh      = slice(db.get(Keys.unitokenBalanceHistory(addressId)), from, toHeigth)
      val lbh      = slice(db.get(Keys.leaseBalanceHistory(addressId)), from, toHeigth)
      for {
        (wh, lh) <- merge(wbh, lbh)
        wb = balanceAtHeightCache.get((wh, addressId), () => db.get(Keys.unitokenBalance(addressId)(wh)))
  ...
  private def transactionsAtHeight(h: Height): List[(TxNum, Transaction)] = readOnly { db =>
    import com.decentralchain.protobuf.transaction.PBSignedTransaction
