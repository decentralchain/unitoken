...
import com.decentralchain.api.common.AddressPortfolio
import com.decentralchain.common.state.ByteStr
import com.decentralchain.common.utils.{Base58, Base64, EitherExt2}
import com.decentralchain.database.{DBExt, KeyTags, Keys, LevelDBWriter, openDB, readTransactionBytes}
...
import com.decentralchain.state.{Blockchain, Diff, Height, Portfolio, TxNum}
import com.decentralchain.transaction.Asset.IssuedAsset
import com.decentralchain.transaction.{Transaction, TransactionParsers}
...
object Explorer extends ScorexLogging {
  ...
  def main(argsRaw: Array[String]): Unit = {
    if (argsRaw.isEmpty) {
      System.err.println("Usage: unitoken explore <command> [args] [--config|-c <cfg file>]")
      return
    }
    ...
    try {
      ...
      flag match {
        case "WB" =>
          ...
          db.iterateOver(KeyTags.unitokenBalance) { e =>
          ...
          val expectedTotalBalance = Constants.UnitsInWave * Constants.Totalunitoken + actualTotalReward
          val byKeyTotalBalance    = reader.unitokenAmount(blockchainHeight)

          if (actualTotalBalance != expectedTotalBalance || expectedTotalBalance != byKeyTotalBalance)
            log.error(
              s"Something wrong, actual total unitoken balance: $actualTotalBalance," +
                s" expected total unitoken balance: $expectedTotalBalance, total unitoken balance by key: $byKeyTotalBalance"
            )
          else
            log.info(s"Correct total unitoken balance: $actualTotalBalance WAVELETS")
        ...
        case "A" =>
          ...
          val balances = wbh.map { h =>
            val k = Keys.unitokenBalance(addressId)(h)
        ...
        case "AP" =>
          ...
          log.info(s"$address : ${pf.balance} unitoken, ${pf.lease}, ${pf.assets.size} assets")
