...
import com.decentralchain.api.common.{CommonAccountsApi, CommonAssetsApi, CommonBlocksApi, CommonTransactionsApi}
...
object Importer extends ScorexLogging {
  ...
  final case class ImportOptions(
      configFile: File = new File("unitoken-testnet.conf"),
  ...
  def parseOptions(args: Array[String]): ImportOptions = {
    lazy val commandParser = {
      ...
      OParser.sequence(
        programName("unitoken import"),
        head("unitoken Blockchain Importer", Version.VersionString),
        opt[File]('c', "config")
  ...
  def loadSettings(file: File): unitokenSettings = {
  ...
  def initExtensions(
    ...
    if (unitokenSettings.extensions.isEmpty) Seq.empty
    else {
      val extensionContext: Context = {
        new Context {
          override def settings: unitokenSettings = unitokenSettings
      ...
      val extensions = unitokenSettings.extensions.map { extensionClassName =>
  ...
  def main(args: Array[String]): Unit = {
    ...
    def initFileStream(file: String): InputStream = {
      ...
      file match {
        ...
        case _ =>
          System.setProperty("http.agent", s"unitoken-node/${Version.VersionString}")
    ...
    val actorSystem              = ActorSystem("decentralchain-import")
