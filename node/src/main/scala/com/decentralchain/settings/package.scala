...
package object settings {
  ...
  def loadConfig(maybeUserConfig: Option[Config]): Config = {
    ...
    val cmdDefaults =
      Try(external.getConfig("unitoken.defaults"))
    ...
    val networkDefaults = {
      val network = withApp.getString("unitoken.blockchain.type").toLowerCase
      withApp.getConfig(s"unitoken.defaults.$network")
    }

    external
      .withFallback(cmdDefaults)
      .withFallback(networkDefaults.atKey("unitoken"))
      .withFallback(ConfigFactory.parseString(s"unitoken.directory = ${defaultDirectory(withApp)}"))
  ...
  def defaultDirectory(config: Config): String = {
    ...
    def withNetwork(config: Config): String = {
      val bc = config.getString("unitoken.blockchain.type")
      val suffix =
        if (bc == "CUSTOM") {
          val char = config.getString("unitoken.blockchain.custom.address-scheme-character").headOption.getOrElse(0.toChar)
