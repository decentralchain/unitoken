...
object FunctionalitySettings {
  ...
  val configPath = "unitoken.blockchain.custom.functionality"
}
...
object GenesisSettings { // TODO: Move to network-defaults.conf
  val MAINNET = GenesisSettings(
    1460678400000L,
    1465742577614L,
    Constants.UnitsInWave * Constants.Totalunitoken,
    ByteStr.decodeBase58("FSH8eAAzZNqnG8xgTZtz5xuLqXySsXgAjmFEC25hXMbEufiGjqWPnGCZFt6gLiVLJny16ipxRNAkkzjjhqTjBE2").toOption,
    List(
      GenesisTransactionSettings("3PAWwWa6GbwcJaFzwqXQN5KQm7H96Y7SHTQ", Constants.UnitsInWave * Constants.Totalunitoken - 5 * Constants.UnitsInWave),
  ...
  val TESTNET = GenesisSettings(
    ...
    List(
      GenesisTransactionSettings("3My3KZgFQ3CrVHgz6vGRt8687sH4oAA1qp8", (Constants.UnitsInWave * Constants.Totalunitoken * 0.04).toLong),
      GenesisTransactionSettings("3NBVqYXrapgJP9atQccdBPAgJPwHDKkh6A8", (Constants.UnitsInWave * Constants.Totalunitoken * 0.02).toLong),
      GenesisTransactionSettings("3N5GRqzDBhjVXnCn44baHcz2GoZy5qLxtTh", (Constants.UnitsInWave * Constants.Totalunitoken * 0.02).toLong),
      GenesisTransactionSettings("3NCBMxgdghg4tUhEEffSXy11L6hUi6fcBpd", (Constants.UnitsInWave * Constants.Totalunitoken * 0.02).toLong),
      GenesisTransactionSettings(
        "3N18z4B8kyyQ96PhN5eyhCAbg4j49CgwZJx",
        (Constants.UnitsInWave * Constants.Totalunitoken - Constants.UnitsInWave * Constants.Totalunitoken * 0.1).toLong
      )
    ),
    153722867L,
    60.seconds
  ...
  val STAGENET = GenesisSettings(
    ...
    List(
      GenesisTransactionSettings("3Mi63XiwniEj6mTC557pxdRDddtpj7fZMMw", Constants.UnitsInWave * Constants.Totalunitoken)
    ),
    5000,
    1.minute
  )
...
object BlockchainSettings {
  ...
  def fromRootConfig(config: Config): BlockchainSettings = config.as[BlockchainSettings]("unitoken.blockchain")
