...
import com.decentralchain.protobuf.transaction.Transaction.Data
import com.decentralchain.serialization.Deser
import com.decentralchain.state.{BinaryDataEntry, BooleanDataEntry, EmptyDataEntry, IntegerDataEntry, StringDataEntry}
...
import com.decentralchain.transaction.{Proofs, TxValidationError}
import com.decentralchain.utils.StringBytes
import com.decentralchain.{transaction => vt}
...
object PBTransactions {
  ...
  def create(
      ...
      chainId: Byte = 0,
      fee: Long = 0L,
      feeAssetId: VanillaAssetId = unitoken,
      timestamp: Long = 0L,
      version: Int = 0,
      proofsArray: Seq[com.decentralchain.common.state.ByteStr] = Nil,
      data: com.decentralchain.protobuf.transaction.Transaction.Data = com.decentralchain.protobuf.transaction.Transaction.Data.Empty
  ): SignedTransaction =
    new SignedTransaction(
  ...
  def protobuf(tx: VanillaTransaction): PBSignedTransaction = {
    tx match {
      ...
      case vt.PaymentTransaction(sender, recipient, amount, fee, timestamp, signature, chainId) =>
        ...
        PBTransactions.create(sender, chainId, fee, unitoken, timestamp, 1, Seq(signature), Data.Payment(data))
  ...
  def toVanillaDataEntry(de: DataTransactionData.DataEntry): com.decentralchain.state.DataEntry[_] = {
  ...
  def toPBDataEntry(de: com.decentralchain.state.DataEntry[_]): DataTransactionData.DataEntry = {
