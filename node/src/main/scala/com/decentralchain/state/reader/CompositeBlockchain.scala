...
final case class CompositeBlockchain(
  ...
  override def balanceAtHeight(address: Address, h: Int, assetId: Asset = unitoken): Option[(Int, Long)] = {
  ...
  override def unitokenAmount(height: Int): BigInt = inner.unitokenAmount(height)
