...
import com.decentralchain.database.Storage
import com.decentralchain.events.BlockchainUpdateTriggers
import com.decentralchain.features.BlockchainFeatures
import com.decentralchain.lang.ValidationError
import com.decentralchain.metrics.{TxsInBlockchainStats, _}
import com.decentralchain.mining.{Miner, MiningConstraint, MiningConstraints}
import com.decentralchain.settings.{BlockchainSettings, unitokenSettings}
import com.decentralchain.state.diffs.BlockDiffer
import com.decentralchain.state.reader.{CompositeBlockchain, LeaseDetails}
...
class BlockchainUpdaterImpl(
    ...
    unitokenSettings: unitokenSettings,
    time: Time,
  ...
  import com.decentralchain.state.BlockchainUpdaterImpl._
  import unitokenSettings.blockchainSettings.functionalitySettings
  ...
  private lazy val maxBlockReadinessAge = unitokenSettings.minerSettings.intervalAfterLastBlockThenGenerationIsAllowed.toMillis
  ...
  override val settings: BlockchainSettings = unitokenSettings.blockchainSettings
  ...
  private def featuresApprovedWithBlock(block: Block): Set[Short] = {
    ...
    if (height % featuresCheckPeriod == 0) {
      ...
      if (unimplementedActivated.nonEmpty) {
        ...
        if (unitokenSettings.featuresSettings.autoShutdownOnUnsupportedFeature) {
  ...
  override def processBlock(block: Block, hitSource: ByteStr, verify: Boolean = true): Either[ValidationError, Seq[Diff]] =
    writeLock {
      ...
      Either
        .cond(
          !unitokenSettings.featuresSettings.autoShutdownOnUnsupportedFeature || notImplementedFeatures.isEmpty,
          (),
              ...
              _ map {
                case (BlockDiffer.Result(newBlockDiff, carry, totalFee, updatedTotalConstraint, detailedDiff), discDiffs, reward, hitSource) =>
                  ...
                  if ((block.header.timestamp > time
                        .getTimestamp() - unitokenSettings.minerSettings.intervalAfterLastBlockThenGenerationIsAllowed.toMillis) || (newHeight % 100 == 0)) {
  ...
  override def unitokenAmount(height: Int): BigInt = readLock {
    ngState match {
      case Some(ng) if this.height == height =>
        leveldb.unitokenAmount(height - 1) + ng.reward.fold(BigInt(0))(BigInt(_))
      case _ => leveldb.unitokenAmount(height)
    }
  }
  ...
  override def balanceAtHeight(address: Address, h: Int, assetId: Asset = unitoken): Option[(Int, Long)] = readLock {
