...
import com.decentralchain.lang.v1.traits.domain.{Burn, Issue, Reissue, SponsorFee}
import com.decentralchain.protobuf.Amount
import com.decentralchain.protobuf.transaction.{PBAmounts, PBRecipients, PBTransactions, InvokeScriptResult => PBInvokeScriptResult}
...
object InvokeScriptResult {
  ...
  def paymentsFromPortfolio(addr: Address, portfolio: Portfolio): Seq[Payment] = {
    val unitoken  = InvokeScriptResult.Payment(addr, unitoken, portfolio.balance)
    val assets = portfolio.assets.map { case (assetId, amount) => InvokeScriptResult.Payment(addr, assetId, amount) }
    (assets.toVector ++ Some(unitoken)).filter(_.amount != 0)
  }
