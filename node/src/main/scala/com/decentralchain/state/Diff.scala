...
object Sponsorship {
  ...
  def calcunitokenFeeAmount(tx: Transaction, getSponsorship: IssuedAsset => Option[Long]): Long = tx.assetFee match {
    case (asset @ IssuedAsset(_), amountInAsset) =>
      val sponsorship = getSponsorship(asset).getOrElse(0L)
      Sponsorship.tounitoken(amountInAsset, sponsorship)

    case (Asset.unitoken, amountInunitoken) =>
      amountInunitoken
  }
  ...
  def fromunitoken(unitokenFee: Long, sponsorship: Long): Long =
    if (unitokenFee == 0 || sponsorship == 0) 0
    else {
      val assetFee = BigInt(unitokenFee) * sponsorship / FeeValidation.FeeUnit
