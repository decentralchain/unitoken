...
import com.decentralchain.features.InvokeScriptSelfPaymentPolicyProvider._
import com.decentralchain.features.ScriptTransferValidationProvider._
import com.decentralchain.lang._
...
import com.decentralchain.lang.v1.traits.domain._
...
object InvokeDiffsCommon {
  def calcFee(
    ...
    tx.assetFee._1 match {
      case unitoken => Right((tx.fee, Map(tx.sender.toAddress -> Portfolio(-tx.fee, LeaseBalance.empty, Map.empty))))
      case asset @ IssuedAsset(_) =>
        for {
          ...
          unitokenFee <- Either.cond(
            ...
            Sponsorship.tounitoken(tx.fee, assetInfo.sponsorship),
            GenericError(s"Asset $asset is not sponsored, cannot be used to pay fees")
          )
        } yield unitokenFee ->
          (Map(tx.sender.toAddress         -> Portfolio(0, LeaseBalance.empty, Map(asset         -> -tx.fee))) |+|
            Map(assetInfo.issuer.toAddress -> Portfolio(-unitokenFee, LeaseBalance.empty, Map(asset -> tx.fee))))
    }
  }
  ...
  def processActions(
    ...
    for {
      ...
      scriptsInvoked <- TracedResult {
        ...
        val minunitoken          = totalScriptsInvoked * ScriptExtraFee + dAppInvocationFee + minIssueFee
        val txName            = Constants.TransactionNames(InvokeScriptTransaction.typeId)
        val assetName         = tx.assetFee._1.fold("unitoken")(_.id.toString)
        Either.cond(
          minunitoken <= feeInfo._1,
          totalScriptsInvoked,
          FailedTransactionError.feeForActions(
            s"Fee in $assetName for $txName (${tx.assetFee._2} in $assetName)" +
              s" with $totalScriptsInvoked total scripts invoked$stepsInfo does not exceed minimal value of $minunitoken unitoken.",
            invocationComplexity
          )
        )
      }
