  ...
  def apply(blockchain: Blockchain, blockTime: Long)(tx: TransferTransaction): Either[ValidationError, Diff] = {
    ...
    for {
      ...
      portfolios = (tx.assetId match {
        ...
        tx.feeAssetId match {
          case unitoken => Map(sender -> Portfolio(-tx.fee, LeaseBalance.empty, Map.empty))
          case asset @ IssuedAsset(_) =>
              ...
              val sponsorPf = blockchain
                ...
                .collect {
                  case desc if desc.sponsorship > 0 =>
                    val feeInunitoken = Sponsorship.tounitoken(tx.fee, desc.sponsorship)
                    Map(desc.issuer.toAddress -> Portfolio(-feeInunitoken, LeaseBalance.empty, Map(asset -> tx.fee)))
                }
                .getOrElse(Map.empty)
