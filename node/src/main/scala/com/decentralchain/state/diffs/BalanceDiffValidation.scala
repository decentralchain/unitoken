...
import com.decentralchain.state.{Blockchain, Diff, Portfolio}
...
object BalanceDiffValidation extends ScorexLogging {
  ...
  def apply(b: Blockchain)(d: Diff): Either[AccountBalanceError, Diff] = {
    ...
    def check(acc: Address): Option[(Address, String)] = {
      ...
      lazy val oldunitoken = b.balance(acc, unitoken)
      lazy val oldLease = b.leaseBalance(acc)
      lazy val lease    = cats.Monoid.combine(oldLease, portfolioDiff.lease)
      (if (balance < 0) {
         val newB = oldunitoken + balance

         if (newB < 0) {
           Some(acc -> s"negative unitoken balance: $acc, old: $oldunitoken, new: $newB")
           ...
           Some(acc -> (if (newB + lease.in - lease.out < 0) {
                          s"negative effective balance: $acc, old: ${(oldunitoken, oldLease)}, new: ${(newB, lease)}"
                        } else if (portfolioDiff.lease.out == 0) {
                          s"$acc trying to spend leased money"
                        } else {
                          s"leased being more than own: $acc, old: ${(oldunitoken, oldLease)}, new: ${(newB, lease)}"
                        }))
         } else {
           None
         }
