...
import com.decentralchain.features.OverdraftValidationProvider._
    ...
    if (blockTime >= blockchain.settings.functionalitySettings.allowTemporaryNegativeUntil) {
      def checkTransfer(
        ...
        val amountDiff = assetId match {
          ...
          case unitoken                => Portfolio(-amount, LeaseBalance.empty, Map.empty)
        }
        val feeDiff = feeAssetId match {
          case aid @ IssuedAsset(_) => Portfolio(0, LeaseBalance.empty, Map(aid -> -feeAmount))
          case unitoken                => Portfolio(-feeAmount, LeaseBalance.empty, Map.empty)
        }

        val spendings       = Monoid.combine(amountDiff, feeDiff)
        val oldunitokenBalance = blockchain.balance(sender, unitoken)

        val newunitokenBalance     = oldunitokenBalance + spendings.balance
        val feeUncheckedBalance = oldunitokenBalance + amountDiff.balance

        val overdraftFilter = allowFeeOverdraft && feeUncheckedBalance >= 0
        if (!overdraftFilter && newunitokenBalance < 0) {
          Left(
            GenericError(
              "Attempt to transfer unavailable funds: Transaction application leads to " +
                s"negative unitoken balance to (at least) temporary negative state, current balance equals $oldunitokenBalance, " +
                s"spends equals ${spendings.balance}, result is $newunitokenBalance"
            )
          )
        } else {
      ...
      tx match {
        case ptx: PaymentTransaction if blockchain.balance(ptx.sender.toAddress, unitoken) < (ptx.amount + ptx.fee) =>
          Left(
            GenericError(
              "Attempt to pay unavailable funds: balance " +
                s"${blockchain.balance(ptx.sender.toAddress, unitoken)} is less than ${ptx.amount + ptx.fee}"
            )
          )
        case ttx: TransferTransaction     => checkTransfer(ttx.sender.toAddress, ttx.assetId, ttx.amount, ttx.feeAssetId, ttx.fee)
        case mtx: MassTransferTransaction => checkTransfer(mtx.sender.toAddress, mtx.assetId, mtx.transfers.map(_.amount).sum, unitoken, mtx.fee)
