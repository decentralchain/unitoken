...
import com.decentralchain.state.patch._
...
import com.decentralchain.transaction.TxValidationError.{ActivationError, _}
...
object BlockDiffer extends ScorexLogging {
  ...
  def fromBlockTraced(
    ...
    val minerReward = blockchain.lastBlockReward.fold(Portfolio.empty)(Portfolio.unitoken)
  ...
  private def maybeApplySponsorship(blockchain: Blockchain, sponsorshipEnabled: Boolean, transactionFee: (Asset, Long)): (Asset, Long) =
    transactionFee match {
      case (ia: IssuedAsset, fee) if sponsorshipEnabled =>
        unitoken -> Sponsorship.tounitoken(fee, blockchain.assetDescription(ia).get.sponsorship)
        ...
        case (TracedResult(Right(Result(currDiff, carryFee, currTotalFee, currConstraint, DetailedDiff(parentDiff, txDiffs))), _), tx) =>
          ...
          txDiffer(currBlockchain, tx).flatMap { thisTxDiff =>
            ...
            if (updatedConstraint.isOverfilled)
              ...
              // carry is 60% of unitoken fees the next miner will get. obviously carry fee only makes sense when both
              // NG and sponsorship is active. also if sponsorship is active, feeAsset can only be unitoken
              val carry = if (hasNg && hasSponsorship) feeAmount - currentBlockFee else 0

              val totalunitokenFee = currTotalFee + (if (feeAsset == unitoken) feeAmount else 0L)
