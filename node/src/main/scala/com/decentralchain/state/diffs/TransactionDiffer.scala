...
import com.decentralchain.metrics.TxProcessingStats.TxTimerExt
...
import com.decentralchain.state.{Blockchain, Diff, InvokeScriptResult, LeaseBalance, NewTransactionInfo, Portfolio, Sponsorship}
  ...
  private def feePortfolios(blockchain: Blockchain, tx: Transaction): Either[ValidationError, Map[Address, Portfolio]] =
    tx match {
      ...
      case ptx: ProvenTransaction =>
        ptx.assetFee match {
          case (unitoken, fee) => Map(ptx.sender.toAddress -> Portfolio(-fee, LeaseBalance.empty, Map.empty)).asRight
          case (asset @ IssuedAsset(_), fee) =>
            for {
              ...
              unitokenFee <- Either.cond(
                ...
                Sponsorship.tounitoken(fee, assetInfo.sponsorship),
                GenericError(s"Asset $asset is not sponsored, cannot be used to pay fees")
              )
            } yield Monoid.combine(
              Map(ptx.sender.toAddress       -> Portfolio(0, LeaseBalance.empty, Map(asset         -> -fee))),
              Map(assetInfo.issuer.toAddress -> Portfolio(-unitokenFee, LeaseBalance.empty, Map(asset -> fee)))
            )
        }
