...
import com.decentralchain.features.FunctionCallPolicyProvider._
...
import com.decentralchain.lang.v1.evaluator.ctx.{EvaluationContext, LazyVal}
...
import com.decentralchain.metrics._
...
import com.decentralchain.transaction.smart.{DApp => DAppTarget, _}
...
object InvokeScriptTransactionDiff {
            ...
            Either.cond(
              ...
              GenericError(
                s"Fee in $assetName for $txName (${tx.assetFee._2} in $assetName)" +
                  s" does not exceed minimal value of $minFee unitoken."
              )
            )
          }
          ...
          result <- for {
            scriptResult <- {
              val scriptResultE = stats.invokedScriptExecution.measureForType(InvokeScriptTransaction.typeId)({
                ...
                val environment = new unitokenEnvironment(
  ...
  private def evaluateV2(
      ...
      environment: unitokenEnvironment,
      limit: Int
  ): Either[ScriptExecutionError, (ScriptResult, EvaluationContext[Environment, Id], Log[Id])] = {
    val unitokenContext = unitokenContext.build(directives)
    val ctx =
      PureContext.build(version).withEnvironment[Environment] |+|
        CryptoContext.build(Global, version).withEnvironment[Environment] |+|
        unitokenContext.copy(vars = Map())

    val freezingLets  = unitokenContext.evaluationContext(environment).letDefs
