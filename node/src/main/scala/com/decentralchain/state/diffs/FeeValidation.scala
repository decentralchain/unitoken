...
object FeeValidation {
  ...
  case class FeeDetails(asset: Asset, requirements: Chain[String], minFeeInAsset: Long, minFeeInunitoken: Long)
  ...
  private def notEnoughFeeError(txType: Byte, feeDetails: FeeDetails, feeAmount: Long): ValidationError = {
    ...
    val actualFee   = s"$feeAmount in ${feeDetails.asset.fold("unitoken")(_.id.toString)}"
    val requiredFee = s"${feeDetails.minFeeInunitoken} unitoken${feeDetails.asset.fold("")(id => s" or ${feeDetails.minFeeInAsset} ${id.id.toString}")}"
  ...
  private case class FeeInfo(assetInfo: Option[(IssuedAsset, AssetDescription)], requirements: Chain[String], unitokenFee: Long)
  ...
  private def feeAfterSponsorship(txAsset: Asset, blockchain: Blockchain, tx: Transaction): Either[ValidationError, FeeInfo] = {
    if (blockchain.height < Sponsorship.sponsoredFeesSwitchHeight(blockchain)) {
      ...
      for {
        ...
        r <- txAsset match {
          case unitoken => Right(FeeInfo(None, Chain.empty, feeInUnits * FeeUnit))
  ...
  def getMinFee(blockchain: Blockchain, tx: Transaction): Either[ValidationError, FeeDetails] = {
    feeAfterSponsorship(tx.assetFee._1, blockchain, tx)
      ...
      .map {
        case FeeInfo(Some((assetId, assetInfo)), reqs, amountInunitoken) =>
          FeeDetails(assetId, reqs, Sponsorship.fromunitoken(amountInunitoken, assetInfo.sponsorship), amountInunitoken)
        case FeeInfo(None, reqs, amountInunitoken) =>
          FeeDetails(unitoken, reqs, amountInunitoken, amountInunitoken)
      }
  }
}
