...
object ExchangeTransactionDiff {
  ...
  private[diffs] def getSpendAmount(
    ...
    Try {
      if (order.orderType == OrderType.SELL) matchAmount
        ...
        if (order.getSpendAssetId == unitoken && !(spend + order.matcherFee).isValidLong) {
  ...
  /**
    * Calculates fee portfolio from the order (taking into account that in OrderV3 fee can be paid in asset != unitoken)
    */
