...
import com.decentralchain.block.Block.{BlockId, GenesisBlockVersion, NgBlockVersion, PlainBlockVersion, ProtoBlockVersion, RewardBlockVersion}
...
trait Blockchain {
  ...
  def unitokenAmount(height: Int): BigInt
  ...
  def balanceAtHeight(address: Address, height: Int, assetId: Asset = unitoken): Option[(Int, Long)]
  ...
  def balance(address: Address, mayBeAssetId: Asset = unitoken): Long
}
...
object Blockchain {
  implicit class BlockchainExt(private val blockchain: Blockchain) extends AnyVal {
    ...
    def unitokenPortfolio(address: Address): Portfolio = Portfolio(
