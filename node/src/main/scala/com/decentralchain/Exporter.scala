...
import com.decentralchain.database.{DBExt, openDB}
...
object Exporter extends ScorexLogging {
  ...
  private[this] final case class ExporterOptions(
      configFileName: File = new File("unitoken-testnet.conf"),
  ...
  private[this] lazy val commandParser = {
    ...
    OParser.sequence(
      programName("unitoken export"),
      head("unitoken Blockchain Exporter", Version.VersionString),
      opt[File]('c', "config")
