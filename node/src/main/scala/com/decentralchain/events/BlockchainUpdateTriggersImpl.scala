...
import com.decentralchain.state.DiffToStateApplier.PortfolioUpdates
import com.decentralchain.state.diffs.BlockDiffer.DetailedDiff
import com.decentralchain.state.reader.CompositeBlockchain
import com.decentralchain.state.{AccountDataInfo, AssetDescription, Blockchain, Diff, DiffToStateApplier}
import com.decentralchain.transaction.{Asset, GenesisTransaction, Transaction}
...
class BlockchainUpdateTriggersImpl(private val events: Observer[BlockchainUpdated]) extends BlockchainUpdateTriggers {
  ...
  override def onProcessBlock(block: Block, diff: DetailedDiff, minerReward: Option[Long], blockchainBefore: Blockchain): Unit = {
    ...
    // updatedunitokenAmount can change as a result of either genesis transactions or miner rewards
    val updatedunitokenAmount = blockchainBefore.height match {
      // genesis case
      case 0 => block.transactionData.collect { case GenesisTransaction(_, amount, _, _, _) => amount }.sum
      // miner reward case
      case _ => blockchainBefore.unitokenAmount(blockchainBefore.height).toLong + minerReward.getOrElse(0L)
    }

    events.onNext(BlockAppended(block.signature, blockchainBefore.height + 1, block, updatedunitokenAmount, blockStateUpdate, txsStateUpdates))
  }
