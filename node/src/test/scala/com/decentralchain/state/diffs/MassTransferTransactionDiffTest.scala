...
class MassTransferTransactionDiffTest extends PropSpec with PropertyChecks with WithState with TransactionGen with NoShrink {
  ...
  property("MassTransfer preserves balance invariant") {
    ...
    import com.decentralchain.transaction.transfer.MassTransferTransaction.{MaxTransferCount => Max}
  ...
  property("MassTransfer fails on non-existent alias") {
    val setup = for {
      ...
      recipient         <- aliasGen
      amount            <- Gen.choose(100000L, 1000000000L)
      transfer          <- massTransferGeneratorP(master, List(ParsedTransfer(recipient, amount)), unitoken)
  ...
  property("validation fails prior to feature activation") {
    val setup = for {
      (genesis, master) <- baseSetup
      transfer          <- massTransferGeneratorP(master, List(), unitoken)
    } yield (genesis, transfer)
