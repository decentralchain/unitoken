...
class CommonValidationTimeTest extends PropSpec with PropertyChecks with Matchers with TransactionGen with NoShrink with WithState {
  ...
  property("disallows too old transacions") {
    forAll(for {
      ...
      fee         <- smallFeeGen
      transfer1 = createunitokenTransfer(master, recipient.toAddress, amount, fee, prevBlockTs - Enabled.maxTransactionTimeBackOffset.toMillis - 1)
        .explicitGet()
  ...
  property("disallows transactions from far future") {
    forAll(for {
      ...
      amount      <- positiveLongGen
      fee         <- smallFeeGen
      transfer1 = createunitokenTransfer(master, recipient.toAddress, amount, fee, blockTs + Enabled.maxTransactionTimeForwardOffset.toMillis + 1)
        .explicitGet()
    } yield (prevBlockTs, blockTs, height, transfer1)) {
      case (prevBlockTs, blockTs, height, transfer1) =>
