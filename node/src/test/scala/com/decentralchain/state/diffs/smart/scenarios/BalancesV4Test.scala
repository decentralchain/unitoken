...
import com.decentralchain.lang.directives.values.{Asset => AssetType, DApp => DAppType, _}
...
import com.decentralchain.lang.v1.compiler.{ContractCompiler, ExpressionCompiler}
...
import com.decentralchain.lang.v1.parser._
...
import com.decentralchain.transaction.smart._
...
class BalancesV4Test extends PropSpec with PropertyChecks with WithState with TransactionGen with NoShrink {
  ...
  private val preconditionsAndTransfer = for {
    ...
    ci        = InvokeScriptTransaction.selfSigned(1.toByte, master, dapp.toAddress, functionCall, Nil, InvokeScriptTxFee, unitoken, ts + 3).explicitGet()
    ...
    t = TransferTransaction
      .selfSigned(TxVersion.V2, dapp, acc1.toAddress, unitoken, 1 * Constants.UnitsInWave + MinFee, unitoken, InvokeScriptTxFee, ByteStr.empty, ts + 5)
      .explicitGet()
  } yield {
  ...
  def script(a: String): Script = {
    ...
    val script =
      s"""
         ...
         | @Callable(i)
         | func bar() = {
         |  let balance = unitokenBalance(Alias("$a"))
         |   [
  ...
  property("unitoken balance details") {
    ...
    forAll(preconditionsAndTransfer) {
      case (genesis, b, acc1, dapp, ci) =>
        assertDiffAndState(
          ...
          case (d, s) =>
            val apiBalance = com.decentralchain.api.common.CommonAccountsApi(d, db, s).balanceDetails(acc1.toAddress)
    ...
    forAll(for { a <- accountGen; b <- accountGen } yield (a, b)) {
      case (acc1, acc2) =>
        ...
        val setScript = SetScriptTransaction
          ...
          .explicitGet()
        val ci = InvokeScriptTransaction.selfSigned(1.toByte, acc1, acc1.toAddress, functionCall, Nil, InvokeScriptTxFee, unitoken, nextTs).explicitGet()
  ...
  property("unitoken balance change while processing script result") {
    ...
    def assetScript(acc: ByteStr): Script = {
      ...
      val script =
        s"""
          ...
          | {-# SCRIPT_TYPE ASSET #-}
          |
          | unitokenBalance(Address(base58'$acc')).regular == $w
        """.stripMargin
      val parsedScript = Parser.parseExpr(script).get.value
      ExprScript(V4, ExpressionCompiler(ctx.compilerContext, parsedScript).explicitGet()._1)
        .explicitGet()
    ...
    forAll(for { a <- accountGen; b <- accountGen } yield (a, b)) {
      case (acc1, acc2) =>
        ...
        val setScript = SetScriptTransaction
          .selfSigned(1.toByte, acc1, Some(dappScript(ByteStr(acc2.toAddress.bytes), issue.id())), SetScriptFee, nextTs)
          .explicitGet()
        val ci = InvokeScriptTransaction.selfSigned(1.toByte, acc2, acc1.toAddress, functionCall, Nil, InvokeScriptTxFee, unitoken, nextTs).explicitGet()
        ...
        assertDiffAndState(Seq(TestBlock.create(Seq(g1, g2, issue, setScript))), TestBlock.create(Seq(ci)), rideV4Activated) {
          case (d, s) =>
            ...
            error.get.code shouldBe 3
            error.get.text should include("Transaction is not allowed by script of the asset")
            s.unitokenPortfolio(acc1.toAddress).balance shouldBe w
        }
