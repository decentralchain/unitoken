...
import com.decentralchain.common.state.diffs.ProduceError._
...
import com.decentralchain.transaction.smart.{unitokenEnvironment, buildThisValue}
import com.decentralchain.transaction.{Proofs, ProvenTransaction, VersionedTransaction}
import com.decentralchain.utils.EmptyBlockchain
import com.decentralchain.{NoShrink, TransactionGen, crypto}
...
class TransactionBindingsTest
    ...
    forAll(invokeScriptGen(paymentOptionGen)) { t =>
      ...
      val script =
        s"""
           ...
           |     then if (${t.payments.headOption.exists(_.assetId != unitoken)})
  ...
  property("MassTransferTransaction binding") {
    forAll(massTransferGen) { t =>
      ...
      val script = s"""
                      ...
                      |    let assetId = if (${t.assetId != unitoken}) then extract(t.assetId) == base58'${t.assetId.maybeBase58Repr
                        .getOrElse("")}'
                      |      else isDefined(t.assetId) == false
    ...
    forAll(exchangeTransactionGen) { t =>
      def pg(ord: Order) = {
        ...
        val script = s"""
                        ...
                        |   let ${oType}AssetPairAmount = if (${ord.assetPair.amountAsset != unitoken}) then extract(t.${oType}Order.assetPair.amountAsset) == base58'${ord.assetPair.amountAsset.maybeBase58Repr
                          .getOrElse("")}'
                        |   else isDefined(t.${oType}Order.assetPair.amountAsset) == false
                        |   let ${oType}AssetPairPrice = if (${ord.assetPair.priceAsset != unitoken}) then extract(t.${oType}Order.assetPair.priceAsset) == base58'${ord.assetPair.priceAsset.maybeBase58Repr
                          .getOrElse("")}'
                        |   else isDefined(t.${oType}Order.assetPair.priceAsset) == false
                        |   let ${oType}MatcherFeeAssetId = if (${ord.matcherFeeAssetId != unitoken}) then extract(t.${oType}Order.matcherFeeAssetId) == base58'${ord.matcherFeeAssetId.maybeBase58Repr
                          .getOrElse("")}'
  ...
  property("Order binding") {
    forAll(orderGen) { t =>
      val s = s"""
                 ...
                 |   ${Range(0, 8).map(letProof(t.proofs, "t")).mkString("\n")}
                 |   let assetPairAmount = if (${t.assetPair.amountAsset != unitoken}) then extract(t.assetPair.amountAsset) == base58'${t.assetPair.amountAsset.maybeBase58Repr
                   .getOrElse("")}'
                 |   else isDefined(t.assetPair.amountAsset) == false
                 |   let assetPairPrice = if (${t.assetPair.priceAsset != unitoken}) then extract(t.assetPair.priceAsset) == base58'${t.assetPair.priceAsset.maybeBase58Repr
                   .getOrElse("")}'
                 |   else isDefined(t.assetPair.priceAsset) == false
                 |   let matcherFeeAssetId = if (${t.matcherFeeAssetId != unitoken}) then extract(t.matcherFeeAssetId) == base58'${t.matcherFeeAssetId.maybeBase58Repr
                   .getOrElse("")}'
  ...
  def runForAsset(script: String): Either[String, EVALUATED] = {
    ...
    import com.decentralchain.lang.v1.CTX._
    ...
    val ctx =
      PureContext.build(V2).withEnvironment[Environment] |+|
        ...
        unitokenContext.build(DirectiveSet(V2, Asset, Expression).explicitGet())

    val environment = new unitokenEnvironment(chainId, Coeval(???), null, EmptyBlockchain, Coproduct[Environment.Tthis](Environment.AssetId(Array())), directives, ByteStr.empty)
    for {
  ...
  def runWithSmartTradingActivated(script: String, t: In = null, chainId: Byte = chainId): Either[String, EVALUATED] = {
    ...
    val env = new unitokenEnvironment(
      chainId,
