...
import com.decentralchain.transaction.{CreateAliasTransaction, DataTransaction, GenesisTransaction, PaymentTransaction, Proofs, Transaction, TxVersion}
...
class CommonValidationTest extends PropSpec with PropertyChecks with Matchers with TransactionGen with WithState with NoShrink {
  ...
  property("disallows double spending") {
    val preconditionsAndPayment: Gen[(GenesisTransaction, TransferTransaction)] = for {
      ...
      transfer: TransferTransaction <- unitokenTransferGeneratorP(master, recipient.toAddress)
  ...
  private def sponsorAndSetScriptGen(sponsorship: Boolean, smartToken: Boolean, smartAccount: Boolean, feeInAssets: Boolean, feeAmount: Long) =
    for {
      ...
      val transferunitokenTx = TransferTransaction.selfSigned(1.toByte, richAcc, recipientAcc.toAddress, unitoken, 10 * Constants.UnitsInWave, unitoken, 1 * Constants.UnitsInWave, ByteStr.empty, ts)
        .explicitGet()
      ...
      val transferBackTx = TransferTransaction.selfSigned(
          ...
          IssuedAsset(issueTx.id()),
          1,
          if (feeInAssets) IssuedAsset(issueTx.id()) else unitoken,
          feeAmount, ByteStr.empty,
          ts
        )
        .explicitGet()

      (TestBlock.create(Vector[Transaction](genesisTx, issueTx, transferunitokenTx, transferAssetTx) ++ sponsorTx ++ setScriptTx), transferBackTx)
    }
  ...
  property("disallows other network") {
    val preconditionsAndPayment: Gen[(GenesisTransaction, Transaction)] = for {
      ...
      tx <- Gen.oneOf(
        ...
        InvokeScriptTransaction(TxVersion.V2, master.publicKey, invChainAddrOrAlias, None, Nil, amount, unitoken, timestamp, Proofs.empty, invChainId)
          .signWith(master.privateKey),
        exchangeV1GeneratorP(master, recipient, asset, unitoken, None, invChainId).sample.get,
        IssueTransaction(
          TxVersion.V2,
          master.publicKey,
        ...
        UpdateAssetInfoTransaction(TxVersion.V2, master.publicKey, asset, "1", "2", timestamp, amount, unitoken, Proofs.empty, invChainId).signWith(master.privateKey),
