...
package object predef {
  ...
  def scriptWithAllV1Functions(tx: DataTransaction, t: TransferTransaction): String =
    s"""${dropLastLine(scriptWithV1PureFunctions(tx, t))}
       |${dropLastLine(scriptWithV1unitokenFunctions(tx, t))}
       |${dropLastLine(scriptWithCryptoFunctions)}
       |if rnd then pure && unitoken else crypto""".stripMargin
  ...
  def scriptWithV1unitokenFunctions(tx: DataTransaction, t: TransferTransaction): String =
    s""" # unitoken context
       | let txById = match tx {
       |     case _: DataTransaction => true
       ...
       | }
       |
       | let balances = assetBalance(tx.sender, unit) > 0 && unitokenBalance(tx.sender) != 0
       |
       | let unitoken = txById && entries && balances && aFromPK && aFromStrOrRecip && height > 0
       | unitoken""".stripMargin
