...
import com.decentralchain.settings.{FunctionalitySettings, TestFunctionalitySettings => TFS}
  ...
  "BlockDiffer DetailedDiff" - {
    ...
    "genesis and transfers" - {
      ...
      val genesisTransfersBlockGen: Gen[(Address, Address, Long, Long, Block)] = for {
        ...
        transfer1 = createunitokenTransfer(a1, a2.toAddress, amount1, transactionFee, ts + 10).explicitGet()
        transfer2 = createunitokenTransfer(a2, a1.toAddress, amount2, transactionFee, ts + 20).explicitGet()
        block = TestBlock
          .create(
            a1,
            Seq(
