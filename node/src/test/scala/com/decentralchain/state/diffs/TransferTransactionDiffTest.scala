...
import com.decentralchain.transaction.{Asset, GenesisTransaction, TxValidationError}
...
class TransferTransactionDiffTest extends PropSpec with PropertyChecks with WithState with TransactionGen with NoShrink {
  ...
  property("transfers assets to recipient preserving unitoken invariant") {
  ...
  property("handle transactions with amount + fee > Long.MaxValue") {
    val precs = for {
      ...
      transfer = TransferTransaction.selfSigned(1.toByte, master, recepient.toAddress, asset, Long.MaxValue, unitoken, 100000, ByteStr.empty,  ts)
        .explicitGet()
