...
class ReissueTransactionDiffTest
  ...
  property("Reissue transaction's fee before feature activation is 1 unitoken") {
  ...
  property("Reissue transaction's fee after feature activation is 0.001 unitoken") {
  ...
  private def genesis: Gen[(KeyPair, Block)] =
    for {
      ...
      block = TestBlock.create(
        ntpNow,
        Seq(GenesisTransaction.create(issuer.toAddress, Constants.Totalunitoken * Constants.UnitsInWave, ntpNow).explicitGet())
      )
    } yield (issuer, block)
