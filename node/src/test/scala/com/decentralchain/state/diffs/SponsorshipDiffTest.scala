...
import com.decentralchain.TransactionGen
...
import com.decentralchain.lagonaki.mocks.TestBlock.{create => block}
...
class SponsorshipDiffTest extends PropSpec with PropertyChecks with WithState with TransactionGen {
  ...
  property("not enough fee") {
    ...
    val setup = for {
      ...
      assetId = issueTx.id()
      assetOverspend = TransferTransaction.selfSigned(1.toByte, master, recipient.toAddress, unitoken, 1000000, IssuedAsset(assetId), issueTx.quantity + 1, ByteStr.empty,  ts + 1)
        .explicitGet()
      ...
      unitokenOverspend = TransferTransaction.selfSigned(1.toByte, master, recipient.toAddress, unitoken, 1000000, IssuedAsset(assetId), fee, ByteStr.empty,  ts + 3)
        .explicitGet()
    } yield (genesis, issueTx, sponsorTx, assetOverspend, insufficientFee, unitokenOverspend)

    forAll(setup) {
      case (genesis, issue, sponsor, assetOverspend, insufficientFee, unitokenOverspend) =>
        ...
        assertDiffEi(setupBlocks, block(Seq(insufficientFee)), s) { blockDiffEi =>
          ...
          val expectedError =
            s"Fee for TransferTransaction (${insufficientFee.fee} in ${issue.assetId.toString})" ++
              s" does not exceed minimal value of 100000 unitoken or $minFee ${issue.assetId.toString}"

          blockDiffEi should produce(expectedError)
        }
        assertDiffEi(setupBlocks, block(Seq(unitokenOverspend)), s) { blockDiffEi =>
          if (unitokenOverspend.fee > issue.quantity)
            blockDiffEi should produce("unavailable funds")
          else
            blockDiffEi should produce("negative unitoken balance")
        }
    }
  }

  property("not enough unitoken to pay fee after leasing") {
    val s = settings(0)
    val setup = for {
      master <- accountGen
      alice  <- accountGen
      ...
      transferAssetTx: TransferTransaction = TransferTransaction
        .selfSigned(1.toByte, master, alice.toAddress, IssuedAsset(assetId), issueTx.quantity, unitoken, fee, ByteStr.empty,  ts + 2)
        .explicitGet()
    ...
    forAll(setup) {
      case (preconditions, senderNotIssuer, _, insufficientReducedFee) =>
        ...
        assertDiffEi(setupBlocks, block(Seq(insufficientReducedFee)), s) { blockDiffEi =>
          blockDiffEi should produce("(99999 in unitoken) does not exceed minimal value of 100000 unitoken")
        }
    }
  }
  ...
  property(s"sponsor has no unitoken but receives them just in time before $BlockV5 activation") {
    val s = settings(0)
    val setup = for {
      master    <- accountGen
      recipient <- accountGen
      ...
      assetTransfer = TransferTransaction.selfSigned(1.toByte, master, recipient.toAddress, assetId, issue.quantity, unitoken, 100000, ByteStr.empty,  ts + 3)
        .explicitGet()
      unitokenTransfer = TransferTransaction.selfSigned(1.toByte, master, recipient.toAddress, unitoken, 99800000, unitoken, 100000, ByteStr.empty,  ts + 4)
        .explicitGet()
      backunitokenTransfer = TransferTransaction
        .selfSigned(1.toByte, recipient, master.toAddress, unitoken, 100000, assetId, 100, ByteStr.empty,  ts + 5)
        .explicitGet()
    } yield (genesis, issue, sponsor, assetTransfer, unitokenTransfer, backunitokenTransfer)

    forAll(setup) {
      case (genesis, issue, sponsor, assetTransfer, unitokenTransfer, backunitokenTransfer) =>
        assertDiffAndState(
          Seq(block(Seq(genesis, issue, sponsor, assetTransfer, unitokenTransfer))),
          block(Seq(backunitokenTransfer)),
        ...
        assertDiffEi(
          ...
          ei should produce("negative unitoken balance")
        }
    }
  }
}
