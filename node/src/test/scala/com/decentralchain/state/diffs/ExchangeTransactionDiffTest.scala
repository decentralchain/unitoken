...
import com.decentralchain.transaction.assets.exchange.{Order, _}
...
import com.decentralchain.{NoShrink, TestValues, TransactionGen, crypto}
...
class ExchangeTransactionDiffTest
  ...
  private def unitokenPortfolio(amt: Long) = Portfolio.unitoken(amt)
  ...
  property("Preserves unitoken invariant, stores match info, rewards matcher") {
  ...
  property("Preserves assets invariant (matcher's fee in one of the assets of the pair or in unitoken), stores match info, rewards matcher") {
            ...
            val restoredMatcherPortfolio =
              Monoid.combineAll(
                Seq(
                  ...
                  unitokenPortfolio(-exchange.fee)
                )
              )
  ...
  property("buy unitoken without enough money for fee") {
    val preconditions: Gen[(GenesisTransaction, GenesisTransaction, IssueTransaction, ExchangeTransaction)] = for {
      ...
      exchange <- Gen.oneOf(
        exchangeV1GeneratorP(buyer, seller, unitoken, IssuedAsset(issue1.id()), fixedMatcherFee = Some(300000)),
        exchangeV2GeneratorP(buyer, seller, unitoken, IssuedAsset(issue1.id()), fixedMatcherFee = Some(300000))
      )
    } yield {
  ...
  property("small fee cases") {
    ...
    forAll(preconditions, priceGen) {
      case ((buyer, seller, matcher, gen1, gen2, issue1), price) =>
        val assetPair = AssetPair(IssuedAsset(issue1.id()), unitoken)
        val buy = Order.buy(
          Order.V1,
          sender = buyer,
  ...
  property("Diff for ExchangeTransaction works as expected and doesn't use rounding inside") {
    ...
    val assetPair                                          = AssetPair(unitoken, IssuedAsset(issue1.id()))
  ...
  property("Disable use Order on SmartAccount") {
    ...
    forAll(for {
      ...
      tr1     = createunitokenTransfer(MATCHER, buyer.toAddress, Long.MaxValue / 3, enoughFee, ts + 1).explicitGet()
      tr2     = createunitokenTransfer(MATCHER, seller.toAddress, Long.MaxValue / 3, enoughFee, ts + 2).explicitGet()
  ...
  property("ExchangeTransaction with Orders V4 uses asset decimals for price calculation") {
    ...
    val unitokenUsdn   = AssetPair(unitoken, usdn)
    val tidexunitoken  = AssetPair(tidex, unitoken)
    val liquidunitoken = AssetPair(liquid, unitoken)
    ...
    val scenarios = Table(
      ...
      (
        mkExchange(TxVersion.V2, Order.V3, Order.V3, 55768188998L, 592600L, 592600L, 592600L, unitokenUsdn),
        mkExchange(TxVersion.V3, Order.V4, Order.V4, 55768188998L, 59260000L, 59260000L, 59260000L, unitokenUsdn),
        mkExchange(TxVersion.V3, Order.V3, Order.V4, 55768188998L, 59260000L, 592600L, 59260000L, unitokenUsdn),
        mkExchange(TxVersion.V3, Order.V4, Order.V3, 55768188998L, 59260000L, 59260000L, 592600L, unitokenUsdn)
      ),
      (
        mkExchange(TxVersion.V2, Order.V3, Order.V3, 213L, 35016774000000L, 35016774000000L, 35016774000000L, tidexunitoken),
        mkExchange(TxVersion.V3, Order.V4, Order.V4, 213L, 35016774L, 35016774L, 35016774L, tidexunitoken),
        mkExchange(TxVersion.V3, Order.V3, Order.V4, 213L, 35016774L, 35016774000000L, 35016774L, tidexunitoken),
        mkExchange(TxVersion.V3, Order.V4, Order.V3, 213L, 35016774L, 35016774L, 35016774000000L, tidexunitoken)
      ),
      (
        mkExchange(TxVersion.V2, Order.V3, Order.V3, 2000000000L, 13898832L, 13898832L, 13898832L, liquidunitoken),
        mkExchange(TxVersion.V3, Order.V4, Order.V4, 2000000000L, 13898832L, 13898832L, 13898832L, liquidunitoken),
        mkExchange(TxVersion.V3, Order.V3, Order.V4, 2000000000L, 13898832L, 13898832L, 13898832L, liquidunitoken),
        mkExchange(TxVersion.V3, Order.V4, Order.V3, 2000000000L, 13898832L, 13898832L, 13898832L, liquidunitoken)
      )
    )
    ...
    val scenario =
      for {
        ...
        ttx4 = TransferTransaction
          ...
          .explicitGet()
        assets = Seq(IssuedAsset(itx1.assetId), IssuedAsset(itx2.assetId), unitoken)
  ...
  property(s"Accepts failed transactions after ${BlockchainFeatures.BlockV5} activation") {
    val scenario =
      for {
        ...
        eTx <- exchangeGeneratorP(buyer, seller, asset, unitoken, fixedMatcher = Some(MATCHER))
          .flatMap { tx =>
            for {
        ...
        buyerBalance   = Map(unitoken -> ENOUGH_AMT, asset         -> 0L)
        sellerBalance  = Map(unitoken -> (ENOUGH_AMT - fee), asset -> iTx.quantity)
        matcherBalance = Map(unitoken -> ENOUGH_AMT, asset         -> 0L)
      ...
      case (exchange, (buyerBalance, sellerBalance, matcherBalance), genesisTxs, throwingScript) =>
        ...
        assertDiffAndState(Seq(TestBlock.create(genesisTxs)), TestBlock.create(Seq(exchange), Block.ProtoBlockVersion), fsWithBlockV5) {
          case (diff, state) =>
            ...
            state.balance(exchange.sender.toAddress, unitoken) shouldBe matcherBalance(unitoken) - exchange.fee
  ...
  property("Counts exchange fee asset complexity") {
    def test(tradeableAssetIssue: IssueTransaction, feeAssetIssue: IssueTransaction, complexity: Long): Unit = {
      ...
      withDomain(
        ...
        val feeUnits = FeeValidation.getMinFee(d.blockchainUpdater, exchange).explicitGet().minFeeInunitoken / FeeValidation.FeeUnit
