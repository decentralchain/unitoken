...
import com.decentralchain.transaction.{Asset, CreateAliasTransaction, GenesisTransaction, Transaction}
...
class CreateAliasTransactionDiffTest extends PropSpec with PropertyChecks with WithState with TransactionGen with NoShrink {
  ...
  property("can create and resolve aliases preserving unitoken invariant") {
          ...
          case (blockDiff, _) =>
            if (transfer.sender.toAddress != aliasTx.sender.toAddress) {
              ...
              transfer.assetId match {
                ...
                case unitoken                => recipientPortfolioDiff shouldBe Portfolio(transfer.amount, LeaseBalance.empty, Map.empty)
              }
            }
        }
    }
