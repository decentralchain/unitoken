...
import com.decentralchain.lang.directives.values.{Asset, V4}
...
import com.decentralchain.state.diffs.{ENOUGH_AMT, _}
    ...
    forAll(multiActionPreconditions(feeMultiplier = 5, withScriptError = false)) {
      case (genesis, setScript, invoke, issue, _, _, _, _, _) =>
        assertDiffEi(
          ...
          TestBlock.create(Seq(invoke)),
          features
        )(_ should produce(s" with 6 total scripts invoked does not exceed minimal value of $minimalFee unitoken"))
    }
  }
    ...
    val deleteEntryPreconditions: Gen[(List[GenesisTransaction], SetScriptTransaction, InvokeScriptTransaction, KeyPair)] =
      for {
        ...
        for {
          ...
          ci       <- InvokeScriptTransaction.selfSigned(1.toByte, invoker, master.toAddress, None, Nil, fee, unitoken, ts + 3)
  ...
  private def multiActionPreconditions(
    ...
    for {
      ...
      for {
        ...
        ci       <- InvokeScriptTransaction.selfSigned(TxVersion.V1, invoker, master.toAddress, None, Nil, fee, unitoken, ts + 3)
