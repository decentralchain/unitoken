...
import com.decentralchain.features.{BlockchainFeatures, EstimatorProvider}
import com.decentralchain.lagonaki.mocks.TestBlock
import com.decentralchain.lang.directives.values.{Expression, StdLibVersion, V1, V4}
...
import com.decentralchain.{BlocksTransactionsHelpers, NoShrink, TransactionGen}
...
class AssetTransactionsDiffTest
  ...
  property("Issue+Reissue+Burn do not break unitoken invariant and updates state") {
  ...
  property("Can burn non-owned alias if feature 'BurnAnyTokens' activated") {
    val setup = for {
      ...
      assetTransfer <- transferGeneratorP(issuer, burner.toAddress, IssuedAsset(issue.assetId), unitoken)
      unitokenTransfer <- unitokenTransferGeneratorP(issuer, burner.toAddress)
      burn = BurnTransaction
        .selfSigned(1.toByte, burner, IssuedAsset(issue.assetId), assetTransfer.amount, unitokenTransfer.amount, timestamp)
        .explicitGet()
    } yield (genesis, issue, assetTransfer, unitokenTransfer, burn)
    ...
    forAll(setup) {
      case (genesis, issue, assetTransfer, unitokenTransfer, burn) =>
        assertDiffAndState(Seq(TestBlock.create(Seq(genesis, issue, assetTransfer, unitokenTransfer))), TestBlock.create(Seq(burn)), fs) {
          case (_, newState) =>
  ...
  property("Can not total issue > long.max") {
    val setup = for {
      ...
      transfer = TransferTransaction.selfSigned(1.toByte, issuer, holder.toAddress, assetId, quantity - 1, unitoken, fee, ByteStr(attachment), timestamp)
        .explicitGet()
  ...
  def genesisIssueTransferReissue(
    ...
    for {
      timestamp          <- timestampGen
      initialunitokenAmount <- Gen.choose(Long.MaxValue / 1000, Long.MaxValue / 100)
      accountA           <- accountGen
      accountB           <- accountGen
      smallFee           <- Gen.choose(1L, 10L)
      genesisTx1 = GenesisTransaction.create(accountA.toAddress, initialunitokenAmount, timestamp).explicitGet()
      genesisTx2 = GenesisTransaction.create(accountB.toAddress, initialunitokenAmount, timestamp).explicitGet()
      reissuable = true
      ...
      transfer = TransferTransaction
        .selfSigned(TxVersion.V1, accountA, accountB.toAddress, assetId, issue.quantity, unitoken, smallFee, ByteStr.empty,  timestamp + 2)
        .explicitGet()
