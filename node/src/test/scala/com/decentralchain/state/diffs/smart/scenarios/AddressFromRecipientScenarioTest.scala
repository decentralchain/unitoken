...
import com.decentralchain.state.diffs.smart.predef._
import com.decentralchain.transaction.Asset.unitoken
import com.decentralchain.transaction.transfer._
import com.decentralchain.transaction.{CreateAliasTransaction, GenesisTransaction}
...
class AddressFromRecipientScenarioTest extends PropSpec with PropertyChecks with WithState with TransactionGen with NoShrink {
  ...
  val preconditionsAndAliasCreations: Gen[(Seq[GenesisTransaction], CreateAliasTransaction, TransferTransaction, TransferTransaction)] = for {
    ...
    transferViaAddress <- transferGeneratorP(master, other.toAddress, unitoken, unitoken)
    transferViaAlias   <- transferGeneratorP(master, AddressOrAlias.fromBytes(alias.bytes, 0).explicitGet()._1, unitoken, unitoken)
