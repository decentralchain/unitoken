...
class ScriptsCountTest extends PropSpec with PropertyChecks with WithState with TransactionGen with NoShrink with Inside {
  ...
  property("check pre-Ride4DApps scripts run count") {
    forAll(for {
      ...
      tr1 = TransferTransaction.selfSigned(2.toByte, master, acc.toAddress, unitoken, 10000000000L, unitoken, fee, ByteStr.empty, timestamp)
        .explicitGet()
      tr2 = TransferTransaction
        .selfSigned(2.toByte, master, acc.toAddress, IssuedAsset(issueScr.id()), 1000000000L, unitoken, fee, ByteStr.empty, timestamp)
        .explicitGet()
      mt1 = MassTransferTransaction
        .selfSigned(1.toByte, master, unitoken, List(ParsedTransfer(acc.toAddress, 1)), fee, timestamp, ByteStr.empty)
        .explicitGet()
  ...
  property("check scripts run count") {
    forAll(for {
      ...
      tr1 = TransferTransaction
        .selfSigned(2.toByte, master, acc.toAddress, unitoken, 10000000000L, unitoken, fee, ByteStr.empty, timestamp)
        .explicitGet()
      tr2 = TransferTransaction
        .selfSigned(2.toByte, master, acc.toAddress, IssuedAsset(issueScr.id()), 1000000000L, unitoken, fee, ByteStr.empty, timestamp)
        .explicitGet()
