      ...
      case (genesis, setDApp, ci, activation) =>
        assertDiffEi(Seq(TestBlock.create(genesis :+ setDApp)), TestBlock.create(Seq(ci)), features(activation)) { r =>
          if (activation) r should produce("AccountBalanceError")
            ...
            r should produce(
              s"Fee in unitoken for InvokeScriptTransaction (1 in unitoken) does not exceed minimal value of $InvokeFee unitoken"
            )
        }
    }
  }
    ...
    forAll(
      ...
      case (genesis, setDApp, ci, activation, issue) =>
        assertDiffEi(Seq(TestBlock.create(genesis ++ List(setDApp, issue))), TestBlock.create(Seq(ci)), features(activation)) {
          _ should produce("leads to negative unitoken balance to (at least) temporary negative state")
        }
    }
  }
  ...
  private def paymentPreconditions(
    ...
    for {
      ...
      for {
        ...
        ci       <- InvokeScriptTransaction.selfSigned(1.toByte, invoker, master.toAddress, None, payment, fee, unitoken, ts + 3)
  ...
  private def splitPaymentPreconditions(
    ...
    for {
      ...
      for {
        ...
        ci       <- InvokeScriptTransaction.selfSigned(1.toByte, invoker, master.toAddress, None, payments, InvokeFee, unitoken, ts + 3)
      } yield (List(genesis, genesis2), setDApp, ci, issue)
    }.explicitGet()
