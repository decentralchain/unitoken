...
class LeaseTransactionsDiffTest extends PropSpec with PropertyChecks with WithState with TransactionGen with NoShrink {
  ...
  property("can lease/cancel lease preserving unitoken invariant") {
  ...
  def cancelLeaseTwice(ts: Long): Gen[(GenesisTransaction, TransferTransaction, LeaseTransaction, LeaseCancelTransaction, LeaseCancelTransaction)] =
    for {
      ...
      payment <- unitokenTransferGeneratorP(ts, master, recpient.toAddress) suchThat (_.amount > lease.amount)
