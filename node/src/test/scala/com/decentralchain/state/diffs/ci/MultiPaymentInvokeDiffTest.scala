...
class MultiPaymentInvokeDiffTest extends PropSpec with PropertyChecks with Matchers with TransactionGen with NoShrink with WithState with Inside {
  property("multi payment with verifier and transfer set") {
    val unitokenTransfer = 111
    forAll(
      paymentPreconditions(
        dApp(V4, transferPaymentAmount = unitokenTransfer, _),
      ...
      case (genesis, setVerifier, setDApp, ci, issues, dAppAcc, invoker, fee) =>
        assertDiffAndState(
          ...
          case (diff, blockchain) =>
            ...
            diff.portfolios(dAppAcc.toAddress).balance shouldBe -unitokenTransfer
            diff.portfolios(invoker.toAddress).balance shouldBe unitokenTransfer - fee
        }
    }
  }
    ...
    forAll(
      ...
      case (genesis, setVerifier, setDApp, ci, issues, _, _, _) =>
        assertDiffEi(
          ...
          _ should produce(
            s"Fee in unitoken for InvokeScriptTransaction (${ci.fee} in unitoken) " +
              s"with ${ContractLimits.MaxAttachedPaymentAmount} total scripts invoked " +
              s"does not exceed minimal value of ${expectedFee.toLong} unitoken"
          )
        }
    }
  }
  ...
  property("single payment V3 scripts with activated multi payment") {
    val unitokenTransfer = 100
    forAll(
      paymentPreconditions(
        dApp(V3, transferPaymentAmount = unitokenTransfer, _),
  ...
  private def paymentPreconditions(
    ...
    for {
      ...
      for {
        ...
        ci <- InvokeScriptTransaction.selfSigned(1.toByte, invoker, master.toAddress, None, payments, fee, unitoken, ts + 3)
