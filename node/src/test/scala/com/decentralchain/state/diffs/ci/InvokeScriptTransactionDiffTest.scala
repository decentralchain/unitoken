...
import com.decentralchain.lang.v1.evaluator.FunctionIds.{CREATE_LIST, THROW}
import com.decentralchain.lang.v1.evaluator.ctx.impl.unitoken.{FieldNames, unitokenContext}
import com.decentralchain.lang.v1.evaluator.ctx.impl.{CryptoContext, PureContext}
import com.decentralchain.lang.v1.evaluator.{FunctionIds, ScriptResultV3}
...
import com.decentralchain.settings.{TestFunctionalitySettings, TestSettings}
...
import com.decentralchain.state.diffs.{ENOUGH_AMT, FeeValidation, produce}
...
import com.decentralchain.transaction.assets._
...
import com.decentralchain.transaction.{Asset, _}
...
class InvokeScriptTransactionDiffTest
  ...
  def paymentContract(
      ...
      assets: List[Asset] = List(unitoken),
  ...
  def paymentContractGen(address: Address, amount: Long, assets: List[Asset] = List(unitoken), version: StdLibVersion = V3)(func: String): Gen[DApp] =
    for {
      senderBinging <- validAliasStringGen
      argBinding    <- validAliasStringGen
    } yield paymentContract(senderBinging, argBinding, func, address, amount, assets, version)

  def defaultPaymentContractGen(address: AddressOrAlias, amount: Long, assets: List[Asset] = List(unitoken))(someName: String): Gen[DApp] =
    for {
      senderBinging <- validAliasStringGen
      argBinding    <- validAliasStringGen
  ...
  def preconditionsAndSetContract(
    ...
    for {
      ...
      (issueTx, sponsorTx, _, _) <- sponsorFeeCancelSponsorFeeGen(master)
      sponsoredFee = Sponsorship.fromunitoken(900000L, sponsorTx.minSponsoredAssetFee.get)
      ...
      ci = InvokeScriptTransaction
        .selfSigned(
          ...
          if (sponsored) sponsorTx.asset else unitoken,
          ts + 3
        )
        .explicitGet()
    ...
    forAll(for {
      ...
      case (acc, amount, genesis, setScript, ci) =>
        assertDiffAndState(Seq(TestBlock.create(genesis ++ Seq(setScript))), TestBlock.create(Seq(ci), Block.ProtoBlockVersion), fs) {
          case (blockDiff, newState) =>
            blockDiff.scriptsRun shouldBe 1
            newState.balance(acc.toAddress, unitoken) shouldBe amount
        }
    }
  }
  ...
  property("can't make more than 10 payments") {
    forAll(for {
      a  <- accountGen
      am <- smallFeeGen
      contractGen = paymentContractGen(a.toAddress, am, List.fill(11)(unitoken)) _
      r <- preconditionsAndSetContract(contractGen)
    } yield (a, am, r._1, r._2, r._3)) {
      case (_, _, genesis, setScript, ci) =>
  ...
  property("Contract payment should be positive") {
    forAll(for {
      ...
      case (acc, _, genesis, setScript, ci, asset, master, ts) =>
        val t =
          TransferTransaction
            .selfSigned(2.toByte, master, acc.toAddress, IssuedAsset(asset.id()), asset.quantity / 10, unitoken, enoughFee, ByteStr.empty, ts)
            .explicitGet()
  ...
  property("payment should be positive") {
    forAll(for {
      ...
      ci = InvokeScriptTransaction.selfSigned(1.toByte, invoker, master.toAddress, Some(fc), Seq(Payment(-1, unitoken)), fee, unitoken, ts)
  ...
  property("can't overflow payment + fee") {
    forAll(for {
      ...
      r <- preconditionsAndSetContract(
        contractGen,
        invokerGen = Gen.oneOf(Seq(invoker)),
        payment = Some(Payment(Long.MaxValue, unitoken)),
        feeGen = ciFee(1)
      )
    } yield (r._1, r._2, r._3)) {
      case (genesis, setScript, ci) =>
  ...
  property("can't overflow sum of payment in contract") {
    forAll(for {
      a  <- accountGen
      am <- smallFeeGen
      contractGen = paymentContractGen(a.toAddress, Long.MaxValue / 2 + 2, List.fill(4)(unitoken)) _
      invoker <- accountGen
      ts      <- timestampGen
      r       <- preconditionsAndSetContract(contractGen, invokerGen = Gen.oneOf(Seq(invoker)), payment = Some(Payment(1, unitoken)), feeGen = ciFee(1))
    } yield (r._1, r._2, r._3)) {
      case (genesis, setScript, ci) =>
        assertDiffEi(Seq(TestBlock.create(genesis ++ Seq(setScript))), TestBlock.create(Seq(ci)), fs) {
          _ should produce("Attempt to transfer unavailable funds")
  ...
  property("Default function invocation should produce error if contract default function has arguments") {
    forAll(for {
      ...
      senderBinging <- validAliasStringGen
      argBinding    <- validAliasStringGen
      contractGen   <- Gen.const((someStr: String) => Gen.const(paymentContract(senderBinging, argBinding, "default", a.toAddress, am, List(unitoken))))
      r             <- preconditionsAndSetContract(contractGen, accountGen, accountGen, None, ciFee(0), sponsored = false, isCIDefaultFunc = true)
    } yield (a, am, r._1, r._2, r._3)) {
      case (_, _, genesis, setScript, ci) =>
        assertDiffEi(Seq(TestBlock.create(genesis ++ Seq(setScript))), TestBlock.create(Seq(ci)), fs) {
  ...
  property("Default function invocation should produce error if contract does't have default function") {
    forAll(for {
      ...
      senderBinging <- validAliasStringGen
      argBinding    <- validAliasStringGen
      contractGen   <- Gen.const((someStr: String) => Gen.const(paymentContract(senderBinging, argBinding, "undefault", a.toAddress, am, List(unitoken))))
      r             <- preconditionsAndSetContract(contractGen, accountGen, accountGen, None, ciFee(0), sponsored = false, isCIDefaultFunc = true)
    } yield (a, am, r._1, r._2, r._3)) {
      case (_, _, genesis, setScript, ci) =>
        assertDiffEi(Seq(TestBlock.create(genesis ++ Seq(setScript))), TestBlock.create(Seq(ci)), fs) {
  ...
  property("self-payment and self-transfer V3") {
    forAll(for {
      acc <- accountGen
      am  <- smallFeeGen
      contractGen = paymentContractGen(acc.toAddress, am, assets = List(unitoken)) _
      r <- preconditionsAndSetContract(
        contractGen,
        invokerGen = Gen.const(acc),
        masterGen = Gen.const(acc),
        payment = Some(Payment(1, unitoken)),
        feeGen = ciFee(1)
      )
    } yield (r._1, r._2, r._3)) {
      case (genesis, setScript, ci) =>
  ...
  property("self-transfer V4") {
    forAll(for {
      acc <- accountGen
      am  <- smallFeeGen
      contractGen = paymentContractGen(acc.toAddress, am, assets = List(unitoken), version = V4) _
      r <- preconditionsAndSetContract(
        contractGen,
        invokerGen = Gen.const(acc),
        masterGen = Gen.const(acc),
    ...
    val uniqueAssetIdScenario =
      for {
        ...
        invokeTx = InvokeScriptTransaction
          .selfSigned(TxVersion.V2, invoker, master.toAddress, Some(fc), Seq(), fee, unitoken, ts + 3)
          .explicitGet()
  ...
  property(s"accepts failed transactions after ${BlockchainFeatures.BlockV5} activation") {
    def failInvariant(funcBinding: String, sponsorTx: SponsorFeeTransaction, issueTx: IssueTransaction): Gen[(TxAmount, Asset, DApp, List[EXPR])] = {
      val feeInunitoken = FeeConstants(InvokeScriptTransaction.typeId) * FeeValidation.FeeUnit
      val feeInAsset = Sponsorship.fromunitoken(FeeConstants(InvokeScriptTransaction.typeId) * FeeValidation.FeeUnit, sponsorTx.minSponsoredAssetFee.get)
      Gen.oneOf(
        Gen.const((feeInunitoken, unitoken, issueContract(funcBinding), List.empty[EXPR])),           // insufficient fee
        Gen.const((feeInAsset, sponsorTx.asset, issueContract(funcBinding), List.empty[EXPR])), // insufficient fee
        Gen.const((feeInunitoken, unitoken, throwContract(funcBinding), List.empty[EXPR])),           // DApp script execution
        ...
        } yield (fee, unitoken, paymentContract, List(CONST_BYTESTR(ByteStr(arg)).explicitGet()))
      )
    }
    ...
    val failedTxScenario =
      for {
        ...
        tTx = TransferTransaction
          .selfSigned(TxVersion.V3, other, invoker.toAddress, sTx.asset, i1Tx.quantity, unitoken, enoughFee, ByteStr.empty, ts)
          .explicitGet()
        funcBinding                     <- funcNameGen
    ...
    forAll(failedTxScenario) {
      case (invoke, (unitokenBalance, sponsoredBalance), genesisTxs) =>
        assertDiffAndState(Seq(TestBlock.create(genesisTxs)), TestBlock.create(Seq(invoke), Block.ProtoBlockVersion), fsWithV5) {
          case (diff, state) =>
            ...
            state.balance(invoke.sender.toAddress, invoke.feeAssetId) shouldBe invoke.feeAssetId.fold(unitokenBalance)(_ => sponsoredBalance) - invoke.fee
  ...
  property(
    ...
    val scenario =
      for {
        ...
        tTx = TransferTransaction
          .selfSigned(TxVersion.V3, other, master.toAddress, sponsoredAsset, iTx.quantity / 2, unitoken, enoughFee, ByteStr.empty, ts)
          .explicitGet()
        unitokenFee <- ciFee(1)
        sponsoredFee = Sponsorship.fromunitoken(unitokenFee, sTx.minSponsoredAssetFee.get)
        (feeAsset, fee) <- Gen.oneOf((unitoken, unitokenFee), (sponsoredAsset, sponsoredFee))
        funcBinding     <- funcNameGen
        arg             <- genBoundedStringBytes(1, 32)
        ...
        txs = Seq("throw", "insufficient fee", "negative amount", "overflow amount", "self payment", "max actions", "invalid data entries", "ok")
          .map { arg =>
            ...
            InvokeScriptTransaction
              .selfSigned(TxVersion.V2, invoker, master.toAddress, Some(fc), Seq(), fee, unitoken, ts + 4)
              .explicitGet()
          }
    ...
    val scenario =
      for {
        ...
        tTxs = iTxs.takeRight(3).map { tx =>
          TransferTransaction
            .selfSigned(1.toByte, master, invoker.toAddress, IssuedAsset(tx.assetId), ENOUGH_AMT / 2, unitoken, fee, ByteStr.empty, ts + 3)
            .explicitGet()
        }
        ...
        invokeTx = InvokeScriptTransaction
          .selfSigned(TxVersion.V3, invoker, master.toAddress, Some(fc), payments, fee, unitoken, ts + 6)
          .explicitGet()
