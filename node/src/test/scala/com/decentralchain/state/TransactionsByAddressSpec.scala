...
class TransactionsByAddressSpec extends FreeSpec with ScalaCheckDrivenPropertyChecks with BlockGen with WithDomain with Matchers with NoShrink {
  ...
  val gen = for {
    ...
    genesisBlock = Block
      .genesis(
        GenesisSettings(
          genesisTimestamp,
          genesisTimestamp,
          Constants.Totalunitoken,
          None,
          Seq(GenesisTransactionSettings(sender.toAddress.stringRepr, Constants.Totalunitoken)),
          1000,
          1.minute
        )
      )
    ...
    transactions1 <- Gen.listOfN(txCount1, transferGen(sender, Gen.oneOf(recipient1, recipient2).map(_.toAddress), Constants.Totalunitoken / 2 / txCount1))
    block1 = mkBlock(sender, genesisBlock.id(), transactions1)
    txCount2      <- Gen.choose(10, 50)
    transactions2 <- Gen.listOfN(txCount2, transferGen(sender, Gen.oneOf(recipient1, recipient2).map(_.toAddress), Constants.Totalunitoken / 2 / txCount2))
