...
import com.decentralchain.features.BlockchainFeatures._
import com.decentralchain.features._
...
import com.decentralchain.lang.directives.values.{Account, V4}
...
import com.decentralchain.lang.v1.{FunctionHeader, compiler}
import com.decentralchain.settings.{TestFunctionalitySettings, unitokenSettings}
...
import com.decentralchain.transaction.{CreateAliasTransaction, DataTransaction, GenesisTransaction, Transaction, TxVersion}
import com.decentralchain.utils.StringBytes
import com.decentralchain.{NoShrink, TestTime, TransactionGen, history}
...
class RollbackSpec extends FreeSpec with Matchers with WithDomain with TransactionGen with PropertyChecks with NoShrink {
  ...
  private def transfer(sender: KeyPair, recipient: Address, amount: Long) =
    TransferTransaction.selfSigned(1.toByte, sender, recipient, unitoken, amount, unitoken, 1, ByteStr.empty, nextTs).explicitGet()
  ...
  private def randomOp(sender: KeyPair, recipient: Address, amount: Long, op: Int, nextTs: => Long = nextTs) = {
    ...
    op match {
      ...
      case _ => List(TransferTransaction.selfSigned(1.toByte, sender, recipient, unitoken, amount, unitoken, 1000, ByteStr.empty, nextTs).explicitGet())
    }
  }
  ...
  "Rollback resets" - {
    ...
    "forget rollbacked transaction for querying" in forAll(accountGen, accountGen, Gen.nonEmptyListOf(Gen.choose(1, 10))) {
      case (sender, recipient, txCount) =>
        withDomain(createSettings(MassTransfer -> 0)) { d =>
          d.appendBlock(genesisBlock(nextTs, sender.toAddress, com.decentralchain.state.diffs.ENOUGH_AMT))
    ...
    "unitoken balances" in forAll(accountGen, positiveLongGen, accountGen, Gen.nonEmptyListOf(Gen.choose(1, 10))) {
        ...
        withDomain() { d =>
          ...
          d.appendBlock(
            TestBlock.create(
              ...
              Seq(
                TransferTransaction
                  .selfSigned(1.toByte, sender, recipient.toAddress, IssuedAsset(issueTransaction.id()), assetAmount, unitoken, 1, ByteStr.empty,  nextTs)
                  .explicitGet()
              )
            )
          )
      ...
      def appendBlock(d: Domain, invoker: KeyPair, dApp: KeyPair)(parentBlockId: ByteStr, fc: Terms.FUNCTION_CALL): ByteStr = {
        ...
        val issue =
          InvokeScriptTransaction
            .selfSigned(2.toByte, invoker, dApp.toAddress, Some(fc), Seq.empty, fee, unitoken, nextTs)
            .explicitGet()
    ...
    def createSettings(preActivatedFeatures: (BlockchainFeature, Int)*): unitokenSettings = {
      ...
      history.DefaultunitokenSettings.copy(blockchainSettings = history.DefaultunitokenSettings.blockchainSettings.copy(functionalitySettings = tfs))
    }
    ...
    "relean rollbacked transaction" in forAll(accountGen, accountGen, Gen.listOfN(66, Gen.choose(1, 10))) {
      case (sender, recipient, txCount) =>
        withDomain(createSettings(MassTransfer -> 0)) { d =>
          ...
          d.appendBlock(genesisBlock(ts, sender.toAddress, com.decentralchain.state.diffs.ENOUGH_AMT))
...
object RollbackSpec {
  private val issueReissueBurnScript = {
    import com.decentralchain.lang.directives.values.{DApp => DAppType}
