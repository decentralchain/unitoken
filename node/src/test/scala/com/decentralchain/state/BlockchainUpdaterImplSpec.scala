...
import com.decentralchain.TestHelpers.enableNG
...
import com.decentralchain.settings.{TestFunctionalitySettings, unitokenSettings, loadConfig}
import com.decentralchain.state.diffs.ENOUGH_AMT
import com.decentralchain.state.utils._
...
import com.decentralchain.{EitherMatchers, NTPTime, RequestGen, WithDB}
...
class BlockchainUpdaterImplSpec
  ...
  private lazy val unitokenSettings         = unitokenSettings.fromRootConfig(loadConfig(ConfigFactory.load()))
  ...
  def baseTest(
    ...
    val (fs, settings) =
      if (enableNg) (enableNG(functionalitySettings), enableNG(unitokenSettings)) else (functionalitySettings, unitokenSettings)
  ...
  def createTransfer(master: KeyPair, recipient: Address, ts: Long): TransferTransaction = {
    TransferTransaction
      .selfSigned(1.toByte, master, recipient, unitoken, ENOUGH_AMT / 5, unitoken, 1000000, ByteStr.empty, ts)
      .explicitGet()
  }
  ...
  "blochain update events sending" - {
    ...
    "with NG" - {
      ...
      "block, then 2 microblocks, then block referencing previous microblock" in {
        ...
        val bcu =
          new BlockchainUpdaterImpl(defaultWriter, ignoreSpendableBalanceChanged, enableNG(unitokenSettings), ntpTime, triggersMock, (_, _) => Seq.empty)
