...
class StateHashSpec extends FreeSpec with Matchers {
  "state hash" - {
    ...
    stateHash.addunitokenBalance(address, 1000)
    ...
    "sections" - {
      ...
      "unitoken balance" in {
        sect(unitokenBalance) shouldBe hash(
          address.bytes,
    ...
    "total" in {
      ...
      allHashes shouldBe Seq(unitokenBalance, AssetBalance, DataEntry, AccountScript, AssetScript, LeaseBalance, LeaseStatus, Sponsorship, Alias)
        .map(sect)
      ...
      result.copy(hashes = result.hashes - SectionId.unitokenBalance).createStateHash(ByteStr.empty).totalHash shouldBe hash(
