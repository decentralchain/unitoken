...
import com.decentralchain.database.{LevelDBFactory, LevelDBWriter, TestStorageFactory, loadActiveLeases}
...
import com.decentralchain.state.diffs.{BlockDiffer, produce}
import com.decentralchain.state.reader.CompositeBlockchain
import com.decentralchain.state.utils.TestLevelDB
import com.decentralchain.state.{Blockchain, BlockchainUpdaterImpl, Diff}
...
trait WithDomain extends WithState { _: Suite =>
  def defaultDomainSettings: unitokenSettings =
    unitokenSettings.fromRootConfig(loadConfig(None))

  def domainSettingsWithFS(fs: FunctionalitySettings): unitokenSettings = {
    val ds = defaultDomainSettings
    ds.copy(blockchainSettings = ds.blockchainSettings.copy(functionalitySettings = fs))
  }

  def withDomain[A](settings: unitokenSettings = defaultDomainSettings)(test: Domain => A): A =
