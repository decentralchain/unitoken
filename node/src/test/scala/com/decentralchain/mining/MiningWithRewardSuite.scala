...
import com.decentralchain.database.{Keys, TestStorageFactory}
...
import com.decentralchain.state.{Blockchain, BlockchainUpdaterImpl, NG}
...
class MiningWithRewardSuite extends AsyncFlatSpec with Matchers with WithDB with TransactionGen with DBCacheSettings {
    ...
    val bps: Seq[BlockProducer] = Seq(
      (ts, reference, account) => {
        ...
        val tx1 = TransferTransaction
          .selfSigned(2.toByte, account, recipient1, unitoken, 10 * Constants.UnitsInWave, unitoken, 400000, ByteStr.empty, ts)
          .explicitGet()
        val tx2 = TransferTransaction
          .selfSigned(2.toByte, account, recipient2, unitoken, 5 * Constants.UnitsInWave, unitoken, 400000, ByteStr.empty, ts)
          .explicitGet()
  ...
  private def withEnv(bps: Seq[BlockProducer], txs: Seq[TransactionProducer] = Seq(), settings: unitokenSettings = MiningWithRewardSuite.settings)(
  ...
  private def resources(settings: unitokenSettings): Resource[Task, (BlockchainUpdaterImpl, DB)] =
    Resource.make {
...
object MiningWithRewardSuite {
  ...
  val settings: unitokenSettings = {
    val commonSettings: unitokenSettings = unitokenSettings.fromRootConfig(loadConfig(ConfigFactory.load()))
  ...
  def settingsWithFeatures(features: BlockchainFeature*): unitokenSettings = {
