...
import com.decentralchain.history.chainBaseAndMicro
import com.decentralchain.lagonaki.mocks.TestBlock
import com.decentralchain.lang.ValidationError
import com.decentralchain.protobuf.block.PBBlocks
import com.decentralchain.settings.{Constants, FunctionalitySettings, TestFunctionalitySettings, WalletSettings, unitokenSettings}
...
import com.decentralchain.transaction.{BlockchainUpdater, GenesisTransaction, Transaction, TxVersion}
import com.decentralchain.utils.Time
import com.decentralchain.utx.UtxPoolImpl
import com.decentralchain.wallet.Wallet
import com.decentralchain.{BlocksTransactionsHelpers, EitherMatchers, NoShrink, TestTime, TransactionGen, crypto, protobuf}
...
class BlockV5Test
  ...
  private def createTx(sender: KeyPair, recipient: AddressOrAlias): Transaction =
    TransferTransaction
      .selfSigned(TxVersion.V1, sender, recipient, unitoken, 10 * Constants.UnitsInWave, unitoken, 100000, ByteStr.empty, ntpTime.getTimestamp())
      .explicitGet()
  ...
  "blockId" should "be header hash" in {
    ...
    forAll(preconditions) {
      case (acc, genesis) =>
        ...
        withDomain(unitokenSettings.default().copy(blockchainSettings = unitokenSettings.default().blockchainSettings.copy(functionalitySettings = fs))) {
          d =>
  ...
  private def genesis: Gen[(KeyPair, KeyPair, Block)] =
    for {
      ...
      genesisBlock = TestBlock.create(
        ...
        txs = Seq(
          GenesisTransaction.create(miner1.toAddress, Constants.Totalunitoken / 2 * Constants.UnitsInWave, ntpNow).explicitGet(),
          GenesisTransaction.create(miner2.toAddress, Constants.Totalunitoken / 2 * Constants.UnitsInWave, ntpNow).explicitGet()
        ),
        version = Block.GenesisBlockVersion
      )
    } yield (miner1, miner2, genesisBlock)

  private def withBlockchain(disabledFeatures: AtomicReference[Set[Short]], time: Time = ntpTime, settings: unitokenSettings = testSettings)(
  ...
  private def withMiner(blockchain: Blockchain with BlockchainUpdater with NG, time: Time, settings: unitokenSettings = testSettings)(
...
object BlockV5Test {
  ...
  private val defaultSettings = unitokenSettings.fromRootConfig(ConfigFactory.load())
