...
import com.decentralchain.protobuf.utils.PBImplicitConversions.PBByteStringOps
import com.decentralchain.settings.{Constants, unitokenSettings}
import com.decentralchain.state.diffs.ENOUGH_AMT
import com.decentralchain.state.{Blockchain, Diff, NewTransactionInfo}
...
import com.decentralchain.transaction.{BlockchainUpdater, DataTransaction, GenesisTransaction, Transaction}
import com.decentralchain.{BlockGen, TestHelpers, crypto, state}
...
class BlockchainUpdateTriggersSpec extends FreeSpec with Matchers with BlockGen with ScalaCheckPropertyChecks with EventsHelpers {
  private val unitoken_AMOUNT = Constants.UnitsInWave * Constants.Totalunitoken
  ...
  private val initialAmount: Long = unitoken_AMOUNT / 2
  ...
  "updated unitoken amount is calculated correctly for miner reward" in forAll {
    for {
    ...
    case (reward, BlockAppended(_, _, _, updatedunitokenAmount, _, _)) =>
      updatedunitokenAmount shouldBe unitoken_AMOUNT + reward.getOrElse(0L)
  }
  ...
  "appends correctly" - {
    ...
    "block/microblock with balance updates from transfer txs" in forAll {
      for {
        ...
        sender2recipient = TransferTransaction.selfSigned(2.toByte, sender, recipient.toAddress, unitoken, master2sender.amount - fee, unitoken, fee, ByteStr.empty, 2)
          .explicitGet()
