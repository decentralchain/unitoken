...
import com.decentralchain.consensus.{PoSSelector, TransactionsOrdering}
import com.decentralchain.database.{LevelDBWriter, TestStorageFactory, openDB}
import com.decentralchain.db.WithDomain
import com.decentralchain.events.UtxEvent
import com.decentralchain.features.BlockchainFeatures
import com.decentralchain.history.Domain.BlockchainUpdaterExt
import com.decentralchain.history.randomSig
...
import com.decentralchain.lang.v1.compiler.{CompilerContext, ExpressionCompiler}
import com.decentralchain.mining._
import com.decentralchain.network.{InvalidBlockStorage, PeerDatabase}
import com.decentralchain.settings._
import com.decentralchain.state._
import com.decentralchain.state.appender.{ExtensionAppender, MicroblockAppender}
import com.decentralchain.state.diffs._
import com.decentralchain.state.utils.TestLevelDB
...
import com.decentralchain.transaction.{Asset, Transaction, _}
...
class UtxPoolSpecification
  ...
  private def mkBlockchain(senderAccount: Address, senderBalance: Long) = {
    ...
    val origSettings    = unitokenSettings.default()
  ...
  private def transfer(sender: KeyPair, maxAmount: Long, time: Time) =
    (for {
      ...
      .selfSigned(1.toByte, sender, recipient.toAddress, unitoken, amount, unitoken, fee, ByteStr.empty, time.getTimestamp())
      .explicitGet())
  ...
  private def transferV2(sender: KeyPair, maxAmount: Long, time: Time) =
    (for {
      ...
      .selfSigned(TxVersion.V2, sender, recipient.toAddress, unitoken, amount, unitoken, fee, ByteStr.empty, time.getTimestamp())
      .explicitGet())
  ...
  private def massTransferWithRecipients(sender: KeyPair, recipients: List[PublicKey], maxAmount: Long, time: Time) = {
    ...
    val txs = for { fee <- chooseNum(minFee, amount) } yield MassTransferTransaction
      .selfSigned(1.toByte, sender, unitoken, transfers, fee, time.getTimestamp(), ByteStr.empty)
      .explicitGet()
  ...
  private val twoOutOfManyValidPayments = (for {
    ...
    val txs        = for (_ <- 1 to n) yield createunitokenTransfer(sender, recipient.toAddress, amountPart, fee, time.getTimestamp()).explicitGet()
  ...
  private def transactionV1Gen(sender: KeyPair, ts: Long, feeAmount: Long): Gen[TransferTransaction] = accountGen.map { recipient =>
    TransferTransaction.selfSigned(1.toByte, sender, recipient.toAddress, unitoken, unitoken(1), unitoken, feeAmount, ByteStr.empty, ts).explicitGet()
  }

  private def transactionV2Gen(sender: KeyPair, ts: Long, feeAmount: Long): Gen[TransferTransaction] = accountGen.map { recipient =>
    TransferTransaction.selfSigned(2.toByte, sender, recipient.toAddress, unitoken, unitoken(1), unitoken, feeAmount, ByteStr.empty, ts).explicitGet()
  }
  ...
  "UTX Pool" - {
    ...
    "correctly process constraints in packUnconfirmed" in {
      withDomain(decentralchain.history.TransfersV2ActivatedAt0unitokenSettings) { d =>
      ...
      "doesnt validate transactions which are removed" in {
        ...
        forAll(gen) {
          case (tx1, rest) =>
            ...
            (() => blockchain.settings).when().returning(unitokenSettings.default().blockchainSettings)
            (() => blockchain.height).when().returning(1)
            (() => blockchain.activatedFeatures).when().returning(Map.empty)

            val utx =
              new UtxPoolImpl(ntpTime, blockchain, ignoreSpendableBalanceChanged, unitokenSettings.default().utxSettings)
    ...
    "priority pool" - {
        ...
        def addPriorityTxs(txs: Seq[Transaction]): Unit = {
          val asDiffs = txs.map {
            case tt: TransferTransaction =>
              ...
              ).view.mapValues(Portfolio.unitoken).toMap
              Diff(tt, pfs)
      ...
      def createState(scripted: Address, settings: unitokenSettings = unitokenSettings.default(), setBalance: Boolean = true): Blockchain = {
        val blockchain = stub[Blockchain]
      ...
      val settingsWithNG = decentralchain.history.settingsWithFeatures(BlockchainFeatures.NG, BlockchainFeatures.SmartAccounts)
      ...
      "fires events correctly" in {
        val preconditions = for {
          ...
          validTransfer = TransferTransaction
            .selfSigned(TxVersion.V1, richAcc, secondAcc.toAddress, unitoken, 1, unitoken, fee, ByteStr.empty, ts)
            .explicitGet()
          invalidTransfer = TransferTransaction
            .selfSigned(TxVersion.V1, secondAcc, richAcc.toAddress, unitoken, 2, unitoken, fee, ByteStr.empty, ts)
            .explicitGet()
        ...
        forAll(preconditions) {
          case (genesis, validTransfer, invalidTransfer) =>
            withDomain() { d =>
              ...
              val utxPool =
                new UtxPoolImpl(time, d.blockchainUpdater, ignoreSpendableBalanceChanged, unitokenSettings.default().utxSettings, events += _)
