...
import com.decentralchain.state.{BlockchainUpdaterImpl, Height, TransactionId, TxNum}
...
import com.decentralchain.{EitherMatchers, RequestGen, TransactionGen, WithDB, database}
...
class LevelDBWriterSpec
  ...
  def baseTest(gen: Time => Gen[(KeyPair, Seq[Block])])(f: (LevelDBWriter, KeyPair) => Unit): Unit = {
    ...
    val settings0     = unitokenSettings.fromRootConfig(loadConfig(ConfigFactory.load()))
  ...
  def createTransfer(master: KeyPair, recipient: Address, ts: Long): TransferTransaction =
    TransferTransaction
      .selfSigned(1.toByte, master, recipient, unitoken, ENOUGH_AMT / 10, unitoken, 1000000, ByteStr.empty, ts)
      .explicitGet()
