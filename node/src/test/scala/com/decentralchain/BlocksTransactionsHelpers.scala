...
trait BlocksTransactionsHelpers { self: TransactionGen =>
  object QuickTX {
    ...
    def transfer(
      ...
      for {
        timestamp <- timestamp
      } yield TransferTransaction.selfSigned(1.toByte, from, to, unitoken, amount, unitoken, FeeAmount, ByteStr.empty, timestamp).explicitGet()
    ...
    def transferV2(
      ...
      for {
        timestamp <- timestamp
      } yield TransferTransaction.selfSigned(2.toByte, from, to, unitoken, amount, unitoken, FeeAmount, ByteStr.empty, timestamp).explicitGet()
    ...
    def transferAsset(
      ...
      for {
        timestamp <- timestamp
      } yield TransferTransaction.selfSigned(1.toByte, from, to, asset, amount, unitoken, FeeAmount, ByteStr.empty, timestamp).explicitGet()
    ...
    def invokeScript(
      ...
      for {
        timestamp <- timestamp
      } yield InvokeScriptTransaction.selfSigned(1.toByte, from, dapp, Some(call), payments, FeeAmount * 2, unitoken, timestamp).explicitGet()
  }
