...
import com.decentralchain.transaction.{Asset, CreateAliasTransaction, DataTransaction, Proofs, Transaction, TxVersion, VersionedTransaction}
import com.decentralchain.utx.UtxPool
import com.decentralchain.{TestWallet, TransactionGen}
  ...
  "Proto transactions should be able to broadcast " - {
    ...
    "InvokeScriptTransaction" in test {
      ...
      val invokeScriptTxUnsigned = InvokeScriptTransaction
        .create(
          ...
          Seq(Payment(100, Asset.unitoken)),
          InvokeScriptTxFee,
    ...
    "TransferTransaction" in test {
      ...
      val transferTxUnsigned =
        TransferTransaction
          .create(TxVersion.V3, account.publicKey, recipient, asset, 100, Asset.unitoken, MinFee, ByteStr(attachment), Now, Proofs.empty)
          .explicitGet()
    ...
    "MassTransferTransaction" in test {
      ...
      val massTransferTxUnsigned =
        MassTransferTransaction
          .create(TxVersion.V2, account.publicKey, Asset.unitoken, transfers, MassTransferTxFee, Now, ByteStr(attachment), Proofs.empty)
          .explicitGet()
