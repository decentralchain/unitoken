...
import com.decentralchain.api.http.assets._
import com.decentralchain.api.http.requests.{SignedTransferV1Request, SignedTransferV2Request}
...
import com.decentralchain.transaction.{Asset, Proofs, Transaction}
  ...
  "returns appropriate error code when validation fails for" - {
    ...
    "transfer transaction" in {
      forAll(broadcastTransferReq) { tr =>
        ...
        forAll(nonPositiveLong) { q =>
          posting(tr.copy(amount = q)) should produce(NonPositiveAmount(s"$q of ${tr.assetId.getOrElse("unitoken")}"))
        }
        forAll(invalidBase58) { pk =>
  ...
  "compatibility" - {
    ...
    val versionedTransferRequest = createSignedVersionedTransferRequest(
      TransferTransaction(
        ...
        assetId = Asset.unitoken,
        amount = 1.unitoken,
        feeAssetId = Asset.unitoken,
        fee = 0.3.unitoken,
