...
import com.decentralchain.lang.v1.compiler.Terms.{CONST_BOOLEAN, CONST_LONG, FUNCTION_CALL}
...
import com.decentralchain.{BlockGen, NoShrink, TestTime, TestWallet, TransactionGen}
  ...
  routePath("/calculateFee") - {
    "transfer with unitoken fee" - {
      ...
      "TransferTransaction" in forAll(transferTxScenario) {
        case (_, transferTx) =>
          (addressTransactions.calculateFee _).expects(*).returning(Right((Asset.unitoken, 100000L, 0L))).once()
      ...
      "MassTransferTransaction" in forAll(massTransferTxScenario) {
        case (_, transferTx) =>
          (addressTransactions.calculateFee _).expects(*).returning(Right((Asset.unitoken, 200000L, 0L))).once()
