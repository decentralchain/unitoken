...
import com.decentralchain.state.{AssetDescription, AssetScriptInfo, Blockchain, Height, LeaseBalance, NG, StateHash, VolumeAndFee}
import com.decentralchain.transaction.TxHelpers
import com.decentralchain.transaction.assets.exchange.OrderType
import com.decentralchain.transaction.smart.script.ScriptCompiler
import com.decentralchain.{NTPTime, TestValues, TestWallet}
...
class DebugApiRouteSpec extends RouteSpec("/debug") with RestAPISettingsHelper with TestWallet with NTPTime with PathMockFactory {
  ...
  val configObject  = unitokenSettings.config.root()
  ...
  routePath("/validate") - {
    def createBlockchainStub(): Blockchain with NG = {
      trait Blockchain1 extends Blockchain with NG
      val blockchain = stub[Blockchain1]
      (() => blockchain.settings).when().returns(unitokenSettings.default().blockchainSettings)
    ...
    "valid tx" in {
      ...
      val tx = TxHelpers.transfer(TxHelpers.defaultSigner, TestValues.address, 1.unitoken)
