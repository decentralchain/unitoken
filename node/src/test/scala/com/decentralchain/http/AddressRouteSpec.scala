...
import com.decentralchain.api.http.AddressApiRoute
...
import com.decentralchain.{NoShrink, TestTime, TestWallet, WithDB, crypto}
...
class AddressRouteSpec
  ...
  routePath(s"/scriptInfo/${allAddresses(1)}") in {
    ...
    Get(routePath(s"/scriptInfo/${allAddresses(3)}")) ~> route ~> check {
      ...
      // [WAIT]                                           Decompiler(
      //      testContract,
      //      Monoid.combineAll(Seq(PureContext.build(com.decentralchain.lang.directives.values.StdLibVersion.V3), CryptoContext.build(Global))).decompilerContext)
