...
import com.decentralchain.network.PeerDatabaseImpl
...
class PeerDatabaseImplSpecification extends path.FreeSpecLike with Matchers {
  ...
  private val config1   = ConfigFactory.parseString("""unitoken.network {
      |  file = null
      |  known-peers = []
      |  peers-data-residence-time: 2s
      |}""".stripMargin).withFallback(ConfigFactory.load()).resolve()
  private val settings1 = config1.as[NetworkSettings]("unitoken.network")

  private val config2   = ConfigFactory.parseString("""unitoken.network {
      |  file = null
      |  known-peers = []
      |  peers-data-residence-time: 10s
      |}""".stripMargin).withFallback(ConfigFactory.load()).resolve()
  private val settings2 = config2.as[NetworkSettings]("unitoken.network")

  private val config3   = ConfigFactory.parseString(s"""unitoken.network {
                                                      |  file = null
                                                      |  known-peers = ["$host1:1"]
                                                      |  peers-data-residence-time: 2s
                                                      |  enable-peers-exchange: no
                                                      |}""".stripMargin).withFallback(ConfigFactory.load()).resolve()
  private val settings3 = config3.as[NetworkSettings]("unitoken.network")
  ...
  "Peer database" - {
    ...
    "if blacklisting is disabled" - {
      "should clear blacklist at start" in {
        val databaseFile = Files.createTempFile("unitoken-tests", "PeerDatabaseImplSpecification-blacklisting-clear").toAbsolutePath.toString
        val path         = if (File.separatorChar == '\\') databaseFile.replace('\\', '/') else databaseFile
        val prevConfig   = ConfigFactory.parseString(s"""unitoken.network {
             |  file = "$path"
             |  known-peers = []
             |  peers-data-residence-time: 100s
             |}""".stripMargin).withFallback(ConfigFactory.load()).resolve()
        val prevSettings = prevConfig.as[NetworkSettings]("unitoken.network")
        val prevDatabase = new PeerDatabaseImpl(prevSettings)
        prevDatabase.blacklist(address1.getAddress, "I don't like it")
        prevDatabase.close()

        val config   = ConfigFactory.parseString(s"""unitoken.network {
             |  file = "$path"
             |  known-peers = []
             |  peers-data-residence-time: 100s
             |  enable-blacklisting = no
             |}""".stripMargin).withFallback(ConfigFactory.load()).resolve()
        val settings = config.as[NetworkSettings]("unitoken.network")
