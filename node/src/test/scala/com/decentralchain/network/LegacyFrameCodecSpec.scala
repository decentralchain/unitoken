...
import com.decentralchain.network.message.{MessageSpec, Message => ScorexMessage}
...
import com.decentralchain.{TestValues, TransactionGen, crypto}
...
class LegacyFrameCodecSpec extends FreeSpec with Matchers with MockFactory with PropertyChecks with TransactionGen {
  ...
  "should pack update asset info in PB message" in {
    val tx = UpdateAssetInfoTransaction
      .selfSigned(1, TestValues.keyPair, TestValues.asset.id, "bomz", "", System.currentTimeMillis(), TestValues.fee, unitoken)
      .explicitGet()
