...
import com.decentralchain.transaction.assets.exchange._
import com.decentralchain.transaction.lease._
...
import com.decentralchain.transaction.transfer.MassTransferTransaction.{MaxTransferCount, ParsedTransfer}
...
trait TransactionGenBase extends ScriptGen with TypedScriptGen with NTPTime { _: Suite =>
  ...
  protected def unitoken(n: Float): Long = (n * 100000000L).toLong
  ...
  val unitokenAssetGen: Gen[Option[ByteStr]] = Gen.const(None)
  val assetIdGen: Gen[Option[ByteStr]]    = Gen.frequency((1, unitokenAssetGen), (10, Gen.option(bytes32gen.map(ByteStr(_)))))

  val assetPairGen: Gen[AssetPair] = assetIdGen.flatMap {
    case None => bytes32gen.map(b => AssetPair(unitoken, IssuedAsset(ByteStr(b))))
    case Some(a1bytes) =>
      ...
      Gen.oneOf(Gen.const(None), a2bytesGen).map { a2 =>
        ...
        val asset2 = a2.fold[Asset](unitoken)(arr => IssuedAsset(ByteStr(arr)))
  ...
  def transferGeneratorP(timestamp: Long, sender: KeyPair, recipient: AddressOrAlias, maxAmount: Long): Gen[TransferTransaction] =
    ...
    } yield TransferTransaction.selfSigned(1.toByte, sender, recipient, unitoken, amount, unitoken, feeAmount, attachment, timestamp).explicitGet()

  def transferGeneratorPV2(timestamp: Long, sender: KeyPair, recipient: AddressOrAlias, maxAmount: Long): Gen[TransferTransaction] =
    for {
      amount                                    <- Gen.choose(1, maxAmount)
      (_, _, _, _, _, _, feeAmount, attachment) <- transferParamGen
    } yield TransferTransaction.selfSigned(2.toByte, sender, recipient, unitoken, amount, unitoken, feeAmount, attachment, timestamp).explicitGet()
  ...
  def unitokenTransferGeneratorP(sender: KeyPair, recipient: AddressOrAlias): Gen[TransferTransaction] =
    transferGeneratorP(sender, recipient, unitoken, unitoken)

  def unitokenTransferGeneratorP(timestamp: Long, sender: KeyPair, recipient: AddressOrAlias): Gen[TransferTransaction] =
    transferGeneratorP(timestamp, sender, recipient, unitoken, unitoken)
  ...
  def createunitokenTransfer(
    ...
    TransferTransaction.selfSigned(1.toByte, sender, recipient, unitoken, amount, unitoken, fee, ByteStr.empty, timestamp)
  ...
  def versionedTransferGenP(sender: PublicKey, recipient: Address, proofs: Proofs): Gen[TransferTransaction] =
    (for {
      ...
      fee       <- smallFeeGen
      timestamp <- timestampGen
    } yield TransferTransaction(2.toByte, sender, recipient, unitoken, amt, unitoken, fee, ByteStr.empty, timestamp, proofs, recipient.chainId))
      .label("VersionedTransferTransactionP")

  val transferWithunitokenFeeGen: Gen[TransferTransaction] = for {
    (assetId, sender, recipient, amount, timestamp, _, feeAmount, attachment) <- transferParamGen
  } yield TransferTransaction.selfSigned(1.toByte, sender, recipient, assetId, amount, unitoken, feeAmount, attachment, timestamp).explicitGet()

  val selfTransferWithunitokenFeeGen: Gen[TransferTransaction] = for {
    (assetId, sender, _, amount, timestamp, _, feeAmount, attachment) <- transferParamGen
  } yield TransferTransaction.selfSigned(1.toByte, sender, sender.toAddress, assetId, amount, unitoken, feeAmount, attachment, timestamp).explicitGet()
  ...
  val updateAssetInfoTxGen: Gen[UpdateAssetInfoTransaction] =
    for {
      ...
      tx = UpdateAssetInfoTransaction
        .selfSigned(1.toByte, account, ByteStr(assetId), new String(assetName), new String(description), timestamp, fee, unitoken)
        .explicitGet()
    } yield tx
  ...
  def invokeScriptGen(paymentsGen: Gen[Seq[Payment]]): Gen[InvokeScriptTransaction] =
    for {
      ...
      .selfSigned(1.toByte, sender, dappAddress.toAddress, Some(fc), payments, fee, unitoken, timestamp)
      .explicitGet()

  val paymentListGen: Gen[Seq[Payment]] =
    for {
      unitokenPayment <- Gen.option(positiveLongGen.map(Payment(_, unitoken)))
      assetPayment = for {
    ...
    } yield assetPayments ++ unitokenPayment
  ...
  val exchangeTransactionGen: Gen[ExchangeTransaction] = for {
    ...
    buyerMatcherFeeAssetId  <- Gen.oneOf(assetPair.amountAsset, assetPair.priceAsset, buyerAnotherAsset, unitoken)
    sellerMatcherFeeAssetId <- Gen.oneOf(assetPair.amountAsset, assetPair.priceAsset, sellerAnotherAsset, unitoken)
    r <- Gen.oneOf(
  ...
  def exchangeV2GeneratorP(
      ...
      buyMatcherFeeAssetId: Asset = unitoken,
      sellMatcherFeeAssetId: Asset = unitoken,
      fixedMatcher: Option[KeyPair] = None
  ): Gen[ExchangeTransaction] = {
