...
import com.decentralchain.{TransactionGen, crypto}
 ...
 "minSponsoredAssetFee": null
  ...
  property("miner receives one satoshi less than sponsor pays") {
    val setup = for {
      ...
      transfer = TransferTransaction.selfSigned(1.toByte, acc, acc.toAddress, unitoken, 1L, feeAsset = IssuedAsset(issue.id()), minFee, ByteStr.empty,  ts).explicitGet()
    ...
    forAll(setup) {
      case (acc, genesis, issue, sponsor, transfer) =>
        ...
        assertNgDiffState(Seq(b0, b1), b2, NgAndSponsorshipSettings) {
          case (_, state) =>
            state.balance(acc.toAddress, unitoken) - ENOUGH_AMT shouldBe 0
        }
    }
  }
  ...
  property("miner receives one satoshi more than sponsor pays") {
    val setup = for {
      ...
      sponsor = SponsorFeeTransaction.selfSigned(1.toByte, acc, IssuedAsset(issue.id()), Some(minFee), One, ts).explicitGet()
      transfer1 = TransferTransaction.selfSigned(1.toByte, acc, acc.toAddress, unitoken, 1L, feeAsset = IssuedAsset(issue.id()), minFee + 7, ByteStr.empty,  ts)
        .explicitGet()
      transfer2 = TransferTransaction.selfSigned(1.toByte, acc, acc.toAddress, unitoken, 1L, feeAsset = IssuedAsset(issue.id()), minFee + 9, ByteStr.empty,  ts)
        .explicitGet()
  ...
  property("sponsorship changes in the middle of a block") {
    val setup = for {
      ...
      transfer1 = TransferTransaction.selfSigned(1.toByte, acc, acc.toAddress, unitoken, 1L, IssuedAsset(issue.id()), fee = minFee, ByteStr.empty,  ts).explicitGet()
      sponsor2  = SponsorFeeTransaction.selfSigned(1.toByte, acc, IssuedAsset(issue.id()), Some(minFee * 10), One, ts).explicitGet()
      transfer2 = TransferTransaction.selfSigned(1.toByte, acc, acc.toAddress, unitoken, 1L, IssuedAsset(issue.id()), fee = minFee * 10, ByteStr.empty,  ts).explicitGet()
    ...
    forAll(setup) {
      case (genesis, issue, sponsor, actualFee) =>
        ...
        assertDiffEi(Seq(b0), b1, NgAndSponsorshipSettings) { ei =>
          ei should produce(s"Fee for SponsorFeeTransaction ($actualFee in unitoken) does not exceed minimal value of $One unitoken.")
        }
