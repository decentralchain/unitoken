...
import com.decentralchain.transaction.assets.exchange.{AssetPair, Order, OrderType}
  ...
  property("AssetPair test") {
    forAll(assetIdGen, assetIdGen) { (assetA, assetB) =>
      whenever(assetA != assetB) {
        val assetAId: Asset = assetA.fold[Asset](unitoken)(arr => IssuedAsset(arr))
        val assetBId: Asset = assetB.fold[Asset](unitoken)(arr => IssuedAsset(arr))
