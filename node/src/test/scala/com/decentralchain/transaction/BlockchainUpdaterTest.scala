...
import com.decentralchain.{EitherMatchers, history}
...
class BlockchainUpdaterTest extends FreeSpec with Matchers with EitherMatchers with HistoryTest with ShouldVerb with WithDomain {
  ...
  private val unitokenSettings = history.DefaultunitokenSettings.copy(
    blockchainSettings = history.DefaultunitokenSettings.blockchainSettings.copy(
      functionalitySettings = history.DefaultunitokenSettings.blockchainSettings.functionalitySettings.copy(
        featureCheckBlocksPeriod = ApprovalPeriod,
        blocksForFeatureActivation = (ApprovalPeriod * 0.9).toInt,
        preActivatedFeatures = Map.empty
      )
    ),
    featuresSettings = history.DefaultunitokenSettings.featuresSettings.copy(autoShutdownOnUnsupportedFeature = true)
  )

  private val unitokenSettingsWithDoubling = unitokenSettings.copy(
    blockchainSettings = unitokenSettings.blockchainSettings.copy(
      functionalitySettings = unitokenSettings.blockchainSettings.functionalitySettings.copy(
  ...
  "features approved and accepted as height grows" in withDomain(unitokenSettings) { domain =>
  ...
  "features rollback with block rollback" in withDomain(unitokenSettings) { domain =>
    val b = domain.blockchainUpdater
    b.processBlock(genesisBlock)
  ...
  "feature activation height is not overriden with further periods" in withDomain(unitokenSettings) { domain =>
    val b = domain.blockchainUpdater
  ...
  "feature activated only by 90% of blocks" in withDomain(unitokenSettings) { domain =>
    val b = domain.blockchainUpdater
  ...
  "features votes resets when voting window changes" in withDomain(unitokenSettings) { domain =>
    val b = domain.blockchainUpdater
  ...
  "sunny day test when known feature activated" in withDomain(unitokenSettings) { domain =>
    val b = domain.blockchainUpdater
    b.processBlock(genesisBlock)
  ...
  "empty blocks should not disable activation" in withDomain(unitokenSettings) { domain =>
    val b = domain.blockchainUpdater
  ...
  "doubling of feature periods works in the middle of activation period" in withDomain(unitokenSettingsWithDoubling) { domain =>
    val b = domain.blockchainUpdater
  ...
  "doubling of feature periods should work after defined height" in withDomain(unitokenSettingsWithDoubling) { domain =>
    val b = domain.blockchainUpdater
