...
import com.decentralchain.{NTPTime, NoShrink, TransactionGen, crypto}
...
class ExchangeTransactionSpecification extends PropSpec with PropertyChecks with Matchers with TransactionGen with NTPTime with NoShrink {
    ...
    forAll(preconditions) {
      case (sender1, sender2, matcher, pair, buyerMatcherFeeAssetId, sellerMatcherFeeAssetId, versions) =>
        ...
        val buy = Order.buy(
          ...
          expirationTimestamp,
          buyMatcherFee,
          if (buyV == 3) buyerMatcherFeeAssetId else unitoken
        )
        val sell = Order.sell(
          sellV,
          sender2,
          ...
          expirationTimestamp,
          sellMatcherFee,
          if (sellV == 3) sellerMatcherFeeAssetId else unitoken
        )
        ...
        create(
          buyOrder = buy.copy(assetPair = buy.assetPair.copy(amountAsset = unitoken)),
  ...
  property("JSON format validation") {
    ...
    val buy = Order(
      ...
      AssetPair.createAssetPair("unitoken", "9ZDWzK53XT5bixkmMwTJi2YzgxCqn5dUajXFcT2HcFDy").get,
      OrderType.BUY,
      2,
      6000000000L,
      1526992336241L,
