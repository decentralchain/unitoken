  ...
  property("VersionedTransferTransactionSpecification id doesn't depend on proof") {
    forAll(accountGen, accountGen, proofsGen, proofsGen, attachmentGen) {
      case (_, acc2, proofs1, proofs2, attachment) =>
        val tx1 = TransferTransaction(2.toByte, acc2.publicKey, acc2.toAddress, unitoken, 1, unitoken, 1, attachment, 1, proofs1, acc2.toAddress.chainId)
        val tx2 = TransferTransaction(2.toByte, acc2.publicKey, acc2.toAddress, unitoken, 1, unitoken, 1, attachment, 1, proofs2, acc2.toAddress.chainId)
