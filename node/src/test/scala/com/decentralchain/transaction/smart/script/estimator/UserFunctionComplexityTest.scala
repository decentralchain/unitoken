...
import com.decentralchain.lang.v1.evaluator.ctx.impl.unitoken._
...
class UserFunctionComplexityTest(estimator: ScriptEstimator) extends PropSpec with PropertyChecks with Matchers with TypedScriptGen {
  ...
  property("estimate script for stdLib V1 with UserFunctions") {
    ...
    val exprunitokenBalance = FUNCTION_CALL(User("unitokenBalance"), List(CONST_STRING("alias").explicitGet()))
    est(exprunitokenBalance).explicitGet() shouldBe 110
  }
