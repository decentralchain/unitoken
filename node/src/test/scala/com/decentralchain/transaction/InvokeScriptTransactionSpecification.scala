...
import com.decentralchain.common.utils.{Base64, _}
import com.decentralchain.lang.v1.compiler.Terms
import com.decentralchain.lang.v1.compiler.Terms.{ARR, CONST_LONG, CaseObj}
import com.decentralchain.lang.v1.compiler.Types.CASETYPEREF
import com.decentralchain.lang.v1.{ContractLimits, FunctionHeader, Serde}
import com.decentralchain.protobuf.transaction._
...
import com.decentralchain.{EitherMatchers, TransactionGen, crypto}
...
class InvokeScriptTransactionSpecification extends PropSpec with PropertyChecks with Matchers with EitherMatchers with TransactionGen {
  ...
  property("Signed InvokeScriptTransactionRequest parser") {
    ...
    val req = SignedInvokeScriptRequest(
      ...
      payment = Some(Seq(Payment(1, unitoken))),
      dApp = "3Fb641A9hWy63K18KsBJwns64McmdEATgJd",
      timestamp = 11,
      proofs =
  ...
  property("can't have zero amount") {
    ...
    val req = SignedInvokeScriptRequest(
      ...
      payment = Some(Seq(Payment(0, unitoken))),
      dApp = "3Fb641A9hWy63K18KsBJwns64McmdEATgJd",
      timestamp = 11,
      proofs =
        Proofs(List("CC1jQ4qkuVfMvB2Kpg2Go6QKXJxUFC8UUswUxBsxwisrR8N5s3Yc8zA6dhjTwfWKfdouSTAnRXCxTXb3T6pJq3T").map(s => ByteStr.decodeBase58(s).get))
    )
    req.toTx shouldBe Left(NonPositiveAmount(0, "unitoken"))
    AddressScheme.current = DefaultAddressScheme
  }
  ...
  property("can't have negative amount") {
    ...
    val req = SignedInvokeScriptRequest(
      ...
      payment = Some(Seq(Payment(-1L, unitoken))),
      dApp = "3Fb641A9hWy63K18KsBJwns64McmdEATgJd",
      timestamp = 11,
      proofs =
        Proofs(List("CC1jQ4qkuVfMvB2Kpg2Go6QKXJxUFC8UUswUxBsxwisrR8N5s3Yc8zA6dhjTwfWKfdouSTAnRXCxTXb3T6pJq3T").map(s => ByteStr.decodeBase58(s).get))
    )
    req.toTx shouldBe Left(NonPositiveAmount(-1, "unitoken"))
    AddressScheme.current = DefaultAddressScheme
  }
}
