...
class MicroBlockSpecification extends FunSuite with Matchers with MockFactory with ShouldVerb {
  ...
  test("MicroBlock with txs bytes/parse roundtrip") {
    ...
    val tr: TransferTransaction  = TransferTransaction.selfSigned(1.toByte, sender, gen.toAddress, unitoken, 5, unitoken, 2, ByteStr.empty,  ts + 1).explicitGet()
    val assetId                  = IssuedAsset(ByteStr(Array.fill(AssetIdLength)(Random.nextInt(100).toByte)))
    val tr2: TransferTransaction = TransferTransaction.selfSigned(1.toByte, sender, gen.toAddress, assetId, 5, unitoken, 2, ByteStr.empty,  ts + 2).explicitGet()
  ...
  test("MicroBlock cannot contain more than Miner.MaxTransactionsPerMicroblock") {
    ...
    val transaction =
      TransferTransaction.selfSigned(1.toByte, sender, gen.toAddress, unitoken, 5, unitoken, 1000, ByteStr.empty,  System.currentTimeMillis()).explicitGet()
