...
class BlockSpecification extends PropSpec with PropertyChecks with TransactionGen with Matchers with NoShrink {
  ...
  val blockGen = for {
    ...
    paymentTransaction        <- unitokenTransferGeneratorP(time, sender, recipient.toAddress)
    transferTrancation        <- transferGeneratorP(1 + time, sender, recipient.toAddress, assetId, unitoken)
    anotherPaymentTransaction <- unitokenTransferGeneratorP(2 + time, sender, recipient.toAddress)
  ...
  def bigBlockGen(amt: Int): Gen[Block] =
    for {
      ...
      sender                                  <- accountGen
      recipient                               <- accountGen
      paymentTransaction: TransferTransaction <- unitokenTransferGeneratorP(time, sender, recipient.toAddress)
    } yield Block
      .buildAndSign(
        3.toByte,
        time,
