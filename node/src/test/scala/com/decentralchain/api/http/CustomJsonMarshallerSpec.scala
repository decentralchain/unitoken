...
import com.decentralchain.http.{ApiErrorMatchers, RestAPISettingsHelper}
...
import com.decentralchain.{NTPTime, NoShrink, TestWallet, TransactionGen}
...
class CustomJsonMarshallerSpec
  ...
  property("/transactions/calculateFee") {
    ...
    forAll(transferV2Gen) { tx =>
      (transactionsApi.calculateFee _).expects(*).returning(Right((Asset.unitoken, 1, 1))).twice()
  ...
  property("/blockchain/rewards") {
    ...
    (blockchain.unitokenAmount _).expects(*).returning(BigInt(10000000)).twice()
    (blockchain.blockRewardVotes _).expects(1000).returning(Seq(100L)).twice()

    checkRoute(Get("/blockchain/rewards/1000"), rewardRoute, "totalunitokenAmount", "currentReward", "minIncrement")
  }

  property("/debug/stateunitoken") {
