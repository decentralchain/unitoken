...
import com.decentralchain.common.merkle.Merkle._
...
import com.decentralchain.{BlockGen, NoShrink, TransactionGen}
...
class TransactionsRootSpec
  ...
  val commonGen: Gen[(KeyPair, List[TransferTransaction])] =
    for {
      ...
      txs       <- Gen.listOfN(txsLength, versionedTransferGeneratorP(sender, recipient.toAddress, unitoken, unitoken))
    } yield (signer, txs)
  ...
  val incorrectTransactionScenario: Gen[(Block, Transaction)] =
    for {
      ...
      tx               <- versionedTransferGeneratorP(anotherSender, anotherRecipient.toAddress, unitoken, unitoken)
      block            <- versionedBlockGen(txs, signer, Block.ProtoBlockVersion)
    } yield (block, tx)
