...
class BlockchainUpdaterBurnTest extends PropSpec with PropertyChecks with DomainScenarioDrivenPropertyCheck with Matchers with TransactionGen {
  ...
  val preconditions: Gen[Setup] = for {
    ...
    masterToAlice: TransferTransaction = TransferTransaction.selfSigned(1.toByte, master, alice.toAddress, Asset.unitoken, 3 * unitoken, Asset.unitoken, transferAssetunitokenFee, ByteStr.empty,  ts + 1)
      .explicitGet()
    issue: IssueTransaction = IssueTransaction(TxVersion.V1, alice.publicKey, assetName, description, quantity, decimals, false, script = None, unitoken, ts + 100)
      .signWith(alice.privateKey)
    burn: BurnTransaction = BurnTransaction.selfSigned(1.toByte, alice, IssuedAsset(issue.assetId), quantity / 2, unitoken, ts + 200).explicitGet()
    reissue: ReissueTransaction = ReissueTransaction
      .selfSigned(1.toByte, alice, IssuedAsset(issue.assetId), burn.quantity, true, unitoken, ts + 300)
      .explicitGet()
  ...
  val localunitokenSettings: unitokenSettings = settings.copy(blockchainSettings = localBlockchainSettings)

  property("issue -> burn -> reissue in sequential blocks works correctly") {
    scenario(preconditions, localunitokenSettings) {
