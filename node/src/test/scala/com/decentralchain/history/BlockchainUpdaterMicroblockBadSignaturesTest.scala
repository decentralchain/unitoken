...
class BlockchainUpdaterMicroblockBadSignaturesTest
  ...
  val preconditionsAndPayments: Gen[(GenesisTransaction, TransferTransaction, TransferTransaction)] = for {
    ...
    payment: TransferTransaction  <- unitokenTransferGeneratorP(master, recipient.toAddress)
    payment2: TransferTransaction <- unitokenTransferGeneratorP(master, recipient.toAddress)
