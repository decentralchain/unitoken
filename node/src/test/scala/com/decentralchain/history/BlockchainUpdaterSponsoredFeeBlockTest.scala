...
class BlockchainUpdaterSponsoredFeeBlockTest
  ...
  val sponsorPreconditions: Gen[Setup] = for {
    ...
    master                      <- accountGen
    ts                          <- timestampGen
    transferAssetunitokenFee       <- smallFeeGen
    _                           <- accountGen
    alice                       <- accountGen
    bob                         <- accountGen
    (feeAsset, sponsorTx, _, _) <- sponsorFeeCancelSponsorFeeGen(alice)
    unitokenFee                    = Sponsorship.tounitoken(sponsorTx.minSponsoredAssetFee.get, sponsorTx.minSponsoredAssetFee.get)
    ...
    masterToAlice: TransferTransaction = TransferTransaction
      .selfSigned(
        ...
        alice.toAddress,
        unitoken,
        feeAsset.fee + sponsorTx.fee + transferAssetunitokenFee + unitokenFee,
        unitoken,
        transferAssetunitokenFee,
        ByteStr.empty,
        ts + 1
      )
      .explicitGet()
  ...
  val SponsoredActivatedAt0unitokenSettings: unitokenSettings = settings.copy(blockchainSettings = SponsoredFeeActivatedAt0BlockchainSettings)

  property("not enough unitoken to sponsor sponsored tx") {
    scenario(sponsorPreconditions, SponsoredActivatedAt0unitokenSettings) {
      case (domain, (genesis, masterToAlice, feeAsset, sponsor, aliceToBob, bobToMaster, bobToMaster2)) =>
        ...
        domain.blockchainUpdater.processBlock(block4) should produce("negative unitoken balance" /*"unavailable funds"*/ )
    }
  }
    ...
    scenario(sponsorPreconditions, SponsoredActivatedAt0unitokenSettings) {
      case (domain, (genesis, masterToAlice, feeAsset, sponsor, aliceToBob, bobToMaster, _)) =>
...

        {
          ...
          val microBlocksunitokenFee = microBlocks
            .flatMap(_.transactionData)
            .map(tx => Sponsorship.calcunitokenFeeAmount(tx, ai => domain.blockchainUpdater.assetDescription(ai).map(_.sponsorship)))
            .sum

          domain.blockchainUpdater.bestLiquidDiffAndFees.map(_._3) should contain(block0TotalFee + microBlocksunitokenFee)
        }
    }
  }
