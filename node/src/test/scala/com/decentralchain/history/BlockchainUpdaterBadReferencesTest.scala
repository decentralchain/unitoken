...
class BlockchainUpdaterBadReferencesTest
  ...
  val preconditionsAndPayments: Gen[(GenesisTransaction, TransferTransaction, TransferTransaction, TransferTransaction)] = for {
    ...
    payment: TransferTransaction  <- unitokenTransferGeneratorP(ts, master, recipient.toAddress)
    payment2: TransferTransaction <- unitokenTransferGeneratorP(ts, master, recipient.toAddress)
    payment3: TransferTransaction <- unitokenTransferGeneratorP(ts, master, recipient.toAddress)
