...
class BlockchainUpdaterBlockMicroblockSequencesSameTransactionsTest
      ...
      case (gen, rest) =>
        val finalMinerBalances = rest.map {
          case (bmb: BlockAndMicroblockSequence, last: Block) =>
            withDomain(MicroblocksActivatedAt0unitokenSettings) { d =>
  ...
  property("Miner fee from microblock [Genesis] <- [Empty] <~ (Micro with tx) <- [Empty]") {
    val preconditionsAndPayments: Gen[(KeyPair, GenesisTransaction, TransferTransaction, Int)] = for {
      ...
      payment: TransferTransaction = createunitokenTransfer(master, master.toAddress, amt, fee, ts).explicitGet()
    ...
    val preconditionsAndPayments: Gen[(KeyPair, GenesisTransaction, Seq[Seq[TransferTransaction]], Int)] =
      for {
        ...
        microBlockTxs = (1 to txCount * microBlockCount)
          .map(step => createunitokenTransfer(master, master.toAddress, amt, fee, ts + step).explicitGet())
  ...
  def randomPayment(accs: Seq[KeyPair], ts: Long): Gen[TransferTransaction] =
    for {
      ...
      fee  <- smallFeeGen
      amt  <- smallFeeGen
    } yield createunitokenTransfer(from, to.toAddress, amt, fee, ts).explicitGet()
  ...
  def accsAndGenesis(): Gen[(Seq[KeyPair], KeyPair, Block, Int)] =
    for {
      ...
      miner   <- accountGen
      ts      <- positiveIntGen
      genesis1: GenesisTransaction = GenesisTransaction.create(alice.toAddress, TOTAL_unitoken / 4, ts).explicitGet()
      genesis2: GenesisTransaction = GenesisTransaction.create(bob.toAddress, TOTAL_unitoken / 4, ts + 1).explicitGet()
      genesis3: GenesisTransaction = GenesisTransaction.create(charlie.toAddress, TOTAL_unitoken / 4, ts + 2).explicitGet()
      genesis4: GenesisTransaction = GenesisTransaction.create(dave.toAddress, TOTAL_unitoken / 4, ts + 4).explicitGet()
    } yield (
