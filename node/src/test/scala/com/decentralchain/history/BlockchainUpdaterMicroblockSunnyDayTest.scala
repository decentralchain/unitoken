...
class BlockchainUpdaterMicroblockSunnyDayTest
  ...
  val preconditionsAndPayments: Gen[Setup] = for {
    ...
    masterToAlice: TransferTransaction <- unitokenTransferGeneratorP(ts, master, alice.toAddress)
    aliceToBob  = createunitokenTransfer(alice, bob.toAddress, masterToAlice.amount - fee - 1, fee, ts).explicitGet()
    aliceToBob2 = createunitokenTransfer(alice, bob.toAddress, masterToAlice.amount - fee - 1, fee, ts + 1).explicitGet()
  ...
  property("discarding some of microBlocks doesn't affect resulting state") {
    forAll(preconditionsAndPayments, accountGen) {
      case ((genesis, masterToAlice, aliceToBob, aliceToBob2), miner) =>
        ...
        val minerABalance = withDomain(MicroblocksActivatedAt0unitokenSettings) { da =>
        ...
        val minerBBalance = withDomain(MicroblocksActivatedAt0unitokenSettings) { db =>
