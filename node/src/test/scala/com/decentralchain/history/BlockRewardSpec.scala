...
import com.decentralchain.database.Keys
...
import com.decentralchain.{EitherMatchers, NoShrink, TransactionGen}
...
class BlockRewardSpec
  ...
  private val genesis = for {
    ...
    genesisBlock = TestBlock.create(
      ...
      Seq(
        GenesisTransaction
          .create(sourceAddress.toAddress, (Constants.Totalunitoken - 60000) * Constants.UnitsInWave, ntpTime.getTimestamp())
          .explicitGet(),
  ...
  "Miner receives reward and fees" - {
    ...
    "when NG state is empty" in forAll(ngEmptyScenario) {
      case (miner1, miner2, b2s, b3, m3s) =>
        withDomain(rewardSettings) { d =>
          ...
          d.db.get(Keys.blockMetaAt(Height(BlockRewardActivationHeight - 1))).map(_.totalFeeInunitoken) shouldBe OneTotalFee.some
          ...
          d.blockchainUpdater.liquidBlockMeta.map(_.totalFeeInunitoken) shouldBe 0L.some
          ...
          d.blockchainUpdater.liquidBlockMeta.map(_.totalFeeInunitoken) shouldBe OneTotalFee.some
    ...
    "when all blocks without fees" in forAll(blockWithoutFeesScenario) {
      case (miner1, miner2, b1s, b2, b3s, b4) =>
        withDomain(rewardSettings) { d =>
          val initialunitokenAmount = BigInt(Constants.Totalunitoken) * BigInt(Constants.UnitsInWave)
          ...
          d.blockchainUpdater.unitokenAmount(BlockRewardActivationHeight - 1) shouldBe initialunitokenAmount
          ...
          d.blockchainUpdater.height shouldBe BlockRewardActivationHeight
          d.blockchainUpdater.unitokenAmount(BlockRewardActivationHeight) shouldBe initialunitokenAmount + InitialReward
          ...
          b3s.zipWithIndex.foreach {
            case (b, i) =>
              ...
              d.blockchainUpdater.unitokenAmount(BlockRewardActivationHeight + i + 1) shouldBe initialunitokenAmount + BigInt(InitialReward * (i + 2))
          ...
          d.blockchainUpdater.unitokenAmount(BlockRewardActivationHeight + term) shouldBe initialunitokenAmount + term * InitialReward + InitialReward + minIncrement
