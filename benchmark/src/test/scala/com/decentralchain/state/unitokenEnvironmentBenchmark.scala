...
import com.decentralchain.database.{LevelDBFactory, LevelDBWriter}
...
import com.decentralchain.state.unitokenEnvironmentBenchmark._
...
/**
  ...
  * 2. Import it: https://github.com/decentralchain/unitoken/wiki/Export-and-import-of-the-blockchain#import-blocks-from-the-binary-file
...
class unitokenEnvironmentBenchmark {
  ...
  @Benchmark
  def accountBalanceOf_unitoken_test(st: AccountBalanceOfunitokenSt, bh: Blackhole): Unit = {
...
object unitokenEnvironmentBenchmark {
  ...
  class BaseSt {
    ...
    private val unitokenSettings: unitokenSettings = {
      val config = loadConfig(ConfigFactory.parseFile(new File(benchSettings.networkConfigFile)))
      unitokenSettings.fromRootConfig(config)
    }

    AddressScheme.current = new AddressScheme {
      override val chainId: Byte = unitokenSettings.blockchainSettings.addressSchemeCharacter.toByte
    }

    private val db: DB = {
      val dir = new File(unitokenSettings.dbSettings.directory)
      if (!dir.isDirectory) throw new IllegalArgumentException(s"Can't find directory at '${unitokenSettings.dbSettings.directory}'")
      LevelDBFactory.factory.open(dir, new Options)
    }

    val environment: Environment[Id] = {
      val state = LevelDBWriter.readOnly(db, unitokenSettings)
      new unitokenEnvironment(
