...
class unitokenEnvironmentRebenchmark {
  import unitokenEnvironmentRebenchmark._
  ...
  @Benchmark
  def unitokenBalanceOf(bh: Blackhole, st: St): Unit = {
    val useUnexisting = Random.nextBoolean()
    if (useUnexisting) {
...
object unitokenEnvironmentRebenchmark {
